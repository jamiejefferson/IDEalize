...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // `cordis` is also the fork's own event namespace (`cordis/request-run`,
  // `cordis/inspect-query`, …) and the locale namespace the Cordis panel
  // registers. Those keys are product data the runtime matches verbatim, not
  // package references, so the generic pass would rename an event nobody
  // emits and a dictionary nobody looks up.
