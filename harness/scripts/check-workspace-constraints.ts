...
/**
 * Directories whose packages this repository publishes: one release member
 * each. `packages/idealize/*` is excluded: the fork's packages ship inside the
 * desktop app and are never published, so they stay `private: true` under the
 * rule below (FORK.md).
 */
const releaseMemberDirectory = /^(?:packages\/(?!idealize\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/
