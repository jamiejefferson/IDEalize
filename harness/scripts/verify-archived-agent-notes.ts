...
/**
 * Sealed paths the frozen manifest still lists after their files were deleted:
 * the Chinese counterparts and pairing records removed with the rest of the
 * inherited corpus (JJ, 16 Sep 2026). They are ignored on both sides of the
 * append-only comparison; the next `--write` drops them.
 */
const REMOVED_CORPUS = /\.(?:zh\.md|i18n\.yaml)$/

function withoutRemovedCorpus(manifest: ArchiveManifest): ArchiveManifest {
  return {
    version: 1,
    files: Object.fromEntries(Object.entries(manifest.files).filter(([path]) => !REMOVED_CORPUS.test(path))),
  }
}
...
function readBaselineManifest(ref: string): ArchiveManifest {
  ...
  return withoutRemovedCorpus(parseArchiveManifest(runGit(['show', `${ref}:${manifestRepoPath}`])))
}

let manifest: ArchiveManifest = { version: 1, files: {} }
if (existsSync(manifestPath)) {
  try {
    manifest = withoutRemovedCorpus(parseArchiveManifest(readFileSync(manifestPath, 'utf8')))
  } catch (error: unknown) {
