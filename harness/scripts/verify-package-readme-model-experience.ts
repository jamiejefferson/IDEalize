...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/idealize/studio': { kind: 'none', reason: 'Host-side coordination record store and loopback routes; nothing it holds enters a model request (agents reach the timeline through the idealize CLI, whose tool results @idealize/comm owns).' },
  'packages/idealize/transcribe': { kind: 'none', reason: 'On-device speech-to-text behind loopback routes; the transcript reaches a model only as the message the person chooses to send, which @idealize/comm owns.' },
  'packages/idealize/telegram': { kind: 'indirect', reason: 'Posts the paired chat\'s text to the Studio and answers approvals and questions over the API proxy; the text reaches a model only as the Studio delivery whose mailbox note @idealize/comm owns.' },
  'packages/idealize/askbar': { kind: 'none', reason: 'Desktop-window renderer plus loopback roster/transform routes; nothing it produces enters a model request (panel sends land in mailboxes whose tokens @idealize/comm owns).' },
  ...
  // IDEalize fork packages (packages/idealize/*).
  'packages/idealize/appearance': { kind: 'none', reason: 'Browser presentation and a settings section no model-facing plugin reads; registers nothing model-facing.' },
  'packages/idealize/feedback': { kind: 'none', reason: 'Proxies feedback and announcement rows between the browser and Supabase; touches no model request.' },
  'packages/idealize/freetokens': { kind: 'indirect', reason: 'Registers the freetokens provider route in the llm-pi-ai settings section and an HTTP attribution header; the adapter owns every model request that route serves.' },
  'packages/idealize/hatch': { kind: 'indirect', reason: 'Opens the service source as a project; the workspace-context plugins own how that folder\'s AGENTS.md reaches the model.' },
  'packages/idealize/host-bridge': { kind: 'none', reason: 'Observes host events and serves them to shells as a notification feed; nothing it holds enters a model request.' },
  'packages/idealize/models': { kind: 'indirect', reason: 'The panel and auto policy write the default-model selection into dsh-agent-default-model\'s settings section; request assembly and the adapters own the model-visible request.' },
  'packages/idealize/notify': { kind: 'none', reason: 'The banner, chime and notification read settings and the host bridge event feed and register nothing that reaches a model request.' },
  'packages/idealize/provider-pack': { kind: 'indirect', reason: 'The OAuth credential store authenticates routes the forked llm-pi-ai adapter serves; the adapter owns the model-visible request.' },
  'packages/idealize/skin': { kind: 'none', reason: 'The package injects a stylesheet and splash markup into the served page and writes one settings acknowledgement; nothing reaches a model request.' },
  'packages/idealize/ui-terminal': { kind: 'none', reason: 'The embedded shell runs its own process; its output never enters the session log or a model request.' },
  'packages/idealize/ui-tour': { kind: 'none', reason: 'Browser-side tour overlay and keybind registry; registers nothing model-facing.' },
  'packages/idealize/vault': { kind: 'indirect', reason: 'The commit-evidence appends change project notes that @idealize/doc-policy injects into session-start context; that package owns the model-visible text.' },
  'packages/idealize/gen-fixture': { kind: 'indirect', reason: 'The fixture generation backend renders nothing to a model; the generate_* tools of @idealize/gen-tools own the results that name it.' },
  'packages/idealize/generate': { kind: 'indirect', reason: 'The generation seam, media presets and routes register nothing model-facing; the generate_* tools of @idealize/gen-tools render dispatch outcomes.' },
  'packages/idealize/onboarding': { kind: 'none', reason: 'The first-run wizard writes settings, credentials and brain model choices through host routes; the configured brains own every model-facing effect.' },
  'packages/idealize/services': { kind: 'none', reason: 'The list joins the chat-route and generation-backend registries and stores API keys; the routes and backends those keys authenticate own every model-visible request.' },
  'packages/idealize/setup': { kind: 'none', reason: 'The plugin stores folder aliases and orientation seeds; @idealize/doc-policy owns the documentation context the seeded folder feeds.' },
  'packages/idealize/ui-launch': { kind: 'none', reason: 'Browser-only opening-sequence overlay and stylesheet; registers nothing model-facing.' },
  'packages/idealize/ui-gallery': { kind: 'indirect', reason: 'The aspect and count controls write a trailing tag into the user draft, which reaches the model as ordinary user-message text; the view registers nothing model-facing.' },
  'packages/idealize/ui-schedule': { kind: 'indirect', reason: 'Create-with-chat sends one seeded user message into a session made for it; the pane registers no tool or prompt contribution.' },
  'packages/idealize/ui-studio': { kind: 'none', reason: 'Browser-only pane over @idealize/studio\'s loopback routes; it reads state, opens chats, and registers nothing model-facing.' },
  'packages/idealize/ui-tasks': { kind: 'none', reason: 'The column renders the todos projection the model already wrote through todo_write; it registers no tool, contributes no prompt text, and nothing done in it reaches a model request.' },
  'packages/idealize/ui-soundstage': { kind: 'indirect', reason: 'The length picker and Retry write user-message text into the composer draft; the view registers nothing model-facing.' },
  'packages/idealize/web-search-openrouter': { kind: 'indirect', reason: 'The provider backend delegates model rendering to dsh-tool-web.' },
}
