 ...
 *
 * Agent Notes are historical records and are not scanned: their links to the
 * Chinese corpus and translation tooling removed on 16 Sep 2026 stay as
 * written. A target ending in `.zh.md` or `.i18n.yaml` is skipped everywhere
 * for the same reason.
 */
...
/** Link targets of the removed Chinese corpus; a link to one is not judged. */
...
export function findViolations(
  ...
  const check = (url: string, node: Nodes): void => {
    ...
    if (REMOVED_CORPUS.test(target)) return
...
if (process.argv[1] && import.meta.filename === resolve(process.argv[1])) {
  // Agent Notes remain valid link targets; their own outbound links are not scanned.
