...
for (const manifestPath of manifests) {
  ...
  // A build that splits a module two entries share emits a hashed chunk beside
  // them. A `files` list naming its entries by name drops that chunk, and the
  // package then fails to load from its tarball while passing every in-repo
  // check (packages/idealize/notify, landing 21; @idealize/spaces before it).
  const emitted = globSync('lib/*.js', { cwd: packageDir })
  const declared = new Set(
    (manifest.files ?? [])
      .filter(pattern => pattern.startsWith('lib/'))
      .flatMap(pattern => globSync(pattern, { cwd: packageDir })),
  )
  const dropped = emitted.filter(file => !declared.has(file)).sort()
  if (dropped.length > 0) {
    failures.push(`${packageName}: the build emits ${dropped.join(', ')}, which "files" does not publish — list lib/*.js`)
    continue
  }
