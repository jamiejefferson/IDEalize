...
describe('rewriteMarkdown', () => {
  ...
  it('resolves a source alias to the page that declares it', () => {
    const { root, pages } = fixture()
    mkdirSync(join(root, 'docs/guide'))
    writeFileSync(join(root, 'docs/guide/index.md'), '# Guide\n')
    const aliased: DocsPage[] = [
      ...pages,
      { source: 'docs/guide/index.md', sourceAliases: ['docs/guide'], route: 'guide/quickstart.md', label: 'Guide', sidebar: 'guide', section: 'Test', order: 1 },
    ]
    expect(rewriteMarkdown('[guide](guide) [B](b.md)\n', {
      sourcePath: 'docs/a.md',
      route: 'a.md',
      pages: aliased,
      repoRoot: root,
      repositoryRef: 'abc123',
    })).toBe('[guide](./guide/quickstart.md) [B](./reference/b.md)\n')
  })
...
describe('docsPages routes', () => {
  ...
  it('publishes every route once from an existing English source', () => {
  ...
  it('publishes the Cordis core API under one section', () => {
...
describe('sidebar ordering', () => {
  ...
  it('refuses a section with no declared placement', () => {
    expect(() => sectionSpec('Data structures'))
      .toThrow('Sidebar section "Data structures" has no placement.')
  })
