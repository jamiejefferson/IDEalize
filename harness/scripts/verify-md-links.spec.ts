...
describe('findViolations fragments', () => {
  ...
  it('does not judge a link to the removed Chinese corpus', () => {
    // Agent Notes keep their historical switcher lines and pairing citations.
    const root = layout({ 'a.md': '# A\n\nEnglish | [中文](a.zh.md)\n\n[record](a.i18n.yaml) [ghost](missing.md)\n' })
    expect(violationsIn(root, 'a.md')).toEqual([{ url: 'missing.md', reason: 'target' }])
  })
})
