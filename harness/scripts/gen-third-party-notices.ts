...
function renderNonPermissiveNote(deps: ExternalDep[]): string {
  ...
  return `\n${subject} ${named.length === 1 ? 'runs' : 'run'} only as development tooling; their code is not linked into or distributed with any IDEalize harness artifact.\n`
}
...
IDEalize V1 is a fork of [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness); both are licensed under [MIT](LICENSE). It depends on the third-party software listed below. Each project remains under its own license; nothing in this file changes those terms.
