...
- Reconnect states are user-visible in logs: reconnecting (warn, with attempt count and delay, once per outage; the outage's later failed attempts log at debug), recovered (info), final failure and disabled-loss (error). Disposal cancels any pending reconnect. With `reconnect.enabled: false`, a lost connection keeps tools registered but failing until a reload — the manual-recovery behavior.
