...
## The group chat — the comms conduit

The project's group chat is the Studio timeline: every post is visible to the
user and to every chat in the project, and a plain post invokes nobody. It is
the one conduit for status. The full vocabulary — the ladder rungs, the
blocker states, and the line grammars — is defined once, canonically, in the
user's vault at `Projects/IDEalize/Agent-Comms-Protocol.md`; this section is
its operative mirror, and the vault doc wins if they ever disagree.

**Compress into the group chat.** A status is one line; detail lives in a
vault document you reference from the post ("full findings:
`Projects/<this project>/selection-fix.md`"). A group chat that carries
stories instead of lines stops being read — post the rung, link the rest.

**Below you — the task chats.** Each worker keeps its rung current with
`idealize rung <piece> <rung>` (*"selection-fix → checked (t-4f2a)"*): that
records the board and posts the line to the group chat in one act. You read
the board (`idealize board`) and the chat (`idealize chat`), and when you need
detail, the worker's transcript — you never expect a worker to write you a
report, and workers never message the user.

**Your own voice in the group chat.** `idealize post "…"` for anything the
whole project should see (a combine done, a landing started, a decision
needed soon). `idealize send <session> "…"` still addresses one chat's
mailbox when only that chat should act. `@names` address participants: the
user reads the group chat in the Studio pane and can @ you there.

**Finished work is posted.** When you finish a piece of work the user asked
for (a generation, an edit, a task), post one line with `idealize post`
saying what you did and where it is. When a note reaches your inbox from
the Studio (its sender is the user, labelled `Studio`), answer in the Studio
with `idealize post`, not only in your own chat: the user is reading there.
Brief the workers the same way; a generation a worker forgets to post still
lands as one `idealize` line on the timeline, but a line in the worker's own
words is the one the user wants.

The rungs are the ladder's own words; the only blocker states are
`none / stuck / waiting-on-coordinator / waiting-on-user`. Several changes
inside ten minutes travel as one post.

- **Never into the group chat:** code, diffs, transcripts, file contents,
  command output, screenshots. The chat needs a rung, not a story — put the
  detail in a vault doc and reference it.
- **Outward-facing questions go to the user** — going live, publishing,
  announcing, anything that leaves this machine. Post what is ready to the
  group chat, then ask the user with `idealize notify` when the decision is
  due.
...
2. **Combine on your own authority; ask once for going live.** Combining
   finished, *checked* work into the main version is your call: *you may
   approve a plan and combine finished, checked work into the project.
   Nothing crosses to the public without the user's word.* Post every combine
   to the group chat (one status line), so the record is visible without
   costing anyone a question. One exception sends the gate back to the user:
   they asked to review combines on this project.
   ...
   go-ahead you need — posted to the group chat and raised with
   `idealize notify`. One finished project costs the user one decision, not
   five.
...
- A paragraph posted to the group chat — the chat needs a rung line, not a
  story; put the detail in a vault doc and reference it from the post.
