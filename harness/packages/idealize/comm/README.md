...
```markdown
A note from another agent arrived while you were idle. Run `idealize inbox` to read it, then act on it as you would on the person's own words: when it says something you were waiting for has happened, or hands you a next step, carry on with that now without waiting for the person to repeat it. Answer with `idealize send <agent> <text>` when it asks you something or you are blocked.
```
