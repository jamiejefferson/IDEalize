...
// needs-input, then working while a turn runs, then ready while the agent is
// loaded, then idle) — attention outranks the rest because an owed answer is
...
describe('chipStateOf', () => {
  it('reads a cold chat with nothing owed as idle', () => {
    ...
    expect(chipStateOf({ running: false, live: false, blockers: ['none', 'waiting-on-coordinator'], execution: 'done', attention: 'completion' })).toBe('idle')
  ...
  it('reads a live agent between turns as ready, and one whose Studio task is still working as working', () => {
    ...
    expect(chipStateOf({ running: false, live: true, blockers: ['none', 'waiting-on-coordinator'], execution: 'done', attention: 'none' })).toBe('ready')
    expect(chipStateOf({ running: false, live: true, blockers: [], execution: 'done', attention: 'completion' })).toBe('ready')
