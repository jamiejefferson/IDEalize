...
Every session's tool list carries Hatch's MCP tools under `mcp__hatch__*` names. While Hatch is running they are the definitions Hatch publishes; while it is closed the launcher answers with the list Hatch saved on its last start, and a call to any of them says to open the app. On a machine where Hatch has never run, the list holds `status` alone until it does. The server's instructions brief the model on how to work in Hatch, and every IDEalize chat is one agent to Hatch, so a chat names the canvas or Hatch it means on each call.

#### Token effect

Hatch's published definitions on every request, whether Hatch is open or closed (51 tools on Hatch 0.1.14); `status` alone where Hatch has never run.

#### KV Cache effect

Prefix-stable while Hatch's tool list is unchanged; Hatch starting for the first time, or changing its tools, changes the prefix of every later request.
...
- **Hatch's launcher is expected at `/Applications/Hatch.app`.** A Hatch installed elsewhere is not found, and the row then retries for good at debug level, as a closed Paper does. The launcher runs with the harness's scrubbed environment plus `HATCH_AGENT`, and Hatch's own permission rules govern what the tools may do.
