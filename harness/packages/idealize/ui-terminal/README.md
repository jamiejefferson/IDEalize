...
None, as the embedded shell runs its own process and its output never enters the chat's session log or any model request; the chat's brain and model are chosen by `@idealize/activity-pills` and `@idealize/spaces`.

#### KV Cache effect

Independent: opening, restarting or closing a shell changes nothing in the chat's request.
