 ...
 * locale service, and Host settings load/refresh. (IDEalize fork: the
 * Appearance row no longer registers — the bar's theme-cycle button owns the
 * preference until the dedicated Appearance pane lands.) */
...
describe('ui-theme apply', () => {
  ...
  it('provides the service and registers localized copy, without an Appearance row', async () => {
    const b = await bench()
    ...
    // The fork keeps appearance out of General: the bar cycle button (and the
    // future Appearance pane) own the preference.
