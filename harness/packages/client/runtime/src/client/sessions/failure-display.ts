...
export function displayFailureMessage(failure: unknown): string {
  ...
  // Raw engine copy names env vars and provider routes; the full diagnostic
  // stays in the session log.
  if (record.code === 'MISSING_CREDENTIAL') {
    return 'This model isn’t connected yet. Open the model panel to sign in or add a key, then try again.'
  }
