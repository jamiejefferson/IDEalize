...
export function contextProvenance(source: unknown): ContextProvenanceView {
  ...
  switch (kind) {
    ...
    case 'plugin':
      return { role: 'inject', label: plainProducerName(readString(record, 'plugin') ?? kind) }
    ...
    default:
      return { role: 'inject', label: plainProducerName(kind) }
  }
}

/**
 * Present a package-shaped producer id ("@deepseek-ai/dsh-system-prompt",
 * "skill-catalog") as plain words ("System prompt", "Skill catalog"). The
 * durable record keeps the real id; only the transcript label is softened.
 * @param id - producer id exactly as recorded.
 * @returns human-readable label.
 */
function plainProducerName(id: string): string {
  const bare = id.replace(/^@[^/]+\//, '').replace(/^dsh-/, '')
  if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(bare)) return id
