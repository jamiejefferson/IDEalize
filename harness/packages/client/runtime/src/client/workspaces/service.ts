...
export class WorkspaceRuntime implements IWorkspaces {
   ...
   * {@link noteSessionConfigured}.
   */
  private readonly configured = new Set<SessionId>()
   ...
   * list mirror and nothing has been recorded on it, else create a fresh one
   * on the host (`session.create` births the full Session+Agent — the client
   * holds no intermediate state). The
  ...
  async connectWorkspace(workspaceId: WorkspaceId): Promise<SessionId> {
    ...
    // Nor is a blank session a plugin has already put to work: an empty log
    // makes it reusable, a recorded brain makes it a conversation the user has
    // already set up, and New Session must mint a fresh one rather than
    // handing the same one back with no question asked. The projection says so
    // one round trip after the write that produces it, so `configured` carries
    // the same fact locally from the moment the caller starts the work. A project's Studio
    // chat (the `studio` space) is blank for its whole life and is that
  ...
  /**
   * Mark a blank session as put to work, so the reuse scan skips it before the
   * Host's projection lands.
   * @param sessionId - the blank session the caller is configuring.
   * @returns a disposer that lifts the marker, for a caller whose work failed.
   */
  noteSessionConfigured(sessionId: SessionId): () => void {
    this.configured.add(sessionId)
    return () => { this.configured.delete(sessionId) }
  }
