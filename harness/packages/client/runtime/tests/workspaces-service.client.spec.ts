...
describe('WorkspaceRuntime', () => {
  ...
  it('connectWorkspace mints a fresh session rather than reusing a blank one a plugin has recorded a brain on', async () => {
    ...
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('alpha', [sid('s-launched'), sid('s-idle')])] as never[],
    }))
    api.onList = () => Promise.resolve(ok({
      items: [
        // Blank and accounted, but launched into a space on a brain: the user
        // has already set this conversation up, so New Session must not hand
        // it back with no question asked.
        {
          sessionId: sid('s-launched'), updatedAt: 5, running: false, blank: true, cwd: '/w/alpha',
          projections: { asOfSeq: 3, values: { space: { space: 'gallery' }, brain: { brain: 'gallery-brain' } } },
        },
        // Blank, accounted, and carrying only the derived space every chat
        // projects: still the reuse hit.
        {
          sessionId: sid('s-idle'), updatedAt: 2, running: false, blank: true, cwd: '/w/alpha',
          projections: { asOfSeq: 1, values: { space: { space: 'chat' } } },
        },
      ] as never[],
    }))
    await Promise.all([workspaces.refresh(), sessions.refresh()])
    ...
    await expect(workspaces.connectWorkspace(wid('alpha'))).resolves.toBe('s-idle')
    expect(api.callsOf('session.create')).toEqual([])

    // With only the launched chat left, New Session mints one.
  ...
  it('connectWorkspace skips a blank session a caller has marked, before any projection says so', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('alpha', [sid('s-launching')])] as never[],
    }))
    // The state the race leaves behind: a launcher has begun configuring this
    // chat, and the Host projection that will record the brain has not
    // returned. On the durable rule alone this row is the reuse hit.
    api.onList = () => Promise.resolve(ok({
      items: [{
        sessionId: sid('s-launching'), updatedAt: 5, running: false, blank: true, cwd: '/w/alpha',
        projections: { asOfSeq: 1, values: { space: { space: 'chat' } } },
      }] as never[],
    }))
    await Promise.all([workspaces.refresh(), sessions.refresh()])
    ...
    const lift = workspaces.noteSessionConfigured(sid('s-launching'))
    api.onCreate = () => Promise.resolve(ok({ sessionId: sid('s-fresh') }))
    await expect(workspaces.connectWorkspace(wid('alpha'))).resolves.toBe('s-fresh')
    expect(api.callsOf('session.create')).toEqual([{ workspaceId: 'alpha' }])

    // A launch that failed lifts its own marker, and the chat it never
    // configured goes back to being the reuse hit.
    lift()
    await expect(workspaces.connectWorkspace(wid('alpha'))).resolves.toBe('s-launching')
    expect(api.callsOf('session.create')).toEqual([{ workspaceId: 'alpha' }])
  })

  it('connectWorkspace never reuses a project\'s Studio chat, which stays blank for life', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('alpha', [sid('s-studio')])] as never[],
    }))
    api.onList = () => Promise.resolve(ok({
      items: [{
        sessionId: sid('s-studio'), updatedAt: 5, running: false, blank: true, cwd: '/w/alpha',
        projections: { asOfSeq: 2, values: { space: { space: 'studio' } } },
      }] as never[],
    }))
    await Promise.all([workspaces.refresh(), sessions.refresh()])
    await Promise.resolve()
