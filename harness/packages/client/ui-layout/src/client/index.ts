...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * The banner strip: a full-width grid row ABOVE every column (V0 pinned
     * its announcement banner under the title bar, pushing the columns down).
     * Content here owns its ground; empty entries cost no height. Root-scoped.
     */
    'shell.banner': { kind: 'list'; scope: 'root' }
    /**
     * The right drawer column, a docked grid sibling of the other columns —
     * content here pushes the center aside instead of covering it.
     * `ctx.layout` owns whether it is open (openDrawer/closeDrawer); the
     * occupant owns what it shows. Root-scoped: it renders with or without a
     * current session.
     */
    'shell.drawer': { kind: 'single'; scope: 'root'; owner: DrawerOwnerProps }
    /**
     * The deck: a second docked column between center and the drawer, sized
     * for documents (V0's file/document panel). `ctx.layout` owns whether it
     * is open (openDeck/closeDeck); the user owns its width via the drag
     * handle; the occupant owns what it shows. Root-scoped.
     */
    'shell.deck': { kind: 'single'; scope: 'root'; owner: DeckOwnerProps }
    /**
     * The bottom dock strip, a full-width grid row under every column.
     * Content here has its own layout ground — nothing in the columns can
     * sit underneath it. Empty entries cost no height.
     */
     ...
     */
    'shell.aside': { kind: 'single'; scope: 'session-maybe'; owner: AsideOwnerProps }
  }
}
...
/** Drawer owner share: empty — ctx.layout owns open/closed. */
export interface DrawerOwnerProps {}

/** Deck owner share: empty — ctx.layout owns open/closed, the drag handle owns width. */
export interface DeckOwnerProps {}
