...
beforeEach(() => {
  ...
  Element.prototype.getBoundingClientRect = function (this: Element) {
    // The rail column measures its stubbed occupant width (0 when empty), the
    // aside is empty in every case here; everything else reads the frame width.
    const width = this.className.includes('railCol')
      ? railWidth
      : this.className.includes('asideCol') ? 0 : frameWidth
    return { width, height: 1080, top: 0, left: 0, right: width, bottom: 1080, x: 0, y: 0, toJSON: () => ({}) }
  }
  // The frame lays its columns out from offsetWidth (layout px, zoom-free);
  // jsdom reports 0, so it reads the same stub as the rect at zoom 1.
  Object.defineProperty(HTMLElement.prototype, 'offsetWidth', {
    configurable: true,
    get(this: HTMLElement) {
      if (this.className.includes('railCol')) return railWidth
      if (this.className.includes('asideCol')) return 0
      return frameWidth
    },
  })
...
describe('AppFrame', () => {
  ...
  it('drag base is the rendered (concession-clamped) width, not the preference', () => {
    frameWidth = 1090 // step-2 squeeze: details renders 330 while preference is 360
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openDetails() })
