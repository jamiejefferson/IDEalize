...
describe('ui-sidebar apply', () => {
  ...
  it('registers the shell and declares its child seats', async () => {
    ...
    expect(b.slots.spec('sidebar.rail')).toEqual({ kind: 'single', scope: 'root' })
    // Copy rides the standard locale seat, not the inject face.
    expect(b.slots.entries('sidebar')[0]!.locale).toBe('sidebar')
    const injected = (b.slots.entries('sidebar')[0]!.inject as () => SidebarRootInjected)()
    expect(Object.keys(injected)).toEqual(['startSession', 'toggleSidebar', 'useRailOccupied'])
  ...
  it('removes the entry and child declaration on teardown', async () => {
    ...
    expect(b.slots.spec('sidebar.rail')).toBeUndefined()
  })
})
