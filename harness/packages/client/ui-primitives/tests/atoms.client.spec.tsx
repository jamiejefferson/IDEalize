...
describe('Menu', () => {
  ...
  it('a submenu search filters rows by label and keywords, drops empty groups, picks the first match on Enter, and holds the card open while focused', () => {
    const onSelect = vi.fn()
    render(
      <Menu
        open
        ...
        items={[{
          id: 'parent',
          label: 'Parent',
          submenuSearch: { placeholder: 'Search…', empty: 'No matches' },
          submenu: [
            { type: 'label', id: 'h1', text: 'Skills' },
            { id: 'a', label: 'alpha', keywords: 'brand guidelines' },
            { id: 'b', label: 'beta' },
            { type: 'separator', id: 's1' },
            { type: 'label', id: 'h2', text: 'Commands' },
            { id: 'c', label: '/plan' },
          ],
        }]}
        onSelect={onSelect}
    ...
    expect(screen.getAllByRole('menuitem').map(item => item.textContent)).toEqual(['Parent', 'alpha', 'beta', '/plan'])
    // Keywords match; the Commands heading and its separator go with their emptied group.
    fireEvent.change(search, { target: { value: 'BRAND' } })
    const sub = screen.getAllByRole('menu')[1] as HTMLElement
    expect([...sub.querySelectorAll('[role]')].map(node => `${node.getAttribute('role')}:${node.textContent}`))
      .toEqual(['presentation:Skills', 'menuitem:alpha'])
    // The pointer leaving the card does not close it while the field has focus.
    fireEvent.mouseLeave(wrap)
    ...
    // Reopening starts with a blank search.
    search.blur()
    fireEvent.mouseLeave(wrap)
  ...
  it('renders heading labels and separators inside a submenu, neither selectable', () => {
    const onSelect = vi.fn()
    render(
      <Menu
        open
    ...
    const sub = screen.getAllByRole('menu')[1] as HTMLElement
    expect([...sub.children].map(child => child.getAttribute('role'))).toEqual(['presentation', 'menuitem', 'separator', 'presentation', 'menuitem'])
    expect(screen.getByText('Skills').getAttribute('role')).toBe('presentation')
