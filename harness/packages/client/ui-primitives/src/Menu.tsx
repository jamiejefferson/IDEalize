...
export interface MenuItem {
  ...
  danger?: boolean
  /** Nested card opened to the right on hover/focus; separators and heading labels render there as in the primary list. */
  submenu?: readonly MenuEntry[]
  /** Extra text a submenu search matches besides the label (a description, aliases). */
  keywords?: string
  /**
   * A search field at the top of this row's submenu: typing filters the rows
   * by label and keywords, headings and separators left without a row drop
   * out, Enter picks the first match, and the card stays open while the
   * field has focus. `empty` shows when nothing matches.
   */
  submenuSearch?: { placeholder: string; empty: string }
}
...
/**
 * The submenu rows a search query leaves: items whose label text or keywords
 * contain the query (case-insensitive), plus the headings and separators
 * that still introduce at least one visible item.
 * @param entries - the submenu.
 * @param query - the typed query; blank keeps everything.
 * @returns the rows to render, in order.
 */
export function visibleSubmenu(entries: readonly MenuEntry[], query: string): readonly MenuEntry[] {
  const needle = query.trim().toLowerCase()
  if (needle === '') return entries
  const matches = (item: MenuItem): boolean => {
    const text = `${typeof item.label === 'string' ? item.label : ''} ${item.keywords ?? ''}`.toLowerCase()
    return text.includes(needle)
  }
  const out: MenuEntry[] = []
  let pending: MenuEntry[] = []
  for (const entry of entries) {
    if (isSeparator(entry) || isLabel(entry)) {
      pending.push(entry)
      continue
    }
...
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, getAnchorRect, footer, className }: {
  ...
  const [openSubmenuId, setOpenSubmenuIdState] = useState<string | null>(null)
  const [subQuery, setSubQuery] = useState('')
  const searchRef = useRef<HTMLInputElement | null>(null)
  // A submenu opens with an empty search every time.
  const setOpenSubmenuId = (id: string | null): void => {
    setOpenSubmenuIdState(id)
    setSubQuery('')
  }
  ...
  const renderEntry = (entry: MenuEntry) => {
    ...
    return (
      <div
        ...
        onMouseLeave={() => {
          // Typing in the submenu search keeps the card open when the pointer drifts off it.
      ...
      >
        ...
        {subOpen && entry.submenu !== undefined && (() => {
          const search = entry.submenuSearch
          const visible = search === undefined ? entry.submenu : visibleSubmenu(entry.submenu, subQuery)
          const first = visible.find((sub): sub is MenuItem => !isSeparator(sub) && !isLabel(sub) && sub.disabled !== true)
          const rows = visible.map((sub) => {
            if (isSeparator(sub)) return <div key={sub.id} className={css.separator} role="separator" />
            if (isLabel(sub)) return <div key={sub.id} className={css.label} role="presentation">{sub.text}</div>
            return (
              <button
                key={sub.id}
                type="button"
          ...
          if (search === undefined) {
            return <div className={clsx(css.submenu, compact && css.compactList)} role="menu">{rows}</div>
          }
          return (
            <div className={clsx(css.submenu, css.searchable, compact && css.compactList)} role="menu">
              <input
                ref={searchRef}
                type="search"
                className={css.submenuSearch}
                placeholder={search.placeholder}
                aria-label={search.placeholder}
                autoFocus
                value={subQuery}
                onChange={(e) => { setSubQuery(e.currentTarget.value) }}
                onKeyDown={(e) => {
              ...
              />
              <div className={css.submenuScroll}>
                {first === undefined && subQuery !== ''
                  ? <div className={css.label} role="presentation">{search.empty}</div>
                  : rows}
              </div>
            </div>
          )
