...
function makeHarness(init?: Partial<ConversationSnapshot>) {
  ...
  const chatScroll: ChatViewSlotProps['chatScroll'] = {
    ...
    read: () => savedScroll,
    takeReveal: () => {
      const at = revealTarget
      revealTarget = null
      return at
    },
  }
  const setReveal = (at: number | null): void => { revealTarget = at }
  const forkAt = vi.fn()
  ...
  return {
    ...
    chatScroll, setReveal, forkAt, setSelection, toolOwners,
  }
}
...
describe('ChatView', () => {
  ...
  it('a pending reveal target lands on its turn instead of the reader position, once', () => {
    const h = makeHarness({
      nodes: [user(1, 'q'), assistant(2, 'a'), user(5, 'later'), assistant(6, 'b', 2)],
      turnTimings: new Map([[1, { startTime: 1_500 }], [2, { startTime: 5_500 }]]),
      turnEnds: new Map([[1, 2], [2, 6]]),
    })
    // A stored reader position that the reveal must outrank.
    h.chatScroll.save({ anchorKey: 'fixture:user:1', anchorTop: 10, scrollTop: 40 })
    h.setReveal(6_000)
    const view = render(<h.ChatView {...h.props} />)
    const revealed = view.container.querySelector('[data-chat-revealed]')
    expect(revealed?.getAttribute('data-chat-anchor-key')).toBe('fixture:user:5')
    // One-shot: consumed by this mount.
    expect(h.chatScroll.takeReveal()).toBeNull()
  })

  it('a reveal older than every loaded turn lands on the transcript head', () => {
    const h = makeHarness({
      nodes: [user(5, 'later'), assistant(6, 'b', 2)],
      turnTimings: new Map([[2, { startTime: 5_500 }]]),
      turnEnds: new Map([[2, 6]]),
    })
    h.setReveal(100)
    const view = render(<h.ChatView {...h.props} />)
    expect(view.container.querySelector('[data-chat-revealed]')?.getAttribute('data-chat-anchor-key')).toBe('fixture:user:5')
  })
