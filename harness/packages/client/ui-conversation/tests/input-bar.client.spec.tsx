...
import type { ComposerMenuEntry, ComposerMenuRow, ComposerMenuSession } from '../src/client/composer-menu.ts'
import {
...
interface BenchOptions {
  ...
  composerMenu?: readonly ComposerMenuEntry[]
}
...
function bench(over?: BenchOptions) {
  ...
  const composerMenu = createSnapshotStore<readonly ComposerMenuEntry[]>(over?.composerMenu ?? [])
  ...
  const renderSlot = ((key: string, owner: object, opts?: { fallback?: React.ReactNode }) => {
    ...
    // The access seat is unoccupied in this bench, so it renders the bar's own
    // chip through the fallback, the way the real renderer does.
    if (key === 'conversation.input.access') return over?.accessEntry ?? opts?.fallback ?? null
    return null
  ...
  const props: InputBarProps = {
    ...
    enterInserts: over?.enterInserts ?? (() => false),
    toggleCommandMenu: over?.toggleCommandMenu ?? vi.fn(),
    useNotices: bindSnapshotSelector(shell.notices),
    useLexicon: bindSnapshotSelector(shell.lexicon),
    useMenuLauncher: bindSnapshotSelector(menuLauncher),
    useComposerMenu: bindSnapshotSelector(composerMenu),
    stop,
  ...
  return {
    ...
    menuLauncher, composerMenu,
    steerQueue: over?.steerQueue,
  }
}

describe('attach control', () => {
  /** Ordinary deployment limits: the attach split only reads `mediaTypes`. */
  const LIMITS = {
  ...
  const picker = (view: { container: HTMLElement }) =>
    view.container.querySelector<HTMLInputElement>('input[type="file"]')!

  /** Fire the picker's change with a fixed file list, which jsdom will not set. */
  const pick = (input: HTMLInputElement, files: readonly File[]): void => {
    Object.defineProperty(input, 'files', { value: files, configurable: true })
    fireEvent.change(input)
  }

  it('sends picked images down the same intake as a paste', () => {
    const addImages = vi.fn(() => null)
    const { view } = bench({ addImages, imageLimits: LIMITS })
    const image = new File([Uint8Array.of(1)], 'a.png', { type: 'image/png' })
    pick(picker(view), [image])
    expect(addImages).toHaveBeenCalledWith([image])
  })

  it('announces a document it cannot resolve to a path rather than attaching nothing', () => {
    const addImages = vi.fn(() => null)
    const { view, shell } = bench({ addImages, imageLimits: LIMITS })
    pick(picker(view), [new File(['x'], 'notes.md', { type: 'text/markdown' })])
    expect(addImages).not.toHaveBeenCalled()
    expect(view.getByText('这个文件只能拖入或粘贴')).toBeTruthy()
    expect(shell.snapshot.draft).toBe('')
  })

  it('appends a resolved document path to the draft', () => {
    const addImages = vi.fn(() => null)
    const bridge = { getPathForFile: (file: File) => `/docs/${file.name}` }
    Object.defineProperty(window, '__DSH_DESKTOP_FILE_PATH__', { value: bridge, configurable: true })
    try {
      const { view, textarea, shell } = bench({ addImages, imageLimits: LIMITS })
      fireEvent.change(textarea, { target: { value: '读一下' } })
      pick(picker(view), [new File(['x'], 'notes.md', { type: 'text/markdown' })])
      expect(shell.snapshot.draft).toBe('读一下 /docs/notes.md ')
    } finally {
      Reflect.deleteProperty(window, '__DSH_DESKTOP_FILE_PATH__')
    }
  })
})
...
describe('Enter semantics', () => {
  ...
  it('Enter-to-send off: plain Enter falls through to the native newline and Cmd/Ctrl+Enter submits', () => {
    const { textarea, sink } = bench({ draft: 'hello', enterInserts: () => true })
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(sink).not.toHaveBeenCalled() // native newline, no submission
    fireEvent.keyDown(textarea, { key: 'Enter', metaKey: true })
    expect(sink).toHaveBeenCalledWith('hello', [], 'queue')
  })
...
describe('the "+" menu and control seats', () => {
  it('renders the "+" launcher; the Access chip is absent without the permissions projection; the control seats render EMPTY without entries', () => {
    ...
    expect(slotCalls.map(c => c.key)).toEqual([
      'conversation.input.access', 'conversation.input.plan', 'conversation.input.model',
    ])
    expect(view.queryByLabelText('Plan mode')).toBeNull()
    expect(view.queryByLabelText('Model')).toBeNull()
  })

  it('lists files first, then contributed entries, then Commands, which passes the textarea selection', () => {
    const toggleCommandMenu = vi.fn()
    const onSelect = vi.fn()
    const entry: ComposerMenuEntry = { id: 'plain', order: 10, label: 'Plain entry', onSelect }
    const { view, textarea } = bench({ draft: 'draft text', toggleCommandMenu, composerMenu: [entry] })
    ...
    expect(items.map(item => item.textContent)).toEqual(['添加文件或照片⌘U', 'Plain entry', '/命令'])
    // An entry without rows is itself the pick.
    fireEvent.click(items[1]!)
    expect(onSelect).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ sessionId: SID }), undefined)
  ...
  it('reads a contributed entry\'s rows on open, shows them as a submenu, and a pick inserts through the session', async () => {
    let settle: (rows: readonly ComposerMenuRow[]) => void = () => {}
    const rows = vi.fn(() => new Promise<readonly ComposerMenuRow[]>((resolve) => { settle = resolve }))
    const onSelect = vi.fn((session: ComposerMenuSession, row: string | undefined) => { session.insertText(`/${row} `) })
    const entry: ComposerMenuEntry = { id: 'skills', order: 10, label: 'Skills', rows, onSelect }
    const { view, shell } = bench({ draft: 'read', composerMenu: [entry] })
    fireEvent.click(view.getByLabelText('添加'))
    expect(rows).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ sessionId: SID }))
    const parent = view.getByRole('menuitem', { name: 'Skills' })
    fireEvent.mouseEnter(parent.parentElement!)
    expect((view.getByRole('menuitem', { name: '加载中…' }) as HTMLButtonElement).disabled).toBe(true)
    await act(async () => { settle([{ id: 'deploy', label: 'deploy' }]) })
    fireEvent.click(view.getByRole('menuitem', { name: 'deploy' }))
    expect(onSelect).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ sessionId: SID }), 'deploy')
    expect(shell.snapshot.draft).toBe('read /deploy ')
    expect(view.queryByRole('menuitem')).toBeNull()
    // Reopening reads the rows afresh.
    fireEvent.click(view.getByLabelText('添加'))
    expect(rows).toHaveBeenCalledTimes(2)
  })

  it('an entry that offers commands replaces the Commands row, and its heading rows render as labels', async () => {
    const toggleCommandMenu = vi.fn()
    const onSelect = vi.fn()
    const entry: ComposerMenuEntry = {
      id: 'skills',
      order: 10,
      ...
      search: true,
      rows: () => [
        { id: 'heading:skills', label: 'Skills', heading: true },
        { id: 'skill:deploy', label: 'deploy', keywords: 'ship it' },
        { id: 'heading:commands', label: 'Commands', heading: true },
        { id: 'command:plan', label: '/plan' },
      ],
      onSelect,
    }
    const { view } = bench({ toggleCommandMenu, composerMenu: [entry] })
    fireEvent.click(view.getByLabelText('添加'))
    expect(view.getAllByRole('menuitem').map(item => item.textContent)).toEqual(['添加文件或照片⌘U', 'Skills & Commands'])
    await act(async () => { await Promise.resolve() })
    const parent = view.getByRole('menuitem', { name: 'Skills & Commands' })
    fireEvent.mouseEnter(parent.parentElement!)
    const submenu = view.getAllByRole('menu')[1]!
    // A search field leads; a hairline precedes every heading after the first.
    const search = view.getByLabelText('搜索…') as HTMLInputElement
    expect(submenu.firstElementChild).toBe(search)
    expect([...submenu.querySelectorAll('[role]')].map(node => `${node.getAttribute('role')}:${node.textContent}`))
      .toEqual(['presentation:Skills', 'menuitem:deploy', 'separator:', 'presentation:Commands', 'menuitem:/plan'])
    fireEvent.change(search, { target: { value: 'ship' } })
    expect(view.getAllByRole('menuitem').map(item => item.textContent)).toEqual(['添加文件或照片⌘U', 'Skills & Commands', 'deploy'])
    fireEvent.change(search, { target: { value: '' } })
    fireEvent.click(view.getByText('Commands'))
    expect(onSelect).not.toHaveBeenCalled()
    fireEvent.click(view.getByRole('menuitem', { name: '/plan' }))
    expect(onSelect).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ sessionId: SID }), 'command:plan')
    expect(toggleCommandMenu).not.toHaveBeenCalled()
  })

  it('headings alone count as no rows', async () => {
    const headingsOnly: ComposerMenuEntry = {
      id: 'bare', order: 20, label: 'Bare', rows: () => [{ id: 'h', label: 'Only a heading', heading: true }], onSelect: vi.fn(),
    }
    const { view } = bench({ composerMenu: [headingsOnly] })
    fireEvent.click(view.getByLabelText('添加'))
    await act(async () => { await Promise.resolve() })
    fireEvent.mouseEnter(view.getByRole('menuitem', { name: 'Bare' }).parentElement!)
    expect((view.getByRole('menuitem', { name: '暂时没有' }) as HTMLButtonElement).disabled).toBe(true)
  })

  it('shows an empty row for an entry with no rows and for one whose read fails', async () => {
    const empty: ComposerMenuEntry = { id: 'empty', order: 10, label: 'Empty', rows: () => [], onSelect: vi.fn() }
    const failing: ComposerMenuEntry = {
      id: 'failing', order: 20, label: 'Failing', rows: () => Promise.reject(new Error('offline')), onSelect: vi.fn(),
    }
    const { view } = bench({ composerMenu: [empty, failing] })
    fireEvent.click(view.getByLabelText('添加'))
    await act(async () => { await Promise.resolve() })
    for (const label of ['Empty', 'Failing']) {
      const parent = view.getByRole('menuitem', { name: label })
      fireEvent.mouseEnter(parent.parentElement!)
      expect((view.getByRole('menuitem', { name: '暂时没有' }) as HTMLButtonElement).disabled).toBe(true)
  ...
  it('the files row and ⌘U both open the file picker', () => {
    const { view, textarea } = bench({ addImages: () => null })
    const input = view.container.querySelector<HTMLInputElement>('input[type="file"]')!
    const click = vi.spyOn(input, 'click').mockImplementation(() => {})
    fireEvent.click(view.getByLabelText('添加'))
    fireEvent.click(view.getByRole('menuitem', { name: /添加文件或照片/ }))
    ...
    fireEvent.keyDown(textarea, { key: 'u' })
    expect(click).toHaveBeenCalledTimes(3)
  })
