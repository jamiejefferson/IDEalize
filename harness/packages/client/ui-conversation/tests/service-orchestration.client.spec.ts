...
async function bench(readAttachment?: SessionFace['readAttachment'], reveals?: Map<SessionId, number>) {
...
describe('ConversationController', () => {
  ...
  it('records an explicitly addressed one-shot reveal target, last write winning', async () => {
    const reveals = new Map<SessionId, number>()
    const b = await bench(undefined, reveals)
    b.root.revealAt('s1' as SessionId, 1_000)
    b.root.revealAt('s1' as SessionId, 2_000)
    expect(reveals.get('s1' as SessionId)).toBe(2_000)
    await b.runtime.dispose()
  })
