...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string }) => {
    ...
    if (key === 'conversation.hero.workspace') {
      // The menu-less second instance (the launcher's New-project action) is not the picker under test.
      if ((owner as { addOnly?: boolean }).addOnly !== true) pickerOwner = owner
      return null
    }
    // The shipped occupant (HeroLauncherDefault) renders the owner share bare;
    // the mock does the same so the chip and the recent cards stay reachable.
    if (key === 'conversation.hero.launcher') {
      const share = owner as { projectRow: ReactNode; recentProjects: ReactNode }
      return <>{share.projectRow}{share.recentProjects}</>
    }
    ...
    if (key === 'conversation.composer.bar') {
      ...
      return (
        <InputBar
          ...
          useComposerMenu={bindSnapshotSelector(createSnapshotStore<readonly ComposerMenuEntry[]>([]))}
          stop={stop}
...
describe('Hero chrome', () => {
  it('renders the headline without a preview badge', () => {
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} />)
    expect(view.getByText('What shall we Idealize?')).toBeTruthy()
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero recents: cards offer other workspaces, exclude the current one, and pick on click', () => {
    const b = mount(
      conversationSnapshot({ composerPhase: 'blank', blank: true }),
      [
        { ...workspace('one'), sessionIds: [SID] },
        { ...workspace('second'), title: 'Recent Project' },
      ],
    )
    // The current workspace ('one') gets no card; the other one does.
    const card = b.view.getByRole('button', { name: /Recent Project/ })
    expect(card.textContent).toContain('/projects/second')
    expect(b.view.queryByRole('button', { name: /\/projects\/one/ })).toBeNull()
  ...
  it('keeps the Chat fallback selected by id when a view is inserted before it', () => {
    ...
    // The ring renders no tab row: the header carries no per-view control.
    expect(b.view.queryAllByRole('tab')).toHaveLength(0)
  ...
  it('rolls the pending workspace label back when switching fails', async () => {
    ...
    // Chip-scoped: after rollback the pending title leaves the CHIP; the
    // recent-project cards may legitimately still offer that workspace.
    const chip = b.view.getByRole('button', { name: '选择工作区' })
    expect(chip.textContent).not.toContain('Selected Folder')
  ...
  it('blank session keeps the interactive picker chip (workspace switchable until the first message)', () => {
    ...
    // choices are only open before the first message. The row rides the
    // launcher seat's owner share.
    expect(b.slotCalls).toContain('conversation.hero.launcher')
