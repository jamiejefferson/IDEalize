...
export interface IConversation {
  ...
  /**
   * Aim a session's next chat mount at an instant (epoch ms): the chat view
   * scrolls to the turn nearest it instead of restoring the reader position —
   * Studio's source navigation (FR-P0-23). One-shot: a later call overwrites
   * it, and the mount that lands consumes it. Explicitly addressed, unlike
   * the scoped operations above, because the caller navigates to another
   * session's chat.
   * @param sessionId - the session whose next chat mount consumes the target.
   * @param atMs - the instant to land near.
   */
  revealAt(sessionId: SessionId, atMs: number): void
}
...
export class ConversationController extends Service implements IConversation {
  ...
  private readonly reveals: Map<SessionId, number>
   ...
   * factories close over), plus the reveal-target map the chat view's inject
   * reads (own empty map when absent, for benches that never mount the view).
   */
  constructor(ctx: Context, config: { input: SessionInputResolver; blocks: ComposerBlocks; reveals?: Map<SessionId, number> }) {
    super(ctx, 'conversation')
    this.input = config.input
    this.blocks = config.blocks
    this.reveals = config.reveals ?? new Map<SessionId, number>()
    ctx.effect(() => () => {
      this.disposed = true
  ...
  /** Aim a session's next chat mount at an instant (see {@link IConversation.revealAt}). */
  revealAt(sessionId: SessionId, atMs: number): void {
    this.reveals.set(sessionId, atMs)
  }
