...
/** Breathing room above a revealed row, so the landing reads as a place. */
const REVEAL_MARGIN = 24

/**
 * The row a reveal target lands on: the first node of the turn whose start is
 * nearest at-or-before the instant (the transcript head when every loaded
 * turn starts later), resolved through the snapshot's turn clocks and seq
 * bounds so no per-kind payload is read. Null when nothing is rendered.
 */
function revealRow(
  list: HTMLElement,
  order: readonly string[],
  nodes: { get(key: string): { readonly anchorSeq: number } | undefined },
  turnTimings: ReadonlyMap<number, { readonly startTime: number }>,
  turnEnds: ReadonlyMap<number, number>,
  atMs: number,
): HTMLElement | null {
  let turn: number | null = null
  for (const [candidate, timing] of turnTimings) {
    if (timing.startTime > atMs) continue
    if (turn === null || timing.startTime > (turnTimings.get(turn)?.startTime ?? Number.NEGATIVE_INFINITY)) turn = candidate
  }
  const afterSeq = turn === null ? -1 : turnEnds.get(turn - 1) ?? -1
  const key = order.find((candidate) => {
    const seq = nodes.get(candidate)?.anchorSeq
    return seq !== undefined && seq > afterSeq
  }) ?? order.at(-1)
  return key === undefined ? null : anchorElement(list, key)
}
...
export function ChatView({
  ...
  useLayoutEffect(() => {
    ...
    if (openState === 'open' && !openedRef.current) {
      ...
      // A pending reveal target (Studio's source navigation) outranks the
      // reader position: land at the turn nearest the instant, mark the row.
      const revealAt = chatScroll.takeReveal()
      const revealed = revealAt === null ? null : revealRow(local, order, nodeStore, turnTimings, turnEnds, revealAt)
      if (revealed !== null) {
        el.scrollTop += flowTop(revealed, el) - REVEAL_MARGIN
        revealed.dataset.chatRevealed = ''
        observedTopRef.current = el.scrollTop
