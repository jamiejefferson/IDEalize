...
export const StatsLine = memo(function StatsLine({ useSession, useProjection, t }: StatsLineProps) {
  ...
  // An empty row keeps its seat, so the composer card sits at the same height
  // over a chat that has no turns yet (the Studio's, a fresh chat) as over one
  // that has (JJ, 14 Sep 2026: the Studio's ask bar sat tight to the floor).
  if (groups.length === 0) return <div className={css.root} aria-hidden />
  return (
