...
export function ConversationRoot({
  ...
  // The project row (chip + picker + preset chip) rides the launcher's owner
  // share: the shipped occupant (HeroLauncherDefault) renders it bare above
  // the recents; a composition may shadow the seat with its own welcome card.
  ...
  const projectRow = (
    <div className={css.heroWorkspaceRow}>
      ...
      {/* A second picker instance that skips the menu: the launcher's
          New-project action raises the folder flow directly. Always mounted:
          the flow consumes the open request by closing the (absent) menu and
          then owns its own dialog state, so unmounting on close would drop
          the dialog. */}
      {renderSlot('conversation.hero.workspace', {
        open: newProjectOpen,
        addOnly: true,
  ...
  const inputBar = renderSlot('conversation.composer.bar', {
    ...
    accessory: hero
      ? renderSlot('conversation.hero.launcher', {
        projectRow,
        projectOpen: chipTitle !== undefined,
        recentProjects: (
          <RecentProjects
            workspaces={workspaces.items}
            sessions={sessionsById}
            excludeId={pendingWorkspaceId ?? sessionWorkspace?.workspaceId}
            onPick={pickWorkspace}
            t={t}
          />
        ),
        onNewProject: () => { setNewProjectOpen(true) },
        onFindProject: () => { setPickerOpen(true) },
      })
      : undefined,
  ...
  return (
    <div className={css.root} data-phase={phase} data-idealize-surface="chat">
