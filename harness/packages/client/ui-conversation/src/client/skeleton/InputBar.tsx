...
import type { ComposerMenuEntry, ComposerMenuRow, ComposerMenuSession } from '../composer-menu.ts'
...
/**
 * The desktop preload's file-path bridge, read structurally off the window:
 * the browser build has none, and a File from an `<input type=file>` carries
 * no disk path of its own. Only the desktop shell can answer, so a document
 * picked in a browser deployment is announced as unattachable rather than
 * attached wrongly.
 */
const FILE_PATH_BRIDGE = '__DSH_DESKTOP_FILE_PATH__'

/**
 * Resolve one picked file to its path on disk.
 * @param file - the picked file.
 * @returns the absolute path, or undefined outside the desktop shell.
 */
function diskPath(file: File): string | undefined {
  const bridge = (window as unknown as Record<string, { getPathForFile?: (f: File) => string } | undefined>)[FILE_PATH_BRIDGE]
  if (typeof bridge?.getPathForFile !== 'function') return undefined
  const path = bridge.getPathForFile(file).trim()
  return path === '' ? undefined : path
}

/** The attach control's glyph: a paperclip, as V0 had it (JJ, 11 Sep 2026). */
...
export function InputBar({
  ...
  resolveSubmitMode, enterInserts, toggleCommandMenu, stop, command, t,
  renderSlot, useNotices, useLexicon, useMenuLauncher, useComposerMenu,
  ...
  const menuEntries = useComposerMenu(entries => entries)
  ...
  const attachable = addImages !== undefined && !locked
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>): void => {
    ...
    // ⌘U / Ctrl+U opens the file picker, the "+" menu's first row.
    if ((e.metaKey || e.ctrlKey) && !e.shiftKey && !e.altKey && (e.key === 'u' || e.key === 'U')) {
      e.preventDefault()
      if (attachable) fileInputRef.current?.click()
      return
    }
    ...
    // Enter-to-send off: plain Enter is the native newline (no preventDefault)
    // and only the Cmd/Ctrl-accelerated chord reaches submission.
    if (!accelerated && enterInserts()) return
    e.preventDefault()
  ...
  // The attach control's picker. Images go down the same intake as a paste or
  // a drop; everything else reaches the agent the way the Files pane's "add to
  // chat" sends a document — its path appended to the draft, which is the only
  // handle the model has on a file it must open itself.
  // Text a pick lands in the draft, after a space when the draft does not
  // already end in one; the "+" menu's rows and picked document paths share it.
  const appendToDraft = useCallback((text: string): void => {
    const draftNow = inputRef.current?.value ?? ''
    const lead = draftNow === '' || draftNow.endsWith(' ') ? draftNow : `${draftNow} `
    // Undefined only with no session, where the menu is disabled anyway.
    inputActions?.setDraft(`${lead}${text}`)
  }, [inputActions])
  const onPickFiles = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
    const picked = [...event.target.files ?? []]
    // Same input element every time, so a re-pick of one file still fires.
    event.target.value = ''
    if (picked.length === 0) return
    const types = imageLimits?.mediaTypes as readonly string[] | undefined
    const images = picked.filter(file => (types ?? []).includes(file.type))
    const documents = picked.filter(file => !images.includes(file))
    if (images.length > 0) intakeImages(images)
    if (documents.length === 0) return
    const paths = documents.flatMap(file => diskPath(file) ?? [])
    if (paths.length < documents.length) showToast(t('input.attachUnavailable'))
    if (paths.length === 0) return
    appendToDraft(`${paths.join(' ')} `)
  }, [appendToDraft, imageLimits, intakeImages, showToast, t])
  ...
  // The "+" menu: files first, then what plugins contributed (each read
  // afresh on open, a loading row while a promise settles), Commands last.
  // Portaled: the tool row scrolls sideways (overflow-x: auto), which clips
  // anything opening above it (JJ, 15 Sep 2026: "the modal is broken").
  const [plusOpen, setPlusOpen] = useState(false)
  const [loadedRows, setLoadedRows] = useState<ReadonlyMap<string, readonly ComposerMenuRow[] | 'loading'>>(new Map())
  const menuSession = useMemo<ComposerMenuSession | undefined>(
    () => (sessionId === undefined ? undefined : { sessionId, insertText: appendToDraft }),
    [sessionId, appendToDraft],
  )
  useEffect(() => {
    if (!plusOpen || menuSession === undefined) {
      setLoadedRows(current => (current.size === 0 ? current : new Map()))
      return
    }
    let live = true
    const next = new Map<string, readonly ComposerMenuRow[] | 'loading'>()
    for (const entry of menuEntries) if (entry.rows !== undefined) next.set(entry.id, 'loading')
    setLoadedRows(next)
    for (const entry of menuEntries) {
      if (entry.rows === undefined) continue
      Promise.resolve(entry.rows(menuSession)).then(
        (rows) => { if (live) setLoadedRows(current => new Map(current).set(entry.id, rows)) },
        () => { if (live) setLoadedRows(current => new Map(current).set(entry.id, [])) },
      )
    }
    return () => { live = false }
  }, [plusOpen, menuEntries, menuSession])
  const rowKey = (entry: ComposerMenuEntry, row: ComposerMenuRow): string => `row:${entry.id}:${row.id}`
  const menuItems = useMemo((): MenuEntry[] => {
    ...
    if (addImages !== undefined) {
      items.push({
        ...
        label: (
          <span className={css.menuRow} data-composer-attach>
            <span className={css.menuRowLabel}>{t('menu.attach')}</span>
            <kbd className={css.menuHint}>⌘U</kbd>
          </span>
        ),
        disabled: !attachable,
      })
    }
    if (menuEntries.length > 0 && items.length > 0) items.push({ type: 'separator', id: 'sep:contributed' })
    for (const entry of menuEntries) {
      const loaded = entry.rows === undefined ? undefined : loadedRows.get(entry.id)
      const submenu = entry.rows === undefined
        ? undefined
        : loaded === undefined || loaded === 'loading'
          ? [{ id: `row:${entry.id}:`, label: t('menu.loading'), disabled: true }]
          : loaded.every(row => row.heading === true)
            ? [{ id: `row:${entry.id}:`, label: t('menu.empty'), disabled: true }]
            : loaded.flatMap((row, index): MenuEntry[] => (row.heading === true
              ? [
                ...index === 0 ? [] : [{ type: 'separator' as const, id: `${rowKey(entry, row)}:rule` }],
                { type: 'label', id: rowKey(entry, row), text: row.label },
              ]
              : [{
                id: rowKey(entry, row),
                label: row.label,
      ...
      items.push({
        ...
        ...entry.search === true ? { submenuSearch: { placeholder: t('menu.search'), empty: t('menu.noMatch') } } : {},
        disabled: menuSession === undefined,
      })
    }
    if (toggleCommandMenu !== undefined && !menuEntries.some(entry => entry.commands === true)) {
      if (items.length > 0) items.push({ type: 'separator', id: 'sep:commands' })
      items.push({ id: 'commands', icon: <span className={css.slashGlyph} aria-hidden>/</span>, label: t('input.commands'), disabled: locked })
    }
    return items
  }, [addImages, attachable, menuEntries, loadedRows, menuSession, toggleCommandMenu, locked, t])
  const onMenuPick = (id: string): void => {
    ...
    for (const entry of menuEntries) {
      if (id === `entry:${entry.id}`) {
        entry.onSelect(menuSession, undefined)
        return
      }
      if (entry.rows === undefined) continue
      const rows = loadedRows.get(entry.id)
      if (rows === undefined || rows === 'loading') continue
      const row = rows.find(candidate => rowKey(entry, candidate) === id)
      if (row !== undefined) {
        entry.onSelect(menuSession, row.id)
        return
      }
    }
  }
  ...
  return (
    <div className={clsx(css.root, variant === 'hero' && css.hero)}>
      ...
      >
        ...
        <div className={css.row} data-input-row>
          <div className={css.tools}>
            <Menu
              ...
              portal
              anchor={
                <Tooltip label={t('input.menu')} side="top" delayMs={500}>
                  <button
                    type="button"
                    className={css.add}
                    aria-label={t('input.menu')}
                    aria-haspopup="menu"
                    aria-expanded={plusOpen}
                    data-composer-menu
                    disabled={locked && !plusOpen}
            ...
            <div className={css.modes}>
              {renderSlot('conversation.input.access', { locked, control: accessSelect }, { fallback: accessSelect })}
