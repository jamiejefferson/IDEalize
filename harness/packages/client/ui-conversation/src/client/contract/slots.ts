...
import type { ComposerMenuEntry } from '../composer-menu.ts'
import type {
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * The launcher strip at the top of the hero composer card (mode toggle +
     * activity chips in the fork's welcome design). Root scope: no session
     * exists yet; the filler stages choices for the next one.
     */
    'conversation.hero.launcher': { kind: 'single'; scope: 'root'; owner: HeroLauncherOwnerProps }
    ...
    /**
     * The access-mode seat at the left end of the composer tool row, holding
     * the permission chip. Unoccupied it renders that chip, so the bar's
     * default composer is unchanged; an occupant receives the chip as
     * `control` ({@link InputAccessOwnerProps}) and decides where it goes —
     * the seat exists so a product can gather the row's chips into one
     * overflow control without the bar knowing what that control is.
     */
    'conversation.input.access': { kind: 'single'; scope: 'session'; owner: InputAccessOwnerProps }
    /**
...
/**
 * Hero launcher owner props. The shell owns the project choice (picker
 * state, pending pick, retarget) and hands the launcher the rendered pieces;
 * the occupant decides how the welcome card lays them out. The shipped
 * occupant renders the row over the recents; a lower-priority registration
 * shadows it.
 */
export interface HeroLauncherOwnerProps {
  /** The project chip, its picker menu, and the agent-preset chip (switchable until the first message). */
  projectRow: ReactNode
  /** Recent-project cards for the no-project welcome, null when none exist. */
  recentProjects: ReactNode
  /** Whether the blank session already sits in a project. */
  projectOpen: boolean
  /** Raise the folder flow straight away (new or existing folder becomes a project). */
  onNewProject: () => void
  /** Open the project picker menu (existing projects + add). */
  onFindProject: () => void
}
...
export interface ComposerBarInjected {
  ...
  /** Whether plain Enter types a newline instead of submitting (the persisted Enter-to-send preference, inverted). */
  enterInserts: () => boolean
  /** Toggle the shared slash menu with only its command source (the "+" menu's Commands row);
   * absent without ui-input-trigger or a session. */
  ...
  hooks: {
    ...
    /** The entries plugins contributed to the composer's "+" menu (`ctx.composerMenu`), sorted. */
    composerMenu: ObservableSnapshot<readonly ComposerMenuEntry[]>
  }
}
...
/**
 * The access seat's owner share: the lock plus the bar's own access control,
 * already rendered. An occupant that only wants to reposition the control
 * (into an overflow menu, say) renders `control` and adds nothing of its own;
 * one replacing the affordance ignores it.
 */
export interface InputAccessOwnerProps extends InputControlOwnerProps {
  /** The bar's permission chip, ready to place. */
  control: ReactNode
}
...
export type ComposerBarProps =
  ...
  & PropsRenderSlots<'conversation.input.plan' | 'conversation.input.model' | 'conversation.input.access'>
...
export interface ChatViewInjected {
  ...
  chatScroll: {
    ...
    /**
     * Consume the session's pending reveal target (epoch ms), written by
     * `conversation.revealAt` before this chat mounted: the view scrolls to
     * the turn nearest that instant instead of restoring the reader position
     * and marks the landed row `data-chat-revealed`. One-shot — the read
     * clears it, so a later remount restores normally.
     */
    takeReveal: () => number | null
  }
...
export interface EmptyWorkspaceOwnerProps {
  ...
  onClose: () => void
  /** Skip the menu: opening raises the directory flow directly (the hero's New-project action). */
  addOnly?: boolean
}
