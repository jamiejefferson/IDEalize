...
import { ReturnToggle } from './input/ReturnToggle.tsx'
...
import { createComposerMenu } from './composer-menu.ts'
...
export function apply(ctx: Context): void {
  // The composer "+" menu's registry: contributors write it, the bar reads it.
  const composerMenu = createComposerMenu()
  ctx.provide('composerMenu', composerMenu)
  ...
  // The Return-to-send preference, resident in the composer strip (V0's
  // toggle idiom) — its single home.
  ctx.slots.inject('conversation.input.right', () => ctx.slots.register({
    name: 'conversation.input.right',
    id: 'composer-return-toggle',
    order: 5,
    locale: NS,
    inject: (): ReturnToggleInjected => ({
      hooks: { enterToSend: submissionPolicy.enterToSend },
      setEnterToSend: (enabled) => { submissionPolicy.setEnterToSend(enabled) },
    }),
  }, ReturnToggle))
  ...
  // One-shot reveal targets (epoch ms) by session: written by
  // conversation.revealAt (Studio's source navigation, FR-P0-23), consumed by
  // the chat view's next mount of that session.
  const chatReveals = new Map<SessionId, number>()
  ...
  slots.register({
    ...
    children: {
      ...
      'conversation.hero.launcher': { kind: 'single', scope: 'root' },
    },
    ...
    inject: (sessionId: SessionId | undefined): ComposerBarInjected => {
      if (sessionId === undefined) {
        return {
          ...
          enterInserts: () => !submissionPolicy.enterToSend.getSnapshot(),
          toggleCommandMenu: undefined,
          stop: undefined,
          command: undefined,
          hooks: {
            notices: ABSENT_NOTICES, lexicon: ABSENT_LEXICON, menuLauncher: ABSENT_MENU_LAUNCHER, composerMenu: composerMenu.entries,
          },
        }
      }
      const conversation = concreteConversation(ctx)
      ...
      return {
        ...
        hooks: {
          ...
          composerMenu: composerMenu.entries,
        },
      }
    },
  }, InputBar)
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        ...
        chatScroll: {
          ...
          takeReveal: () => {
            const at = chatReveals.get(sessionId) ?? null
            chatReveals.delete(sessionId)
            return at
          },
        },
        forkAt: (seq) => {
  ...
  // The hero card's accessory: project row + recents. Shadowable (priority 0).
  slots.register({ name: 'conversation.hero.launcher', locale: NS }, HeroLauncherDefault)
  ...
  ctx.plugin(ConversationController, { input: inputHub, blocks: composerBlocks, reveals: chatReveals })
