 ...
 * require, returns the exports (apply + inject), and a mounted apply publishes
 * the ledger service without touching the conversation view ring. Skips when
 * dist/ is not built
 * (`pnpm --filter @deepseek-ai/dsh-client-ui-trajectory bundle`).
 */
...
describe('tsdown client artifact', () => {
  ...
  it.skipIf(code === undefined)('hands off with the manifest id and a DI-require factory', async () => {
    ...
    expect(exports.inject).toEqual([
      'conversationEvents', 'conversationViews', 'sessions', 'locale',
    ])
  })

  it.skipIf(code === undefined)('mounted as an object plugin, apply publishes the ledger and leaves the ring alone', async () => {
    ...
    // ledger, so the binding stays deliberately empty. The locale plugin backs
    // the ledger's bound translate (its settings scope needs a connection
    ...
    expect(ctx.get('trajectorySection')).toBeDefined()
    expect(events.entries().length).toBeGreaterThan(0)
    expect(views.entries()).toHaveLength(1)
    await fiber.dispose()
    expect(ctx.get('trajectorySection')).toBeUndefined()
