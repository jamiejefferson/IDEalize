...
/**
 * Section-service acceptance on the real framework stack: the plugin fiber
 * publishes the ledger as `trajectorySection`, leaves the conversation view
 * ring to chat alone, serves one wired face per chat, renders the event ledger
 * with its timing overview from that face alone, and retires the service plus
 * both Definition registries on disposal. Timeline projection and inclusive
 * focus edge cases ride along.
 */
...
import type {
  ConversationSnapshot, RequestView, SessionId,
} from '@deepseek-ai/dsh-client-runtime/client'
import type { ConvViewOwnerProps, ViewTab } from '@deepseek-ai/dsh-client-ui-conversation/client'
import { apply as localeApply, inject as localeInject } from '@deepseek-ai/dsh-client-locale/client'
import { stubSettingsScope } from '@deepseek-ai/dsh-client-test-runtime'
import type { LocaleKeysOf } from '@deepseek-ai/dsh-client-ui-slots'
import { zh, type TrajectoryKey } from '../src/client/locales.ts'
import {
  apply, inject, type TrajectorySectionHost,
...
function historySnapshot(
  ...
  return {
    sessionId: SID,
    views: {
      // The store's generic parameter cannot narrow from an equality test, so
      // the stub states its own contract: `trajectory` for that target, and
      // nothing for any other registered target.
...
/** Ledger props a consuming surface supplies from the section face. */
...
async function bench(snapshot = historySnapshot(NODES)) {
  ...
  // A bindings map rather than a constant, so a case can take a chat's binding
  // away and see what the section serves without one.
  const bindings = new Map<SessionId, { session: typeof session }>([[SID, { session }]])
  ...
  // The locale plugin backs the ledger's bound translate ('locale' in inject);
  // its settings scope needs a connection handle and the forwarded-event port.
  ...
  await fiber.await()
  return { ctx, slots, fiber, loadOlder, sessionStore, bindings }
}

/** Tab projection twin of the session body's viewTabs (the render-side consumption path). */
...
/** The section service as the consuming surface reads it off the Context. */
function sectionOf(ctx: Context): TrajectorySectionHost {
  return ctx.get('trajectorySection') as TrajectorySectionHost
}

/**
 * Mount the ledger the way a consuming pane does: the wired face straight off
 * the section service, plus the inspect handoff the pane owns. The copy is
 * pinned to zh here so the case assertions read one dictionary; the face's own
 * locale-following `t` has its own case.
 */
function mount(bench: { ctx: Context }, owner: ConvViewOwnerProps = {}) {
  const face = sectionOf(bench.ctx).face(SID)
  if (face === undefined) throw new Error('the section served no face for the benched session')
  return render(
    <TrajectoryView
      {...face}
      {...owner}
...
describe('plugin registration', () => {
  it('publishes the ledger as a service and leaves the view ring to chat alone', async () => {
    const b = await bench()
    expect(tabsOf(b.slots)).toEqual([{ id: 'chat', label: 'Chat' }])
    const section = sectionOf(b.ctx)
    expect(section.Component).toBe(TrajectoryView)
    expect(section.face(SID)).toBeDefined()
  })

  it('serves no face for a chat with no binding', async () => {
    const b = await bench()
    b.bindings.delete(SID)
    expect(sectionOf(b.ctx).face(SID)).toBeUndefined()
  })

  it('fiber disposal retires the service and both registries', async () => {
    const b = await bench()
    ...
    expect(b.ctx.get('trajectorySection')).toBeUndefined()
    expect(tabsOf(b.slots).map(v => v.id)).toEqual(['chat'])
    expect(events.entries()).toEqual([])
    expect(views.entries()).toEqual([])
  })

  it('keeps one selector hook per chat, so a re-read does not remount the ledger', async () => {
    const b = await bench()
    const section = sectionOf(b.ctx)

    expect(section.face(SID)?.useSession).toBe(section.face(SID)?.useSession)
  })

  it('shares one browser-wide duration preference across chats', async () => {
    const b = await bench()
    const section = sectionOf(b.ctx)
    b.bindings.set('s2' as SessionId, b.bindings.get(SID)!)

    section.face(SID)!.setActualDuration(true)

    render(<TrajectoryView {...section.face('s2' as SessionId)!} />)
  ...
  it('reports whether loading older history changed the Trajectory snapshot', async () => {
    const b = await bench()
    const face = sectionOf(b.ctx).face(SID)!

    expect(await face.loadOlder()).toBe(false)

    b.loadOlder.mockImplementationOnce(async () => {
      b.sessionStore.set(historySnapshot([...NODES]))
    })
    expect(await face.loadOlder()).toBe(true)
  })
})

describe('the ledger in a consuming pane', () => {
  it('renders the ledger, its timeline and its folds from the wired face alone', async () => {
    const b = await bench()
    const view = mount(b)
  ...
  it('asks the shell to float a composer over the ledger where one exists', async () => {
    const b = await bench()
    const view = mount(b)
    expect(view.container.querySelector('[data-conversation-composer-overlay]')).toBeTruthy()
  })

  it('reveals the inspect handoff record and acknowledges it once', async () => {
    const b = await bench()
    const onInspectDone = vi.fn()
    mount(b, { inspect: { callId: 'c1' }, onInspectDone })

    const row = screen.getByRole('row', { name: /c1|TOOL/ })
    expect(row.getAttribute('aria-selected')).toBe('true')
    expect(onInspectDone).toHaveBeenCalledTimes(1)
  })

  it('the wired translate follows the active locale', async () => {
    const b = await bench()
    const face = () => sectionOf(b.ctx).face(SID)!
    expect(face().t('view.trajectory')).toBe('Trajectory')
    const locale = b.ctx.get('locale') as { setLocale(id: string): void }
    locale.setLocale('zh')
    expect(face().t('view.trajectory')).toBe('轨迹')
    locale.setLocale('en')
    expect(face().t('view.trajectory')).toBe('Trajectory')
  })
