...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('keeps 2px between rows and rules 12px+12px between workspace groups', () => {
    ...
    // A rule between projects so each group reads as one (JJ, 2026-08-28). It
    // takes l3 rather than the l1 hairline, which vanished against the
    // sidebar's ground (JJ, 11 Sep 2026: "need an HR between projects").
    const between = declarations('.groupSection + .groupSection')
    expect(between?.get('margin-top')).toBe('12px')
    expect(between?.get('padding-top')).toBe('12px')
    expect(between?.get('border-top')).toBe('1px solid var(--dsw-alias-border-l3)')
  })
  ...
  it('keeps the compact fade, overflow control, search field, and row heights', () => {
    ...
    // The space lane is the row's fixed leading cell, as wide as the 18px
    // glyph it holds and never flexing, so every row's glyph sits on one
    // vertical line and no row jitters.
    expect(rowDeclarations('.spaceLane')?.get('width')).toBe('18px')
    expect(rowDeclarations('.spaceLane')?.get('flex')).toBe('none')
