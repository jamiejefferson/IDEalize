...
describe('WorkspaceBrowser', () => {
  ...
  it('renders the grouped tree by default and switches to the flat list via Group by', () => {
    ...
    // Groups open by default: the sessions show without a click.
  ...
  it('opens a session row from the open-by-default group; a header click folds it', () => {
    const open = vi.fn()
    mount({
  ...
  it('shows a failed fork\'s notice as a transient alert and keeps the selection', async () => {
    const forkSession = vi.fn(async () => { throw new Error('This chat has nothing to fork yet.') })
    const b = mount({
      useSessions: hook(sessionState([summary('s1', 1, { title: 'One', displayTitle: 'One' })], { current: sid('s1') })),
      useWorkspaces: hook(workspaceState([workspace('ws', ['s1'])])),
      forkSession,
    })
  ...
  it('reopens a folded group before starting a session from its ＋', () => {
  ...
  it('shows the Ungrouped bucket open for a loose current session; its menu archives every started loose chat and its ＋ is inert', () => {
    const startSession = vi.fn()
    const archiveSession = vi.fn(async (_sessionId: SessionId) => {})
    mount({
      useSessions: hook(sessionState(
        [summary('loose', 3), summary('loose-2', 2), summary('draft', 1, { blank: true })],
        { current: sid('draft') },
      )),
    ...
    // The loose session's group is UNGROUPED_KEY, open like every other group.
    expect(screen.getByText('loose')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '在“未分组”中新建会话' }))
    expect(startSession).not.toHaveBeenCalled()
    // The header menu offers only Archive all; the blank placeholder is neither counted nor archived.
    fireEvent.click(screen.getByRole('button', { name: '工作区“未分组”的操作' }))
    expect(screen.getAllByRole('menuitem').map(m => m.textContent)).toEqual(['归档全部会话（2）'])
    fireEvent.click(screen.getByRole('menuitem', { name: '归档全部会话（2）' }))
    expect(archiveSession.mock.calls.map(c => c[0]).sort()).toEqual([sid('loose'), sid('loose-2')].sort())
  })

  it('keeps an open group open when the selection moves within it', () => {
    ...
    // Selection hop inside the same group: no fold is recorded, so the
    // group stays open until the header is clicked.
  ...
  it('confirms Workspace deletion, explains retention, and blocks duplicate submission', async () => {
    ...
    const confirm = screen.getByRole<HTMLButtonElement>('button', { name: '移除工作区' })
    ...
    expect(screen.getByRole('status').textContent).toBe('正在移除工作区…')
    fireEvent.keyDown(document, { key: 'Escape' })
    ...
    expect(screen.queryByRole('dialog', { name: '移除工作区' })).toBeNull()
  })
