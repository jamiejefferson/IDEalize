...
describe('ui-workspace apply', () => {
  ...
  it('rejects a failed fork with the notice the person reads, in the active locale', async () => {
    const b = await bench()
    declare(b.slots, 'sidebar.workspaces')
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const browser = (b.slots.entries('sidebar.workspaces')[0]!.inject as () => WorkspaceBrowserInjected)()
    const forkError = (code: string, message: string) => new SessionForkError(
      { code, message, details: {} } as never,
      'session' as never,
    )

    // No completed turn yet: the Host's fork-unavailable code becomes the worded notice.
    b.fork.mockRejectedValueOnce(forkError('fork-unavailable', 'session "session" has no completed turn to fork from'))
    await expect(browser.forkSession('session' as never)).rejects.toThrow('这个会话还没有可分叉的内容。等它第一次回复后再分叉。')
    // Any other Host refusal passes its own message through.
    b.fork.mockRejectedValueOnce(forkError('internal', 'store unavailable'))
    await expect(browser.forkSession('session' as never)).rejects.toThrow('store unavailable')
    // A plain Error is the runtime's post-creation rename failure: the child exists, its title did not change.
    b.fork.mockRejectedValueOnce(new Error('fork child rename failed: title-invalid: too long'))
    await expect(browser.forkSession('session' as never)).rejects.toThrow('已分叉，但副本沿用了原来的名称。')
    expect(b.open).not.toHaveBeenCalled()

    b.locale.setLocale('en')
    b.fork.mockRejectedValueOnce(forkError('fork-unavailable', 'no turn'))
    await expect(browser.forkSession('session' as never)).rejects.toThrow('This chat has nothing to fork yet. Fork it after its first reply.')
  })
