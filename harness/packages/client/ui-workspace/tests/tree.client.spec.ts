...
const view = (collapsedGroups: readonly string[] = [], ungroupedOrder?: readonly string[]) => ({
  collapsedGroups,
...
describe('deriveGroups', () => {
  it('keeps Host Workspace and sessionIds order without Client recency sorting', () => {
    ...
    const groups = deriveGroups(sessions, workspaces, noArchive, view())
  ...
  it('shows every group open unless the user folded it', () => {
    const sessions = list(summary('kept', 2), summary('hidden', 1))
    const workspaces = [workspace('open', ['kept']), workspace('folded', ['hidden'])]
    const groups = deriveGroups(sessions, workspaces, noArchive, view(['folded']))
    expect(groups.map(group => [group.key, group.expanded, group.sessions.length])).toEqual([
      ['open', true, 1],
      ['folded', false, 0],
    ])
  ...
  it('carries each chat\'s space from the projection into grouped and flat rows', () => {
    const gallery = { ...summary('gallery-chat', 10), projectionValues: { space: { space: 'gallery' as const } } }
    const motion = { ...summary('motion-chat', 9), projectionValues: { space: { space: 'motion' as const } } }
    const sessions = list(gallery, motion)
    const grouped = deriveGroups(sessions, [workspace('project', ['gallery-chat', 'motion-chat'])], noArchive, view())
    expect(grouped[0]!.sessions.map(session => session.space)).toEqual(['gallery', 'motion'])
    expect(deriveFlat(sessions, noArchive).map(node => node.space)).toEqual(['gallery', 'motion'])
  })

  it('gives a chat with no space projection the Chat glyph rather than an empty lane', () => {
    // A chat made before spaces existed: the host's ladder answers Chat, and a
    // row whose projection has not yet arrived must still carry a glyph or the
    // lane would jitter as the list settles.
    const sessions = list(summary('legacy', 10))
    expect(deriveGroups(sessions, [workspace('project', ['legacy'])], noArchive, view())[0]!.sessions[0]!.space)
      .toBe('chat')
    expect(deriveFlat(sessions, noArchive)[0]!.space).toBe('chat')
  })
  ...
  it('projects pending-interaction state into grouped and flat rows', () => {
    ...
    const grouped = deriveGroups(sessions, [workspace('project', ['awaiting'])], noArchive, view())
  ...
  it('shows only the current blank session in its Workspace count and tree', () => {
    ...
    const groups = deriveGroups(
      sessions, [workspace('first', ['shown', 'current-blank', 'stale-blank'])], noArchive, view(),
    )
  ...
  it('keeps a launched Terminal chat listed and started, prompt or no prompt', () => {
    // A Terminal chat never sends a prompt (keystrokes go to the shell), so it
    // stays blank on the host; its recorded space makes it a started chat.
    const terminal = { ...summary('term', 6), blank: true, projectionValues: { space: { space: 'terminal' as const } } }
    const staleBlank = { ...summary('stale-blank', 4), blank: true }
    const real = summary('shown', 3)
    const sessions = { ...list(real, terminal, staleBlank), current: real.id }
    const groups = deriveGroups(
      sessions, [workspace('first', ['shown', 'term', 'stale-blank'])], noArchive, view(),
    )
    expect(groups[0]!.sessions.map(session => session.id).sort()).toEqual([real.id, terminal.id].sort())
    const node = groups[0]!.sessions.find(session => session.id === terminal.id)!
    expect(node.blank).toBe(false)
    expect(node.shellOnly).toBe(true)
  ...
  it('never lists a project\'s Studio chat, current or not: the pinned card is how it opens', () => {
    ...
    const real = summary('shown', 3)
    const asCurrent = deriveGroups(
      { ...list(real, studio), current: studio.id }, [workspace('first', ['shown', 'studio'])], noArchive, view(),
    )
    expect(asCurrent[0]!.sessions.map(session => session.id)).toEqual([real.id])
    const flat = deriveFlat({ ...list(real, studio), current: studio.id }, noArchive)
    expect(flat.map(session => session.id)).toEqual([real.id])
  })
  ...
  it('projects the completion reminder into session and search rows (absent = false)', () => {
    ...
    const groups = deriveGroups(
      sessions, [workspace('first', ['done', 'plain'])], noArchive, view(),
    )
  ...
  it('ignores fork lineage and sorts every ungrouped session as a top-level row', () => {
    ...
    expect(deriveGroups(list(summary('tie-a', 1), summary('tie-b', 1)), [], noArchive, view())[0]!
  ...
  it('tolerates Workspace membership arriving before its Session summary', () => {
    ...
    const groups = deriveGroups(partial, [workspace('project', ['missing', 'present'])], noArchive, view())
  ...
  it('hides archived sessions from workspace groups and Ungrouped', () => {
    ...
    const groups = deriveGroups(
      sessions, [workspace('first', ['kept', 'gone'])], archived('gone', 'loose-gone'), view(),
    )
