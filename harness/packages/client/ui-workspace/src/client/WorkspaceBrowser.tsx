...
function SessionTree({
  ...
  // Groups open by default; only a fold the user made (and has not undone)
  // hides a group's sessions.
  ...
  const groups = useMemo(
    ...
    [list, orderedWorkspaces, archivedSessionIds, collapsedGroups, sessionOrderByAccount],
  )
  const now = Date.now()
...
export function WorkspaceBrowser({
  ...
  // Fork is dialog-free too; its failures reach the person through the shared
  // transient Toast (the composer's mechanism), keyed by a per-show sequence so
  // a repeated identical notice restarts the fade.
  const [forkNotice, setForkNotice] = useState<{ seq: number; text: string } | null>(null)
  const forkNoticeSeq = useRef(0)
  const onSessionFork = (sessionId: SessionNode['id']) => forkSession(sessionId).catch((reason: unknown) => {
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      ...
      <div className={css.listArea}>
        {/* The pinned strip above every project (IDEalize's Studio card); an
            empty seat costs no height. */}
        {wide && normalizedQuery === '' && (
          <div className={css.pinnedArea} data-sidebar-pinned="">
            {renderSlot('sidebar.workspaces.pinned', {})}
          </div>
        )}
        {wide && (normalizedQuery !== ''
          ? (
      ...
      {forkNotice !== null && (
        <Toast
          key={forkNotice.seq}
          text={forkNotice.text}
          icon={<IconWarningOutline16 />}
          onDone={() => { setForkNotice(null) }}
        />
      )}
    </div>
  )
