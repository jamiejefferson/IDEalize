...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * The pinned strip at the top of the sidebar's list, above every project
     * (declared by the WorkspaceBrowser entry): one card per entry, rendered
     * while the column is wide and no search is typed, in `order`. IDEalize's
     * Studio card lives here (FORK.md); V0 pinned its lead-agent card in the
     * same place.
     */
...
export type WorkspaceBrowserInjected = DirectoryPickingInjected & {
  ...
  /**
   * Fork a Session at its last completed turn and open the child. Rejects with
   * an Error whose message is the localised notice for the person: the source
   * has no completed turn yet, the child kept the original title, or the
   * Host's own message.
   */
...
export type WorkspaceBrowserProps =
  ...
  & PropsRenderSlots<'sidebar.workspaces.directoryFlow' | 'sidebar.workspaces.pinned'>
