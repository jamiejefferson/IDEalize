...
import { CustomProviderCard, routeFromUrl } from '../src/client/CustomProviderCard.tsx'
...
describe('hand-declared providers', () => {
  ...
  it('derives the route id from the endpoint host until the id is edited', () => {
    expect(routeFromUrl('https://api.fal.ai/v1')).toBe('fal')
    expect(routeFromUrl('https://openrouter.ai/api/v1')).toBe('openrouter')
    expect(routeFromUrl('https://gateway.acme.example/v1')).toBe('acme')
    expect(routeFromUrl('https://www.example.co.uk/v1')).toBe('example')
    expect(routeFromUrl('https://127.0.0.1:8080/v1')).toBe('')
    expect(routeFromUrl('not a url')).toBe('')

    mountCard()
    fireEvent.change(screen.getByLabelText(en.baseUrl), { target: { value: 'https://api.fal.ai/v1' } })
    expect(screen.getByLabelText<HTMLInputElement>(en.customRoute).value).toBe('fal')
    fireEvent.change(screen.getByLabelText(en.customRoute), { target: { value: 'fal-gateway' } })
    fireEvent.change(screen.getByLabelText(en.baseUrl), { target: { value: 'https://api.other.ai/v1' } })
    expect(screen.getByLabelText<HTMLInputElement>(en.customRoute).value).toBe('fal-gateway')
  })

  it('asks the endpoint for its models itself and is ready to create with URL and key alone', async () => {
    vi.useFakeTimers()
    try {
      const { discover, mutate, onClose } = mountCard({}, {
        discover: vi.fn(() => Promise.resolve(ok({ models: [{ id: 'fal-fast' }, { id: 'fal-pro', name: 'Fal Pro', contextWindow: 32_000 }] }))),
      })
      fireEvent.change(screen.getByLabelText(en.baseUrl), { target: { value: 'https://api.fal.ai/v1' } })
      fireEvent.change(screen.getByLabelText(en.keyInput), { target: { value: 'fal-key' } })
      expect(discover).not.toHaveBeenCalled()
      await act(async () => { await vi.advanceTimersByTimeAsync(700) })
      expect(discover).toHaveBeenCalledTimes(1)
      expect(discover.mock.calls[0]?.[0]).toMatchObject({ settingsNs: 'llm-pi-ai', baseURL: 'https://api.fal.ai/v1', api: 'openai-completions', apiKey: 'fal-key' })
      expect(document.querySelector('[data-custom-discovery]')?.getAttribute('data-custom-discovery')).toBe('found')
      expect(screen.getByText('2 models found. Ready to create.')).toBeTruthy()
      expect(screen.getByText<HTMLButtonElement>(en.create).disabled).toBe(false)
      ...
      expect(firstMutate(mutate).ops[0]).toMatchObject({
        ...
        value: {
          ...
          models: [{ id: 'fal-fast' }, { id: 'fal-pro', name: 'Fal Pro', contextWindow: 32_000 }],
        },
      })
    } finally {
      vi.useRealTimers()
  ...
  it('states a failed or empty discovery and leaves the manual editor available', async () => {
    vi.useFakeTimers()
    try {
      mountCard({}, { discover: vi.fn(() => Promise.resolve(fail('connection refused', 'unreachable'))) })
      fireEvent.change(screen.getByLabelText(en.baseUrl), { target: { value: 'https://api.fal.ai/v1' } })
      fireEvent.change(screen.getByLabelText(en.keyInput), { target: { value: 'fal-key' } })
      await act(async () => { await vi.advanceTimersByTimeAsync(700) })
      expect(screen.getByText('Could not read models from api.fal.ai: connection refused')).toBeTruthy()
      expect(screen.getByText<HTMLButtonElement>(en.create).disabled).toBe(true)
      expect(screen.getByRole('button', { name: en.addModel })).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  ...
  it('scopes each card to fields a provider can actually own', async () => {
    ...
    // URL and key first; the derived id, name and protocol sit under More options.
    expect(fields()).toEqual([en.baseUrl, en.keyInput, en.customRoute, en.customDisplayName, en.customApi])
    cleanup()
