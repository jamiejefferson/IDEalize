...
describe('ModelsSection', () => {
  ...
  it('escapes from the catalog select to the custom-provider card', async () => {
    // The shared fixture's pi-ai schema names no protocols; the escape needs
    // them, exactly as the standalone declare button does.
    const declaredConfig = Schema.object({
      providers: Schema.dict(Schema.object({
        apiKeyEnv: Schema.string().role('credential-ref'),
        baseURL: Schema.string(),
    ...
    scripted.face.settings.describe.mockResolvedValue(ok({
      ...
      namespaces: wireNamespaces().map(namespace => namespace.ns === 'llm-pi-ai'
        ? { ...namespace, schema: JSON.parse(JSON.stringify(declaredConfig.toJSON())) as unknown }
        : namespace),
    }))
    await mountFace(scripted)
    fireEvent.click(screen.getByText(en.add))
    await screen.findByLabelText(en.provider)
    const escape = screen.getByRole<HTMLButtonElement>('button', { name: en.customAdd })
    expect(escape.disabled).toBe(false)
    fireEvent.click(escape)
    // The declare card replaces the add card rather than opening beside it.
    expect(screen.getByText(en.customTitle)).toBeTruthy()
    expect(screen.queryByLabelText(en.provider)).toBeNull()
  })

  it('disables the escape while the namespace declares no protocols', async () => {
    await mountSection()
    fireEvent.click(screen.getByText(en.add))
    await screen.findByLabelText(en.provider)
