...
import { adopt, ModelListEditor } from './ModelListEditor.tsx'
...
export function routeFromUrl(url: string): string {
  ...
  // Drop the public suffix (one label, or two for `co.uk`-shaped hosts) when
  // something is left to name the route.
  if (labels.length >= 3 && (labels[labels.length - 2] ?? '').length <= 3) labels.splice(-2, 2)
  else if (labels.length >= 2) labels.pop()
  const stem = (labels[labels.length - 1] ?? '').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').replace(/^[^a-z]+/, '')
  return ROUTE_PATTERN.test(stem) ? stem : ''
}

/** The host of a URL for the status line; the raw text when it is not a URL. */
...
/** What the automatic model discovery is doing, for the status line under the key. */
type Discovery =
  | { state: 'idle' }
  | { state: 'checking'; host: string }
  | { state: 'found'; count: number }
  | { state: 'none'; host: string }
  | { state: 'failed'; host: string; message: string }
...
export function CustomProviderCard(props: CustomProviderCardProps): ReactNode {
  ...
  /** The id as typed under More options; undefined while it follows the URL. */
  const [routeEdit, setRouteEdit] = useState<string | undefined>(undefined)
  const route = routeEdit ?? routeFromUrl(baseURL)
  ...
  // Ask the endpoint for its models once the URL parses and the key field is
  // settled (typed and accepted, or deliberately blank): the whole discovery
  // is adopted, so a provider that answers is ready to create with nothing
  // else typed. Debounced so a key being pasted asks once, not per keystroke.
  useEffect(() => {
    if (committed || !urlValid || keyFailure !== undefined) return
    const url = baseURL.trim()
    const host = hostOf(url)
    let cancelled = false
    const timer = setTimeout(() => {
      setDiscovery({ state: 'checking', host })
      void api.llm.discoverModels({
        ...
        if (!response.result.ok) {
          setDiscovery({ state: 'failed', host, message: response.result.error.message })
          return
        }
        const found = response.result.value.models
        if (found.length === 0) {
          setDiscovery({ state: 'none', host })
          return
        }
        setModels(found.map(adopt))
        setDiscovery({ state: 'found', count: found.length })
      }).catch((error: unknown) => {
        if (!cancelled) setDiscovery({ state: 'failed', host, message: messageOf(error) })
      })
    }, 600)
    return () => {
      cancelled = true
  ...
  }, [api, baseURL, committed, keyFailure, keyValue, protocol, urlValid])

  const discoveryLine = discovery.state === 'idle'
    ? undefined
    : discovery.state === 'checking'
      ? fill(t('customChecking'), { host: discovery.host })
      : discovery.state === 'found'
        ? fill(t('customFound'), { count: discovery.count })
        : discovery.state === 'none'
          ? fill(t('customFoundNone'), { host: discovery.host })
          : fill(t('customCheckFailed'), { host: discovery.host, message: discovery.message })
    ...
    : baseURL.length === 0
      ...
      : !urlValid
        ...
        : modelFailure !== undefined
          ...
          : discovery.state === 'checking' ? undefined : t('customNeedsModels')
  ...
  return (
    <div className={styles['editor']}>
      ...
      <p className={styles['advancedHint']}>{t('customLead')}</p>
      ...
      {discoveryLine === undefined
        ? null
        : (
          <p
            className={discovery.state === 'failed' ? styles['error'] : styles['advancedHint']}
            data-custom-discovery={discovery.state}
          >
            {discoveryLine}
          </p>
        )}
      <details className={styles['customized']} open={moreOpen} onToggle={(event) => { setMoreOpen((event.target as HTMLDetailsElement).open) }}>
        <summary className={styles['customizedSummary']}>{t('moreOptions')}</summary>
        <div className={styles['field']}>
          ...
          <input
            ...
            disabled={profileDisabled}
            onChange={(event) => { setRouteEdit(event.target.value) }}
          />
        </div>
        {/* A rejected id reads as a fault, not as guidance — the same split the
            key field above already makes between its failure and its hint. */}
