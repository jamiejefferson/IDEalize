 ...
 * `composes` references (same-sheet and `from './other.module.css'`) join the
 * exported class list, and composed-from sheets are imported so their style
 * tags inject before the composing sheet's.
...
import type { CSSModuleExport, CSSModuleExports } from 'lightningcss'
  ...
  return {
    ...
    plugins: [{
      ...
      async load(virtualId: string) {
        ...
        const sheets = new Map<string, CSSModuleExports>([[fileId, cssExports ?? {}]])
        const dependencies = new Set<string>()
        const classMap: Record<string, string> = {}
        for (const [local, exp] of Object.entries(cssExports ?? {})) {
          classMap[local] = [...new Set(composedClassList(fileId, exp, sheets, dependencies, new Set()))].join(' ')
        }
        // Composed-from sheets import first: importing injects their style
        // tags, and tag order keeps the composing sheet's declarations winning.
        // One <style data-plugin> per module file; idempotent under re-evaluation.
        return [
          ...[...dependencies].map((file) => `import ${JSON.stringify(file)};`),
...
/**
 * Resolve one CSS Module export into its full class list: the export's own
 * compiled name plus every `composes` reference, transitively. A same-sheet
 * reference recurses through the sheet's own exports so composition chains
 * flatten; a `composes: x from './other'` reference is looked up in the
 * referenced sheet's compiled exports and the sheet path is recorded in
 * `dependencies` so the emitted module imports it — the import is what
 * injects the referenced style tag at runtime. A missing target or a
 * circular reference fails the build.
 * @param fileId - absolute path of the sheet owning the export.
 * @param exp - the lightningcss export entry being resolved.
 * @param sheets - per-load cache of compiled exports, seeded with `fileId`'s.
 * @param dependencies - collects referenced sheet paths for module imports.
 * @param path - exports on the current resolution path; a revisit is a cycle.
 * @returns class names for the export's class map entry (may repeat on
 * diamond references; the caller dedupes).
 */
function composedClassList(
  fileId: string,
  exp: CSSModuleExport,
  sheets: Map<string, CSSModuleExports>,
  dependencies: Set<string>,
  path: Set<string>,
): string[] {
  const key = `${fileId}#${exp.name}`
  if (path.has(key)) throw new Error(`css modules: circular composes through ${key}`)
  path.add(key)
  ...
  for (const ref of exp.composes) {
    ...
    if (ref.type === 'local') {
      // ref.name is the compiled name; find its export to flatten transitive composes
      const target = Object.values(sheets.get(fileId) ?? {}).find((entry) => entry.name === ref.name)
      if (target === undefined || target === exp) names.push(ref.name)
      else names.push(...composedClassList(fileId, target, sheets, dependencies, path))
      continue
    }
    const sheetPath = resolvePath(dirname(fileId), ref.specifier)
    dependencies.add(sheetPath)
    const target = compiledSheetExports(sheetPath, sheets)[ref.name]
    if (target === undefined) {
      throw new Error(`css modules: ${fileId} composes "${ref.name}" from ${ref.specifier}, which does not export it`)
    }
    names.push(...composedClassList(sheetPath, target, sheets, dependencies, path))
  }
  path.delete(key)
  return names
}
...
/**
 * Compile a composed-from sheet just far enough to read its class exports.
 * @param sheetPath - absolute path of the referenced `.module.css` file.
 * @param sheets - per-load cache keyed by sheet path.
 * @returns the sheet's lightningcss class exports.
 */
function compiledSheetExports(sheetPath: string, sheets: Map<string, CSSModuleExports>): CSSModuleExports {
  let sheetExports = sheets.get(sheetPath)
  if (sheetExports === undefined) {
    const { exports: compiled } = transform({
      filename: sheetPath,
      code: readFileSync(sheetPath),
      cssModules: { pattern: '[hash]_[local]' },
      minify: true,
    })
    sheetExports = compiled ?? {}
    sheets.set(sheetPath, sheetExports)
  }
  return sheetExports
}
