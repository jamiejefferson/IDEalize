/**
 * Registration: the General row, the new-session chip, and the header label
 * all come from one apply, and each defers until the slot it fills has been
 * declared. (IDEalize fork: the management section registers no settings
 * page — it is provided as the `agentPresetSection` service for the bar's
 * Models & usage drawer; the same wired face, so a pushed settings change
 * still converges every surface.)
 */
...
describe('ui-agent-preset apply', () => {
  ...
  it('registers the General row and provides the section service', async () => {
    ...
    // Fork shape: the section is a service for the bar's drawer, not a page.
    expect(slots.entries('settings.section')).toHaveLength(0)
    expect(ctx.agentPresetSection.Component).toBe(AgentPresetSection)
  })
  ...
  it('hands each surface its own store and actions', async () => {
    ...
    const section = ctx.agentPresetSection.face()
