...
/**
 * The management section as a cordis service (IDEalize fork seam): the
 * settings modal no longer hosts the page, and the client bundle purity gate
 * forbids cross-plugin value imports, so the bar's Models & usage drawer
 * consumes the component + wired face through the Context instead.
 */
export interface AgentPresetSectionHost {
  /** The section component, rendered by the consuming surface. */
  Component: typeof AgentPresetSection
  /** The wired business face (same controller as every other surface). */
  face: () => AgentPresetSectionInjected
}

declare module '@deepseek-ai/cordis' {
  interface Context {
    agentPresetSection: AgentPresetSectionHost
  }
}
...
export function apply(ctx: ClientContext): void {
  ...
  // usage drawer renders the section through this service instead.
  ctx.provide('agentPresetSection', {
    Component: AgentPresetSection,
    face: sectionInjected,
  } satisfies AgentPresetSectionHost)
}
