...
/**
 * Open-the-settings-panel service (IDEalize fork seam): the shell owns modal
 * open state as component-local viewing state, so surfaces that need to open
 * it (the bar's wrench button) go through this service. Provided by the
 * shell package (ui-settings-general); typed here so consumers depend only
 * on the settings domain base.
 */
export interface SettingsOpenService {
  /** Open the panel, optionally landing on one registered section id. */
  open: (section?: string) => void
}

declare module '@deepseek-ai/cordis' {
  interface Context {
    settingsOpen: SettingsOpenService
  }
}
