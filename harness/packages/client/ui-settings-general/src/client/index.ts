...
import type {
  SettingsOnboardingStep, SettingsOpenRequest, SettingsRootInjected, SettingsSectionRow,
...
export function apply(ctx: ClientContext): void {
  ...
  // Programmatic opens (ctx.settingsOpen): a tick counter the shell follows.
  let openRequest: SettingsOpenRequest = { tick: 0 }
  const openListeners = new Set<() => void>()
  ctx.provide('settingsOpen', {
    open: (section?: string) => {
      openRequest = { tick: openRequest.tick + 1, ...section === undefined ? {} : { section } }
