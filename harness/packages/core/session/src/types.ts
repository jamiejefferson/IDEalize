...
/**
 * Append options for non-surface events. `ignorable: true` stamps the
 * envelope's skip marker (see {@link SessionEvent.ignorable}): set it only on
 * purely informational records whose loss cannot affect reconstruction, so a
 * reader without the event's vocabulary may still interpret the log. Surface
 * events never take it — model-visible content must never be skipped.
 */
export interface NonSurfaceIntent {
  ignorable?: true
}
