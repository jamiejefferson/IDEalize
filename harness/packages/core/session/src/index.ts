...
import type { CreateSessionOptions, EpochHeader, NonSurfaceIntent, PrepareSessionOptions, RequestContext, SessionEvent, SessionEventMap, SessionEventType, SessionHeader, SurfaceIntent, SurfaceEventType } from './types.ts'
...
export class Session {
  ...
  append<T extends SessionEventType>(
    ...
    ...opts: T extends SurfaceEventType ? [opts: SurfaceIntent] : [opts?: NonSurfaceIntent]
  ): SessionEvent<T> {
    const intent = opts[0] as (SurfaceIntent & NonSurfaceIntent) | undefined
    const surfaceOpts: SurfaceIntent | undefined = intent
    ...
    const event = deepFreeze({
      ...
      time: Date.now(),
      data: dataSnapshot,
      ...(intent?.ignorable === true ? { ignorable: true as const } : {}),
