...
export class TestWorkspaces implements IWorkspaces {
  ...
  /**
   * Mark a blank session as put to work (recorded). The default lifts the
   * marker on disposal and records that too, so a test can assert both halves.
   * @param sessionId - the blank session the caller is configuring.
   * @returns the disposer.
   */
  noteSessionConfigured(sessionId: SessionId): () => void {
    this.calls.push({ method: 'noteSessionConfigured', args: [sessionId] })
    const stub = this.stubs.get('noteSessionConfigured')
    if (stub !== undefined) return stub(sessionId) as () => void
    return () => { this.calls.push({ method: 'noteSessionConfigured.dispose', args: [sessionId] }) }
  }
