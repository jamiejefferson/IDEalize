...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      // Node prints `ExperimentalWarning: SQLite` to stderr the moment the
      // session store opens `node:sqlite`. Scenarios that assert the agent
      // wrote nothing to stderr would fail on the runtime's own notice about
      // a feature the harness uses on purpose, so it is disabled by name
      // rather than with a blanket `NODE_NO_WARNINGS`.
