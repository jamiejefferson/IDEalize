...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'artefacts',
    summary: 'The artefact engine service (`ctx.artefacts`): durable records over the storage domain, per-project byte storage, session-event publication, the raw route, and the `artefacts_get` tool.',
    description: 'The artefact engine service (`ctx.artefacts`): durable records over the storage domain, per-project byte storage, session-event publication, the raw route, and the `artefacts_get` tool.',
    methods: [
      {
        signature: 'folders(): ArtefactFolderSettings',
        description: 'The folder settings in force: the registered section, or the schema defaults without a settings provider.',
        parameters: [],
        returns: 'the resolved folders.',
      },
      {
        signature: 'async create(request: ArtefactCreateRequest): Promise<ArtefactRecord>',
        description: 'Create one artefact: stream the bytes into the owning project, hash them, commit the durable record, then append `artefact/created` to the producing session. A failure after acceptance appends `artefact/failed` with the cause, removes any partial file, and rethrows.',
        parameters: [{ name: 'request', description: 'the artefact content, source task, and provenance.' }],
        returns: 'the committed record.',
      },
      {
        signature: 'get(id: ArtefactId): ArtefactRecord | undefined',
        description: 'Look up one artefact record.',
        parameters: [{ name: 'id', description: 'Artefact id.' }],
        returns: 'the record, or `undefined` when unknown.',
      },
      {
        signature: 'list(filter: ArtefactListFilter = {}): ArtefactRecord[]',
        description: 'List records, newest creation first.',
        parameters: [{ name: 'filter', description: 'optional workspace and media-type constraints; a `mediaType` of `image` matches every `image/*` subtype, a full `type/subtype` matches exactly.' }],
        returns: 'the matching records.',
      },
      {
        signature: 'async setDisposition(id: ArtefactId, disposition: ArtefactDisposition): Promise<ArtefactRecord>',
        description: 'Keep or archive one artefact. Archiving moves the file into the `archive` subfolder beside it; keeping moves it back up one folder. The record\'s path follows the file, then `artefact/disposition` is appended to the producing session when it is live (an archived chat\'s record still updates; its log catches up with nothing, because the store is the source of truth). Asking for the disposition a record already has moves nothing and appends nothing.',
        parameters: [{ name: 'id', description: 'the artefact.' }, { name: 'disposition', description: '`kept` or `archived`.' }],
        returns: 'the updated record.',
        throws: ['when the id is unknown, its workspace is gone, or the move fails.'],
      },
      {
        signature: 'resolve(id: ArtefactId): string',
        description: 'Resolve one artefact to its servable absolute path.',
        parameters: [{ name: 'id', description: 'Artefact id.' }],
  ...
  {
    key: 'docPolicy',
    summary: 'The canonical documentation service.',
    description: 'The canonical documentation service. Scans are serialized on one chain; reads (`state`, `search`) are safe at any time.',
    methods: [
      {
        signature: 'folder(): string | undefined',
        description: 'The configured documentation folder, or `undefined` before setup.',
        parameters: [],
        returns: 'the absolute folder path, or `undefined` when unconfigured.',
      },
      {
        signature: 'rulesetVersion(): RulesetVersion',
        description: 'The packaged ruleset version this build applies.',
        parameters: [],
        returns: 'the pinned ruleset version.',
      },
      {
        signature: 'conventions(): string',
        description: 'The canonical conventions text (the DOC-05 authority).',
        parameters: [],
      ...
      {
        signature: 'async search(query: string, limit: number): Promise<DocsSearchHit[]>',
        description: 'Phrase-search the indexed documentation.',
        parameters: [{ name: 'query', description: 'caller text, matched as one literal phrase.' }, { name: 'limit', description: 'maximum hits.' }],
        returns: 'best-first hits (empty when no folder is configured).',
      },
      {
        signature: 'state(): DocsState',
        description: 'Folder + scan state for the settings surface and the state route.',
        parameters: [],
        returns: 'the current configuration and last scan snapshot.',
      },
      {
        signature: 'attachScans(table: KvTable<string, DocScanRecord>): void',
        description: 'Attach the durable scan-history table; a scan that ran before attachment is recorded now so a late-mounting storage form loses nothing.',
        parameters: [{ name: 'table', description: 'the opened `scans` table.' }],
      },
      {
        signature: 'detachScans(): void',
        description: 'Detach the scan-history table ahead of its domain closing.',
        parameters: [],
      },
    ],
  },
  ...
  {
    key: 'generation',
    summary: 'The generation service.',
    description: 'The generation service. Registered as `ctx.generation` (one instance per context). Also mounts the `/idealize/brains/media` preset routes and the `/idealize/generate/inputs` route while a web server and settings service are composed beside it.',
    methods: [
      {
        signature: 'register(backend: GenerationBackend): () => void',
        description: 'Register a generation backend. Throws if its id is already registered — a duplicate is a composition bug, not a task failure. Returns a disposer; disposed with the calling fiber.',
        parameters: [{ name: 'backend', description: 'the backend; its `id` is the registry key.' }],
        returns: 'the disposer that unregisters the backend.',
      },
      {
        signature: 'backends(): readonly string[]',
        description: 'The registered backend ids.',
        parameters: [],
        returns: 'the ids in registration order.',
      },
      {
        signature: 'credentials(): BackendCredentialState[]',
        description: 'The backends that take a key from the person, with whether one is stored and what each serves: a backend is connected while none of its descriptors reads `no-credential`, and its artefacts are the ones its descriptors name, which a backend states before it has fetched a single model.',
        parameters: [],
        returns: 'one row per backend declaring a {@link BackendCredential}, in registration order.',
      },
      {
        signature: 'async refreshBackend(id: string): Promise<boolean>',
        description: 'Re-fetch one backend\'s catalogue after its key changed.',
        parameters: [{ name: 'id', description: 'the backend id.' }],
        returns: 'false when no backend has that id; true once its refresh (if any) settled.',
      },
      {
        signature: 'async inputs(model: GenModelRef): Promise<readonly GenInputField[]>',
        description: 'The inputs a person may choose for one model before generating, from its backend\'s GenerationBackend.inputs.',
        parameters: [{ name: 'model', description: 'the backend and model id.' }],
      ...
      {
        signature: 'catalog(): GenCatalogEntry[]',
        description: 'The merged catalogue: every model each registered backend publishes.',
        parameters: [],
        returns: 'one entry per (backend, model), in registration order.',
      },
      {
        signature: 'compatible(requirements: readonly CapabilityRequirement[]): GenCatalogEntry[]',
        description: 'Catalogue rows whose model satisfies every requirement in the set (artefact matches and required input modalities are a subset).',
        parameters: [{ name: 'requirements', description: 'a preset\'s or task\'s `requiredCapabilities`.' }],
        returns: 'the compatible entries, in catalogue order.',
      },
      {
        signature: 'availability(requirements: readonly CapabilityRequirement[]): GenerationAvailability',
        description: 'Queryable availability for a requirement set: available with the match counts, or unavailable with the reason and a recovery action.',
        parameters: [{ name: 'requirements', description: 'a preset\'s or task\'s `requiredCapabilities`.' }],
        returns: 'the availability verdict.',
      },
      {
        signature: 'async generate(request: GenerationRequest, onProgress?: GenerationProgressSink, signal?: AbortSignal): Promise<GenerationResult>',
        description: 'Run one generation on the request\'s chosen backend and model. The model must exist in that backend\'s catalogue and satisfy the request\'s required capabilities. Backend failures that are not already GenerationErrors are wrapped as `PROVIDER_ERROR` with the cause chained.',
        parameters: [{ name: 'request', description: 'the task request; `model` names the backend and model id.' }, { name: 'onProgress', description: 'optional progress sink; omitted reports are dropped.' }, { name: 'signal', description: 'optional cancellation signal forwarded to the backend.' }],
  ...
  },
  {
    ...
    summary: 'Maps a requesting session to its project attribution header.',
  ...
  {
    key: 'idealizeBridge',
    summary: 'The notification feed behind `ctx.idealizeBridge`: a capped buffer plus its HTTP surface.',
    description: 'The notification feed behind `ctx.idealizeBridge`: a capped buffer plus its HTTP surface.',
    methods: [
      {
        signature: 'readonly buffer: BridgeBuffer = new BridgeBuffer()',
        description: 'The retained feed the HTTP routes read and the producers push into.',
        parameters: [],
      },
    ],
  },
  {
    key: 'idealizeComm',
    summary: 'The `idealize` command service: agent naming, roles, the roster, and mailbox delivery behind `POST /idealize/comm`.',
    description: 'The `idealize` command service: agent naming, roles, the roster, and mailbox delivery behind `POST /idealize/comm`.',
    methods: [
      {
        signature: 'config: () => CommConfig',
        description: 'The current settings section (preset→role mapping, background post window); the plugin swaps this for the settings source.',
        parameters: [],
      },
      {
        signature: 'ensureName(session: Session): Promise<string>',
        description: 'Give a session its agent name if it has none: the project\'s pool is recorded on first use and the draw is logged to the session so the model and the sidebar both learn it.',
        parameters: [{ name: 'session', description: 'the session to name.' }],
        returns: 'the name, new or existing.',
      },
      {
        signature: 'async adoptRole(session: Session, presetId: string | undefined): Promise<CommRole | undefined>',
        description: 'Record a role for a session whose preset carries one and give it the role\'s task title. No-op for presets outside the mapping.',
        parameters: [{ name: 'session', description: 'the session to take the role.' }, { name: 'presetId', description: 'the session\'s preset id, matched against the role mapping.' }],
        returns: 'the adopted role, or `undefined` when the preset carries none.',
      },
      {
        signature: 'async roster(): Promise<RosterEntry[]>',
        description: 'Every addressable chat. The Studio chat (the `studio` space) is the person\'s own seat and never lists: it is neither named nor a delivery target.',
        parameters: [],
        returns: 'the roster, live chats first.',
      },
      {
        signature: 'async handle(request: CommRequest): Promise<CommResponse>',
        description: 'Answer one wire request. Never throws for a caller mistake; those come back as `ok: false`.',
        parameters: [{ name: 'request', description: 'the command envelope from the wire.' }],
        returns: 'the command\'s response payload.',
      },
      {
        signature: 'async wakeCoordinator(event: StudioEvent): Promise<void>',
        description: 'Wake the project\'s coordinator for a qualifying Studio event: a mailbox note always, plus one model invocation when the coordinator is live, idle, and had an empty mailbox — so a burst invokes it once and the rest waits in the inbox. The event\'s own author is never woken.',
        parameters: [{ name: 'event', description: 'the committed Studio event.' }],
      },
      {
        signature: 'async reportFinishedTurn(session: Session, turn: number): Promise<string | undefined>',
        description: 'The safety net under the prompt\'s posting rule: when a turn that called a generation tool ends and the agent posted nothing to the Studio during it, one `idealize`-authored `message` on the project\'s timeline says who generated what, with the artefact ids, and links the agent\'s chat through `source.thread`. Nothing is recorded for a turn with no generation call, a generation that stored no artefact (its failure is in the agent\'s own chat), a chat outside a project, the Studio chat, or a turn in which the agent posted itself (any `message` it authored, a `rung` line included). The system author keeps the coordinator asleep: the line reports an act and asks nobody to do anything. An artefact stored after the turn ended (a `run_in_background` generation) takes the noteBackgroundArtefact path.',
        parameters: [{ name: 'session', description: 'the session whose turn ended; its log is read up to that turn\'s end.' }, { name: 'turn', description: 'the ended turn\'s number.' }],
        returns: 'the recorded line, or undefined when nothing was recorded.',
      },
      {
        signature: 'noteBackgroundArtefact(session: Session, event: SessionEvent<\'artefact/created\'>): void',
        description: 'The safety net\'s second path: a generation tool run with `run_in_background` stores its artefacts after the turn that started it has ended, where reportFinishedTurn never sees them. An `artefact/created` event whose record names a generation tool as its source, and whose own turn (`sourceTask.turnSeq`) has already ended, joins the session\'s pending batch; the batch\'s window (`backgroundPostDelayMs`) restarts on every arrival, and when it closes one `idealize`-authored `message` reports the batch. An artefact whose own turn is still running (every foreground generation), a chat outside a project, and the Studio chat are ignored. Disposing the plugin, or the session (forgetSession), drops a pending batch unposted.',
        parameters: [{ name: 'session', description: 'the session whose log gained the event; its log decides whether a turn is open.' }, { name: 'event', description: 'the `artefact/created` event as appended.' }],
      },
      {
        signature: 'forgetSession(session: Session): void',
        description: 'Drop a session\'s pending background batch: the session is gone, so its line would name a chat the Studio row cannot open.',
        parameters: [{ name: 'session', description: 'the disposed session.' }],
      },
    ],
  },
  {
    key: 'idealizeCron',
    summary: 'The scheduler: durable tasks, armed timers, and run history behind `ctx.idealizeCron`.',
    description: 'The scheduler: durable tasks, armed timers, and run history behind `ctx.idealizeCron`.',
    methods: [
      {
        signature: 'readonly store: CronStore',
        description: 'The durable task list and run history.',
        parameters: [],
      },
      {
        signature: 'async taskViews(): Promise<TaskView[]>',
        description: 'Tasks with computed next fires, for the HTTP surface.',
        parameters: [],
        returns: 'every stored task with its next fire time and running flag.',
      },
      {
        signature: 'async saveTask(input: Partial<CronTask>): Promise<CronTask>',
        description: 'Validate + persist one task and re-arm it. A prompt may be empty only for a reminder (`remind` set): the fire then notifies and runs nothing. A one-off whose instant has passed is refused unless the task already recorded `done`, so renaming a fired reminder still saves.',
        parameters: [{ name: 'input', description: 'Task fields from the calendar form or a calendar tool; `id` absent creates a new task.' }],
        returns: 'the stored task after defaults are applied.',
      },
      {
        signature: 'async deleteTask(id: string): Promise<boolean>',
        description: 'Disarm and remove one task; its run history stays.',
        parameters: [{ name: 'id', description: 'The task to delete.' }],
        returns: 'whether a task with that id existed.',
      },
      {
        signature: 'async toggleTask(id: string): Promise<CronTask | undefined>',
        description: 'Flip one task\'s enabled flag and re-arm it.',
        parameters: [{ name: 'id', description: 'The task to toggle.' }],
        returns: 'the updated task, or undefined when no task has that id.',
      },
      {
        signature: 'async fire(taskId: string): Promise<CronRun>',
        description: 'Run one task now (used by fires; callable for a manual "run now"). A reminder (empty prompt) runs no agent; a `remind` task raises the desktop notification (title = task name, body = the time) beside whatever ran. A one-off records `done` before its run is announced, so the pane\'s refetch already shows it fired.',
        parameters: [{ name: 'taskId', description: 'The task to run.' }],
  ...
  {
    key: 'idealizeOAuth',
    summary: 'Subscription sign-in behind `ctx.idealizeOAuth`: stored OAuth credentials and in-flight logins.',
    description: 'Subscription sign-in behind `ctx.idealizeOAuth`: stored OAuth credentials and in-flight logins.',
    methods: [
      {
        signature: 'readonly store: FileCredentialStore',
        description: 'The stored OAuth credentials, shared with the pi-ai adapter.',
        parameters: [],
      },
      {
        signature: 'oauthProviders(): { id: string; name: string; keyless: boolean; loginLabel?: string }[]',
        description: 'Every installed catalog provider whose auth offers OAuth — minus the withheld ones. Anthropic\'s flow ships in pi-ai but subscription use in third-party agents is barred by its terms today (scope decision 3: key only, standing investigation); offering the button would invite a violation, so it is withheld here rather than hidden downstream.',
        parameters: [],
        returns: 'each offered provider\'s id, display name, whether sign-in is its only auth, and optional login label.',
      },
      {
        signature: 'async status(): Promise<OAuthProviderStatus[]>',
        description: 'Current status for the sign-in surface.',
        parameters: [],
        returns: 'one row per offered provider: stored, pending, and last error.',
      },
      {
        signature: 'login(providerId: string): Promise<string>',
        description: 'Start (or join) one provider\'s login flow. Resolves with the authorize URL as soon as the flow surfaces it; the flow itself continues until the local callback settles it, after which pi-ai has persisted the credential into the store.',
        parameters: [{ name: 'providerId', description: 'a catalog provider with OAuth auth.' }],
        returns: 'the URL the user\'s browser must open.',
      },
      {
        signature: 'async logout(providerId: string): Promise<void>',
        description: 'Delete one provider\'s stored credential and clear its attempt record.',
        parameters: [{ name: 'providerId', description: 'The provider to sign out of.' }],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'async record(input: StudioEventInput): Promise<StudioAppendResult>',
        description: 'Record one event and announce it. A duplicate `messageId` returns the recorded event, writes nothing, and announces nothing.',
        parameters: [{ name: 'input', description: 'the validated submission, its project already resolved.' }],
        returns: 'the recorded (or already-recorded) event.',
      },
      {
        signature: 'async timeline(project: string, since: number = 0): Promise<StudioEvent[]>',
        description: 'A project\'s events after a cursor, oldest first.',
        parameters: [{ name: 'project', description: 'the resolved project folder.' }, { name: 'since', description: 'return events with `seq` greater than this; 0 for all.' }],
        returns: 'the retained events after the cursor.',
      },
      {
        signature: 'async state(project: string): Promise<StudioState>',
        description: 'The folded project state.',
        parameters: [{ name: 'project', description: 'the resolved project folder.' }],
        returns: 'tasks in creation order plus per-owner work views.',
      },
      {
        signature: 'presenceOf(state: StudioState): Record<string, StudioPresence>',
        description: 'Runtime presence for every participant the folded state names as a task owner. In-process, a participant is reachable exactly while a live root agent runs its session; there is no heartbeat to expire, so the answer is current by construction. Non-session participants (`user`) read as unreachable — the surface decides how to render them.',
        parameters: [{ name: 'state', description: 'the folded state whose agents are keyed.' }],
        returns: 'presence per participant key.',
      },
      {
        signature: 'async deliver( input: StudioEventInput & { target: string }, ): Promise<{ event: StudioEvent; duplicate: boolean; delivery: DeliveryState }>',
        description: 'Record one addressed message and attempt its delivery into the target\'s mailbox. One logical message: a repeated `messageId` never records a second message event — a retry re-attempts a still-queued delivery and returns the recorded state once it is delivered or acknowledged.',
        parameters: [{ name: 'input', description: 'the validated submission; `target` names the recipient.' }],
        returns: 'the message event, whether it already existed, and its delivery state.',
      },
      {
        signature: 'async overview(): Promise<StudioOverviewProject[]>',
        description: 'Every stored project\'s folded state, presence and recent timeline: the Studio\'s one read across projects.',
        parameters: [],
        returns: 'the projects in path order.',
      },
      {
        signature: 'async postFromChat(text: string, messageId: string): Promise<StudioChatPost>',
        description: 'Post one message from the Studio chat. A leading `@name` delivers to that participant on its own project\'s timeline. Anything else goes to the one Studio coordinator, on the Studio\'s own timeline.\n\nAn untagged post used to be recorded on every project the Studio watches, which put one typed line on every timeline and woke every project\'s coordinator at once (JJ, 11 Sep 2026: "its literally posting to all the agents! that\'s not right"). The Studio has an agent of its own now; it reads the line and works through the project coordinators itself.',
        parameters: [{ name: 'text', description: 'the user\'s message.' }, { name: 'messageId', description: 'the logical message id (one per send; retries reuse it).' }],
        returns: 'where the post landed, the unresolved token with who is present, or why no coordinator took it.',
      },
      {
        signature: 'async retryQueued(target: string, now: number = Date.now()): Promise<{ delivered: number; expired: number }>',
        description: 'Re-attempt every queued delivery addressed to one participant, across every stored project — the reconnection hook. A message older than the delivery expiry is recorded `delivery-expired` instead of attempted; an explicit deliver retry with its messageId can still revive it.',
        parameters: [{ name: 'target', description: 'the participant whose session came back.' }, { name: 'now', description: 'the clock for the expiry decision (tests pin it).' }],
        returns: 'how many deliveries were delivered and how many expired.',
      },
      {
        signature: 'lastSeq(project: string): number',
        description: 'The newest recorded seq, for cursors and the announce invariant. Synchronous over what is loaded: 0 for a project never read or written.',
        parameters: [{ name: 'project', description: 'the resolved project folder.' }],
  ...
  },
  {
    ...
    summary: 'Which terminal agents are working right now, for the surfaces that show it.',
    description: 'Which terminal agents are working right now, for the surfaces that show it.',
    methods: [
      {
        signature: 'working(): string[]',
        description: 'Which terminal chats are working.',
        parameters: [],
  ...
  },
  {
    ...
    description: 'The transcription service. Registered as `ctx.transcription` (one instance per context). Providers register with `register()`; consumers resolve a capture and run it.',
    methods: [
      {
        signature: 'register(provider: TranscriptionProvider): () => void',
        description: 'Register a speech provider. Throws on a duplicate id, which is a composition bug rather than a task failure.',
        parameters: [{ name: 'provider', description: 'the provider; its `id` is the registry key.' }],
        returns: 'the disposer that unregisters it.',
      },
      {
        signature: 'list(): TranscriptionDescriptor[]',
        description: 'Every registered provider\'s descriptor, in registration order.',
        parameters: [],
        returns: 'the descriptors.',
      },
      {
        signature: 'async readiness(id?: string): Promise<TranscriptionReadiness>',
        description: 'Readiness of the provider a request would resolve to.',
        parameters: [{ name: 'id', description: 'a pinned provider id; absent picks the only registered one.' }],
        returns: 'that provider\'s readiness.',
      },
      {
        signature: 'async prepare(id?: string): Promise<TranscriptionReadiness>',
        description: 'Bring the provider a request would resolve to up to `ready`.',
        parameters: [{ name: 'id', description: 'a pinned provider id; absent picks the only registered one.' }],
        returns: 'readiness once the attempt settles.',
      },
      {
        signature: 'resolve(request: TranscriptionRequest): TranscriptionSpec',
        description: 'Choose the provider for a request and hold the capture to its terms. The defaulting a caller does not state lives here, in the open, rather than inside `transcribe()`.',
        parameters: [{ name: 'request', description: 'the capture and any pinned provider.' }],
      ...
      },
      {
        ...
        description: 'Resolve a capture and run it.',
        parameters: [{ name: 'request', description: 'the capture and any pinned provider.' }, { name: 'signal', description: 'aborts the run; an aborted run rejects and produces nothing.' }],
  ...
  },
  {
    ...
    summary: 'The workspace-alias seam (`ctx.workspaceAliases`) plus the orientation flow behind the `/idealize/setup` routes.',
    description: 'The workspace-alias seam (`ctx.workspaceAliases`) plus the orientation flow behind the `/idealize/setup` routes.\n\nContract for consumers (the Files tabs, the Reconnect flow):\n\n- resolve never throws for a known alias: an unset alias returns `undefined`, a set-but-broken one returns its stored path with a non-`ok` `accessState` and a plain `reason` — render the path with a Reconnect affordance rather than dropping it.\n- set validates before it persists: a failing folder rejects with AliasProbeError and stores nothing, so a stored alias was valid at the moment it was written (it may still die later — resolve re-probes on every call).\n- Writing `documentation` re-seeds `@idealize/doc-policy`\'s folder and re-scans; writing `projectsRoot` re-seeds `@idealize/vault`\'s root. A missing dependent settings section fails loud naming the plugin.',
    methods: [
      {
        signature: 'async resolve(aliasName: WorkspaceAliasName): Promise<WorkspaceAlias | undefined>',
        description: 'Resolve one alias to its stored path and live-probed access state.',
        parameters: [{ name: 'aliasName', description: 'which alias to resolve.' }],
        returns: 'the alias with its probe verdict, or `undefined` while unset.',
      },
      {
        signature: 'async set(aliasName: WorkspaceAliasName, path: string): Promise<WorkspaceAlias>',
        description: 'Point one alias at a folder: probe it, persist it, and re-seed the dependent settings section (the Reconnect flow\'s write path).',
        parameters: [{ name: 'aliasName', description: 'which alias to write.' }, { name: 'path', description: 'absolute folder path.' }],
      ...
      {
        signature: 'async orient(request: OrientationRequest): Promise<OrientationResult>',
        description: 'The orientation component (SET-02/04/06): probe both folders, create and register the first project under the projects root when a name is given, persist the aliases + the orientation seed, seed the documentation and vault sections, and kick the documentation scan. Field failures return in `failures` with nothing persisted; a missing dependent plugin throws.',
        parameters: [{ name: 'request', description: 'the two folders and the optional first-project name.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'idealize/comm-focus',
    mode: 'emit',
    signature: '\'idealize/comm-focus\'(sessionId: string): void',
    summary: 'A chat asked the shell to bring a session to the front (`idealize focus`).',
    description: 'A chat asked the shell to bring a session to the front (`idealize focus`).',
    parameters: [{ name: 'sessionId', description: 'the session to show.' }],
  },
  {
    name: 'idealize/comm-mail',
    mode: 'emit',
    signature: '\'idealize/comm-mail\'(sessionId: string, message: CommMessage, unread: number): void',
    summary: 'A message landed in a session\'s mailbox; the unread count is the badge.',
    description: 'A message landed in a session\'s mailbox; the unread count is the badge.',
    parameters: [{ name: 'sessionId', description: 'the recipient.' }, { name: 'message', description: 'what arrived.' }, { name: 'unread', description: 'the recipient\'s mailbox size after delivery.' }],
  },
  {
    name: 'idealize/comm-notify',
    mode: 'emit',
    signature: '\'idealize/comm-notify\'(title: string, body: string, sound: boolean): void',
    summary: 'A chat asked for a system notification (`idealize notify`).',
    description: 'A chat asked for a system notification (`idealize notify`).',
    parameters: [{ name: 'title', description: 'the notification title, already carrying the sender\'s label.' }, { name: 'body', description: 'the notification text.' }, { name: 'sound', description: 'whether the sender asked for a sound.' }],
  },
  {
    name: 'idealize/comm-reveal',
    mode: 'emit',
    signature: '\'idealize/comm-reveal\'(path: string, open: boolean, sessionId: string): void',
    summary: 'A chat pointed the person at a file (`idealize reveal`).',
    description: 'A chat pointed the person at a file (`idealize reveal`).',
    parameters: [{ name: 'path', description: 'the absolute, symlink-resolved file path.' }, { name: 'open', description: 'whether to open it in the viewer as well as select it.' }, { name: 'sessionId', description: 'the session whose project holds the file.' }],
  },
  {
    name: 'idealize/cron-changed',
    mode: 'emit',
    signature: '\'idealize/cron-changed\'(): void',
    summary: 'The task list changed (a task was added, edited, toggled, deleted, or a one-off recorded done); the host bridge forwards it so the Schedule pane refetches.',
    description: 'The task list changed (a task was added, edited, toggled, deleted, or a one-off recorded done); the host bridge forwards it so the Schedule pane refetches. Carries no payload: the pane reads the routes.',
    parameters: [],
  },
  {
    name: 'idealize/cron-run',
    mode: 'emit',
    signature: '\'idealize/cron-run\'(run: CronRun, task: CronTask): void',
    summary: 'One scheduled run finished (any status); the host bridge forwards it to the shell\'s notification feed.',
    description: 'One scheduled run finished (any status); the host bridge forwards it to the shell\'s notification feed.',
    parameters: [{ name: 'run', description: 'The recorded run: status, timing, and the session it ran in.' }, { name: 'task', description: 'The task definition that fired.' }],
  },
  {
    name: 'idealize/studio-event',
    mode: 'emit',
    signature: '\'idealize/studio-event\'(event: StudioEvent): void',
    summary: 'A Studio event committed to its project\'s timeline; announced after the append is on disk, never for a suppressed duplicate.',
    description: 'A Studio event committed to its project\'s timeline; announced after the append is on disk, never for a suppressed duplicate.',
    parameters: [{ name: 'event', description: 'the recorded event, with its assigned id, seq and time.' }],
  },
  {
    name: 'idealize/terminal-activity',
    mode: 'emit',
    signature: '\'idealize/terminal-activity\': (change: { id: string; working: boolean }) => void',
    summary: 'A terminal agent started or stopped working, from its shell\'s output.',
    description: 'A terminal agent started or stopped working, from its shell\'s output.',
    parameters: [{ name: 'change', description: 'the terminal\'s id and whether it is working now.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AliasAccessState',
    declaration: 'export type AliasAccessState = \'ok\' | \'missing\' | \'not-a-directory\' | \'unreadable\' | \'unwritable\';',
  },
  {
  ...
  {
    name: 'ArtefactCreateRequest',
    declaration: 'export interface ArtefactCreateRequest {\n    session: Session;\n    mediaType: string;\n    bytes: Uint8Array | AsyncIterable<Uint8Array>;\n    settings: JsonValue;\n    sourceTask: {\n        turnSeq: number;\n        callId: CallId;\n        toolName: string;\n    };\n    provenance: {\n        provider: string;\n        model: string;\n        workspaceId: WorkspaceId;\n    };\n}',
  },
  {
    name: 'ArtefactDisposition',
    declaration: 'export type ArtefactDisposition = \'kept\' | \'archived\';',
  },
  {
    name: 'ArtefactFolderSettings',
    declaration: 'export interface ArtefactFolderSettings {\n    images: string;\n    sounds: string;\n    video: string;\n    other: string;\n    archive: string;\n}',
  },
  {
    name: 'ArtefactId',
    declaration: 'export type ArtefactId = Branded<\'ArtefactId\'>;',
  },
  {
    name: 'ArtefactListFilter',
    declaration: 'export interface ArtefactListFilter {\n    workspaceId?: WorkspaceId;\n    mediaType?: string;\n}',
  },
  {
    name: 'ArtefactProvenance',
    declaration: 'export interface ArtefactProvenance {\n    readonly provider: string;\n    readonly model: string;\n    readonly createdAt: string;\n    readonly workspaceId: WorkspaceId;\n}',
  },
  {
    name: 'ArtefactRecord',
    declaration: 'export interface ArtefactRecord {\n    readonly schemaVersion: typeof ARTEFACT_SCHEMA_VERSION;\n    readonly id: ArtefactId;\n    readonly mediaType: string;\n    readonly storage: ArtefactStorage;\n    readonly sourceTask: ArtefactSourceTask;\n    readonly settings: JsonValue;\n    readonly provenance: ArtefactProvenance;\n    readonly disposition?: \'archived\' | undefined;\n}',
  },
  {
    name: 'ArtefactSourceTask',
    declaration: 'export interface ArtefactSourceTask {\n    readonly sessionId: SessionId;\n    readonly turnSeq: number;\n    readonly callId: CallId;\n    readonly toolName: string;\n}',
  },
  {
    name: 'ArtefactStorage',
    declaration: 'export interface ArtefactStorage {\n    readonly kind: \'file\';\n    readonly relPath: string;\n    readonly bytes: number;\n    readonly sha256: string;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface BackendCredential {\n    displayName: string;\n    env: string;\n}',
  },
  {
    name: 'BackendCredentialState',
    declaration: 'export interface BackendCredentialState extends BackendCredential {\n    backend: string;\n    connected: boolean;\n    artefacts: readonly GenArtefact[];\n}',
  },
  {
  ...
  {
    name: 'CommCommand',
    declaration: 'export type CommCommand = \'ping\' | \'list\' | \'notify\' | \'send\' | \'inbox\' | \'peek\' | \'rung\' | \'board\' | \'spawn\' | \'post\' | \'chat\' | \'transcript\' | \'setStatus\' | \'focus\' | \'reveal\' | \'task\' | \'progress\' | \'blocked\' | \'need\' | \'done\' | \'studio\' | \'handoff\' | \'accept\' | \'reject\' | \'decide\' | \'synthesis\';',
  },
  {
    name: 'CommConfig',
    declaration: 'export interface CommConfig extends RoleConfig {\n    backgroundPostDelayMs: number;\n}',
  },
  {
    name: 'CommExchange',
    declaration: 'export interface CommExchange {\n    index: number;\n    question: string;\n    answer?: string;\n}',
  },
  {
    name: 'CommMessage',
    declaration: 'export interface CommMessage {\n    from: string;\n    fromLabel?: string;\n    body: string;\n    timestamp: string;\n    piece?: string;\n    rung?: string;\n    blocker?: string;\n}',
  },
  {
    name: 'CommRequest',
    declaration: 'export interface CommRequest {\n    command: CommCommand;\n    from?: string;\n    target?: string;\n    body?: string;\n    title?: string;\n    sound?: boolean;\n    open?: boolean;\n    limit?: number;\n    name?: string;\n    path?: string;\n    model?: string;\n    piece?: string;\n    rung?: string;\n    blocker?: string;\n    coordinator?: boolean;\n    studio?: boolean;\n    task?: string;\n    owner?: string;\n    action?: boolean;\n}',
  },
  {
    name: 'CommResponse',
    declaration: 'export interface CommResponse {\n    ok: boolean;\n    error?: string;\n    info?: string;\n    warning?: string;\n    sessions?: CommSessionInfo[];\n    messages?: CommMessage[];\n    exchanges?: CommExchange[];\n    rungs?: CommRung[];\n    tasks?: StudioTaskRow[];\n    synthesis?: {\n        body: string;\n        stale: boolean;\n        at: string;\n    };\n}',
  },
  {
    name: 'CommRole',
    declaration: 'export type CommRole = \'project-agent\' | \'studio-agent\';',
  },
  {
    name: 'CommRung',
    declaration: 'export interface CommRung {\n    piece: string;\n    rung: string;\n    blocker: string;\n    note?: string;\n    session: string;\n    sessionLabel?: string;\n    projectPath?: string;\n    updated: string;\n}',
  },
  {
    name: 'CommSessionInfo',
    declaration: 'export interface CommSessionInfo {\n    id: string;\n    name?: string;\n    title: string;\n    projectPath?: string;\n    status?: string;\n    unread: number;\n    role: \'project-agent\' | \'studio-agent\' | \'chat\';\n    running: boolean;\n    live: boolean;\n}',
  },
  {
  ...
  {
    name: 'CronRun',
    declaration: 'export interface CronRun {\n    taskId: string;\n    firedAt: string;\n    status: RunStatus;\n    sessionId?: string;\n    durationMs?: number;\n    detail?: string;\n}',
  },
  {
    name: 'CronStore',
  ...
  {
    name: 'CronTask',
    declaration: 'export interface CronTask {\n    id: string;\n    name: string;\n    schedule: Schedule;\n    prompt: string;\n    cwd: string;\n    remind?: boolean;\n    done?: string;\n    provider?: string;\n    model?: string;\n    enabled: boolean;\n    createdAt: string;\n}',
  },
  {
    name: 'DiffCallView',
  ...
  {
    name: 'DocScanRecord',
    declaration: 'export type DocScanRecord = z.infer<typeof docScanRecordSchema>;',
  },
  {
    name: 'DocsSearchHit',
    declaration: 'export interface DocsSearchHit {\n    path: string;\n    kind: string;\n    title: string;\n    snippet: string;\n}',
  },
  {
    name: 'DocsState',
    declaration: 'export interface DocsState {\n    configured: boolean;\n    folder?: string;\n    policyVersion: string;\n    lastScan?: DocScanRecord;\n}',
  },
  {
    name: 'Domain',
  ...
  {
    name: 'FileCredentialStore',
    declaration: 'export class FileCredentialStore implements CredentialStore {\n    constructor(private readonly path: string);\n    read(providerId: string): Promise<Credential | undefined>;\n    async list(): Promise<readonly CredentialInfo[]>;\n    modify(providerId: string, fn: (current: Credential | undefined) => Promise<Credential | undefined>): Promise<Credential | undefined>;\n    delete(providerId: string): Promise<void>;\n}',
  },
  {
    name: 'FileDiff',
  ...
  {
    name: 'GenArtefact',
    declaration: 'export type GenArtefact = \'image\' | \'video\' | \'audio\';',
  },
  {
    name: 'GenAttachment',
    declaration: 'export interface GenAttachment {\n    mediaType: string;\n    data: string;\n}',
  },
  {
    name: 'GenBooleanField',
    declaration: 'export interface GenBooleanField extends GenInputFieldBase {\n    kind: \'boolean\';\n    default?: boolean;\n}',
  },
  {
    name: 'GenCatalogEntry',
    declaration: 'export interface GenCatalogEntry {\n    backend: string;\n    model: GenModelInfo;\n}',
  },
  {
    name: 'GenEnumField',
    declaration: 'export interface GenEnumField extends GenInputFieldBase {\n    kind: \'enum\';\n    values: readonly string[];\n    default?: string;\n}',
  },
  {
  ...
  {
    name: 'GenInputField',
    declaration: 'export type GenInputField = GenEnumField | GenNumberField | GenBooleanField;',
  },
  {
    name: 'GenInputFieldBase',
    declaration: 'export interface GenInputFieldBase {\n    name: string;\n    label: string;\n}',
  },
  {
    name: 'GenModality',
    declaration: 'export type GenModality = \'text\' | \'image\' | \'audio\' | \'video\';',
  },
  {
    name: 'GenModelInfo',
    declaration: 'export interface GenModelInfo {\n    id: string;\n    name: string;\n    artefact: GenArtefact;\n    inputModalities: readonly GenModality[];\n    outputMediaTypes: readonly string[];\n}',
  },
  {
    name: 'GenModelRef',
    declaration: 'export interface GenModelRef {\n    backend: string;\n    model: string;\n}',
  },
  {
    name: 'GenNumberField',
    declaration: 'export interface GenNumberField extends GenInputFieldBase {\n    kind: \'number\';\n    integer: boolean;\n    min?: number;\n    max?: number;\n    default?: number;\n}',
  },
  {
    name: 'GoalActivation',
  ...
  {
    name: 'OAuthProviderStatus',
    declaration: 'export interface OAuthProviderStatus {\n    id: string;\n    name: string;\n    loginLabel?: string;\n    keyless: boolean;\n    stored: boolean;\n    pending: boolean;\n    userCode?: string;\n    error?: string;\n}',
  },
  {
  ...
  {
    name: 'RulesetVersion',
    declaration: 'export interface RulesetVersion {\n    sha: string;\n    date: string;\n    id: string;\n}',
  },
  {
  ...
  {
    name: 'RunStatus',
    declaration: 'export type RunStatus = \'ok\' | \'error\' | \'missed\' | \'skipped-busy\';',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface StudioAppendResult {\n    event: StudioEvent;\n    duplicate: boolean;\n}',
  },
  {
    name: 'StudioChatPost',
    declaration: 'export type StudioChatPost = {\n    kind: \'delivered\';\n    target: string;\n    project: string;\n    delivery: DeliveryState;\n} | {\n    kind: \'no-coordinator\';\n    reason: string;\n} | {\n    kind: \'unresolved\';\n    token: string;\n    names: string[];\n};',
  },
  {
    name: 'StudioEvent',
    declaration: 'export interface StudioEvent extends StudioEventInput {\n    id: StudioEventId;\n    seq: number;\n    at: string;\n    visibility: \'studio\';\n}',
  },
  {
    name: 'StudioEventId',
    declaration: 'export type StudioEventId = Branded<\'StudioEventId\'>;',
  },
  {
    name: 'StudioEventInput',
    declaration: 'export interface StudioEventInput {\n    project: string;\n    author: string;\n    kind: StudioEventKind;\n    subtype?: string | undefined;\n    target?: string | undefined;\n    taskId?: string | undefined;\n    body?: string | undefined;\n    source?: StudioEventSource | undefined;\n    messageId?: string | undefined;\n    visibility?: \'studio\' | undefined;\n}',
  },
  {
    name: 'StudioEventKind',
    declaration: 'export type StudioEventKind = typeof STUDIO_EVENT_KINDS[number];',
  },
  {
    name: 'StudioEventSource',
    declaration: 'export interface StudioEventSource {\n    thread?: string | undefined;\n    event?: string | undefined;\n}',
  },
  {
    name: 'StudioHandoff',
    declaration: 'export interface StudioHandoff {\n    event: StudioEventId;\n    from: string;\n    to: string;\n    state: \'offered\' | \'needs-clarification\';\n}',
  },
  {
    name: 'StudioOverviewProject',
    declaration: 'export interface StudioOverviewProject {\n    project: string;\n    lastSeq: number;\n    state: StudioState & {\n        presence: Record<string, StudioPresence>;\n    };\n    recent: StudioEvent[];\n}',
  },
  {
    name: 'StudioState',
    declaration: 'export interface StudioState {\n    tasks: StudioTask[];\n    agents: Record<string, StudioAgentView>;\n    deliveries: Record<string, DeliveryState>;\n    synthesis?: StudioSynthesis | undefined;\n}',
  },
  {
    name: 'StudioSynthesis',
    declaration: 'export interface StudioSynthesis {\n    event: StudioEventId;\n    author: string;\n    at: string;\n    body: string;\n    stale: boolean;\n}',
  },
  {
    name: 'StudioTask',
  ...
  {
    name: 'TranscriptionDescriptor',
    declaration: 'export interface TranscriptionDescriptor {\n    id: string;\n    label: string;\n    location: ProcessingLocation;\n    model: string;\n    sampleRate: number;\n}',
  },
  {
  ...
  {
    name: 'TranscriptionReadiness',
    declaration: 'export type TranscriptionReadiness = {\n    state: \'ready\';\n} | {\n    state: \'preparing\';\n    percent: number;\n} | {\n    state: \'unavailable\';\n    detail: string;\n};',
  },
  {
  ...
  {
    name: 'TranscriptionSpec',
    declaration: 'export interface TranscriptionSpec {\n    provider: TranscriptionProvider;\n    samples: Float32Array;\n    sampleRate: number;\n}',
  },
  {
  ...
  {
    name: 'WorkspaceAlias',
    declaration: 'export interface WorkspaceAlias {\n    name: WorkspaceAliasName;\n    path: string;\n    accessState: AliasAccessState;\n    reason?: string;\n}',
  },
  {
    name: 'WorkspaceAliasName',
    declaration: 'export type WorkspaceAliasName = \'projectsRoot\' | \'documentation\' | \'skills\';',
  },
]
