...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The launcher strip at the top of the hero composer card (mode toggle + activity chips in the fork\'s welcome design).',
    doc: 'The launcher strip at the top of the hero composer card (mode toggle +\nactivity chips in the fork\'s welcome design). Root scope: no session\nexists yet; the filler stages choices for the next one.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation HeroLauncherDefault',
      '@idealize/ui-bar HeroLauncher',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'conversation.input.access',
    kind: 'single',
    scope: 'session',
    summary: 'The access-mode seat at the left end of the composer tool row, holding the permission chip.',
    doc: 'The access-mode seat at the left end of the composer tool row, holding\nthe permission chip. Unoccupied it renders that chip, so the bar\'s\ndefault composer is unchanged; an occupant receives the chip as\n`control` (InputAccessOwnerProps) and decides where it goes —\nthe seat exists so a product can gather the row\'s chips into one\noverflow control without the bar knowing what that control is.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      '@idealize/activity-pills ComposerOverflow',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@idealize/ui-tasks NoPlanStrip id \'todo\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      '@idealize/ui-gallery GenerationSettings id \'idealize-gallery-settings\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation ReturnToggle id \'composer-return-toggle\'',
      '@idealize/ui-bar HeartButton id \'idealize-heart\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@idealize/ui-terminal ModeShortcut id \'idealize-mode-shortcut\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@idealize/ui-gallery GalleryView id \'gallery\'',
      '@idealize/ui-gallery GalleryView id \'motion\'',
      '@idealize/ui-soundstage SoundstageEntry id \'soundstage\'',
      '@idealize/ui-studio StudioView id \'studio\'',
      '@idealize/ui-terminal TerminalView id \'terminal\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@idealize/artefacts FoldersRow id \'idealize-artefact-folders\'',
      '@idealize/notify ChimeRow id \'idealize-chime\'',
      '@idealize/onboarding OnboardingRow id \'idealize-onboarding\'',
      '@idealize/telegram TelegramRow id \'idealize-telegram\'',
      '@idealize/ui-bar NoLanguageRow id \'language\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@idealize/ui-bar NoProviderDialog id \'deepseek-official\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'The banner strip: a full-width grid row ABOVE every column (V0 pinned its announcement banner under the title bar, pushing the columns down).',
    doc: 'The banner strip: a full-width grid row ABOVE every column (V0 pinned\nits announcement banner under the title bar, pushing the columns down).\nContent here owns its ground; empty entries cost no height. Root-scoped.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      '@idealize/notify AnnouncementBanner id \'idealize-announcement\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'shell.banner\', () => ctx.slots.register(\n      { name: \'shell.banner\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The deck: a second docked column between center and the drawer, sized for documents (V0\'s file/document panel).',
    doc: 'The deck: a second docked column between center and the drawer, sized\nfor documents (V0\'s file/document panel). `ctx.layout` owns whether it\nis open (openDeck/closeDeck); the user owns its width via the drag\nhandle; the occupant owns what it shows. Root-scoped.',
    registerOptions: [],
    ownerProps: [
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'The bottom dock strip, a full-width grid row under every column.',
    doc: 'The bottom dock strip, a full-width grid row under every column.\nContent here has its own layout ground — nothing in the columns can\nsit underneath it. Empty entries cost no height.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The right drawer column, a docked grid sibling of the other columns — content here pushes the center aside instead of covering it.',
    doc: 'The right drawer column, a docked grid sibling of the other columns —\ncontent here pushes the center aside instead of covering it.\n`ctx.layout` owns whether it is open (openDrawer/closeDrawer); the\noccupant owns what it shows. Root-scoped: it renders with or without a\ncurrent session.',
    registerOptions: [],
    ownerProps: [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      '@idealize/onboarding OnboardingOverlay id \'idealize-onboarding\'',
      '@idealize/ui-launch LaunchOverlay id \'idealize-launch\'',
      '@idealize/ui-tour Overlay id \'idealize-tour\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'Optional window-level controls in the sidebar header, on the collapse toggle\'s row, left of it.',
    doc: 'Optional window-level controls in the sidebar header, on the\ncollapse toggle\'s row, left of it. Declared by this package\'s \'sidebar\'\nentry; each control receives only the column state.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      '@idealize/ui-bar MinimodeButton id \'idealize-minimode\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'sidebar.workspaces.pinned',
    kind: 'list',
    scope: 'root',
    summary: 'The pinned strip at the top of the sidebar\'s list, above every project (declared by the WorkspaceBrowser entry): one card per entry, rendered while the column is wide and no search is typed, in `order`.',
    doc: 'The pinned strip at the top of the sidebar\'s list, above every project\n(declared by the WorkspaceBrowser entry): one card per entry, rendered\nwhile the column is wide and no search is typed, in `order`. IDEalize\'s\nStudio card lives here (FORK.md); V0 pinned its lead-agent card in the\nsame place.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'sidebar.workspaces\' (client-ui-workspace), so it exists while that entry is mounted',
    occupants: [
      '@idealize/ui-bar StudioCard id \'idealize-studio-card\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.workspaces.pinned\', () => ctx.slots.register(\n      { name: \'sidebar.workspaces.pinned\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
