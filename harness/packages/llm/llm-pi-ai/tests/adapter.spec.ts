...
describe('PiAiAdapter provider routing', () => {
  ...
  it('forwards common stream options and profile reasoning', async () => {
    ...
    // The catalog entry pins `compat.maxTokensField` to `max_tokens` for
    // DeepSeek, so the caller's cap travels under that name.
...
describe('provider profile lifecycle', () => {
  ...
  it('lets an OAuth-only route ignore a stale apiKeyEnv and authenticate from the stored credential', async () => {
    // IDEalize fork: openai-codex takes no key, so a reference left in its
    // profile after the key was removed can serve nothing; the route resolves
    // from the OAuth store instead of failing on the reference.
    vi.stubEnv('PI_STALE_REF', '')
    const payload = Buffer.from(JSON.stringify({ 'https://api.openai.com/auth': { chatgpt_account_id: 'acct-1' } })).toString('base64')
    const access = `eyJhbGciOiJub25lIn0.${payload}.sig`
    const stored = { type: 'oauth', access, refresh: 'refresh-1', expires: Date.now() + 60 * 60 * 1000 }
    const store = {
      read: () => Promise.resolve(stored),
      list: () => Promise.resolve([{ providerId: 'openai-codex', type: 'oauth' }]),
      modify: (_provider: string, fn: (current: unknown) => Promise<unknown>) => fn(stored),
    }
    const server = await mockServer([{ events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.provide('idealizeOAuth', { store } as never)
    await ctx.plugin(LlmPiAi, {
      providers: { 'openai-codex': { apiKeyEnv: 'PI_STALE_REF', baseURL: server.url, transport: 'sse' } },
    })
    const result = await assemble(ctx, { provider: 'openai-codex', model: 'gpt-5.4', messages: [] })
    expect(result.finish).not.toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    expect(server.headers[0]?.authorization).toBe(`Bearer ${access}`)
    expect(server.headers[0]?.['chatgpt-account-id']).toBe('acct-1')
  })
