...
describe('catalog routes with per-model configuration', () => {
  ...
  it('lists GPT-6 Astra on the OpenAI API-key route and the ChatGPT sign-in route', async () => {
    // The installed catalog carries the model under both `openai` (API key)
    // and `openai-codex` (ChatGPT OAuth); the harness serves each list
    // untouched, so the model reaches every picker built over either route.
    const ctx = await harness({ providers: { 'openai': {}, 'openai-codex': { apiKeyEnv: KEY_ENV } } })

    for (const route of ['openai', 'openai-codex'] as const) {
      const listed = await ctx.llm.listModels(route)
      expect(listed, `${route} lists gpt-6-astra`).toContainEqual({
        provider: route,
        id: 'gpt-6-astra',
        name: 'GPT-6 Astra',
        inputModalities: ['text', 'image'],
      })
      // The two routes publish different level sets for the same model (the
      // sign-in route adds `minimal`), and a release may move either, so the
      // efforts are read from the installed catalog rather than named here.
      const model = getBuiltinModels(route).find(candidate => candidate.id === 'gpt-6-astra')
      if (model === undefined) throw new Error(`the installed catalog dropped gpt-6-astra from ${route}`)
      const info = await ctx.llm.resolveModelInfo(route, 'gpt-6-astra')
      expect(info.reasoning?.efforts.map(effort => effort.id), `${route} efforts`)
        .toEqual(getSupportedThinkingLevels(model as Model<Api>))
    }
  })
...
describe('reasoning-dispatch compat switches', () => {
  ...
  it('skips models of other protocols on a mixed route instead of failing them', () => {
    // A route-level switch must land on the completions models without
    // invalidating the responses models beside them. Which installed provider
    // spans both protocols drifts between pi-ai releases, so the test takes
    // the first that does rather than naming one.
    const route = getBuiltinProviders().find((provider) => {
      // Across every provider the union of model APIs narrows to `never`, so
      // the set is read as the wider `Api`.
      const apis = new Set<Api>((getBuiltinModels(provider) as readonly Model<Api>[]).map(model => model.api))
      return apis.has('openai-completions') && apis.has('openai-responses')
    })
    if (route === undefined) throw new Error('the installed catalog ships no route spanning openai-completions and openai-responses')
    const catalog = getBuiltinModels(route) as readonly Model<Api>[]
    const completions = catalog.find(model => model.api === 'openai-completions') as Model<Api>
    const responses = catalog.find(model => model.api === 'openai-responses') as Model<Api>
