...
describe('toStreamChunks', () => {
  ...
  it('reads an error stop under an aborted caller signal as the caller’s cancel', async () => {
    // pi-ai resolves auth under the request signal before dispatch; an abort
    // that lands there ends the stream through its setup-failure path, which
    // spells the stop `error` and carries the abort reason as the message.
    const controller = new AbortController()
    controller.abort('already stopped')
    const error = assistant({ stopReason: 'error', errorMessage: 'already stopped' })
    const chunks = await collect(toStreamChunks(feed({ type: 'error', reason: 'error', error }), undefined, controller.signal))
    expect(chunks.at(-1)).toEqual({
      type: 'finish',
  ...
  it('keeps an error stop as an error while the caller signal is live', async () => {
    const error = assistant({ stopReason: 'error', errorMessage: 'boom' })
    const chunks = await collect(toStreamChunks(feed({ type: 'error', reason: 'error', error }), undefined, new AbortController().signal))
    expect(chunks.at(-1)).toEqual({
      type: 'finish',
...
describe('mapStopReason / mapUsage', () => {
  ...
  it.each(['pending', 'deferred'] as const)('fails a terminal %s stop the adapter never requests', (stopReason) => {
    expect(mapStopReason(assistant({ stopReason, content: [{ type: 'text', text: 'ok' }] }))).toEqual({
      kind: 'error',
      failure: {
        message: `pi-ai reported stop reason "${stopReason}" for model "deepseek-v4-flash", which this adapter does not request`,
        code: 'PI_AI_ERROR',
      },
    })
  })
