...
function directoryEntries(
  ...
  // answers what pi-ai ships. IDEalize fork: with a persistent OAuth store
  // composed (`ctx.idealizeOAuth`), an OAuth-only route resolves from its
  // stored credential, so it is offered too.
  for (const provider of catalog) {
    if (catalogProviderTakesApiKey(provider)
      || (oauthAvailable && catalogProviderOffersOAuth(provider))) declare(provider, provider)
  }
...
export function apply(ctx: Context, config: Config): void {
  ...
  const resolveApiKey = async (
    ...
    // IDEalize fork: a catalog provider with no api-key method (openai-codex)
    // can use no key, and pi-ai discovers no ambient key for it either, so a
    // reference left behind in its profile defers to the stored OAuth
    // credential instead of failing the route (JJ, 14 Sep 2026: the Studio
    // coordinator's every turn died on a stale reference after the key was
    // removed).
    if (!catalogProviderTakesApiKey(provider) && catalogProviderOffersOAuth(provider)) return undefined
  ...
  // IDEalize fork seam: the persistent OAuth store, when @idealize/provider-pack
  // is composed. Read per call — the service may join after this plugin mounts.
  const oauthStore = (): CredentialStore | undefined =>
    (ctx.get('idealizeOAuth') as { store?: CredentialStore } | undefined)?.store

  // IDEalize fork: optional attribution service (project labels for spend).
  const annotate = (sessionId: string | undefined): Record<string, string> | undefined =>
    (ctx.get('idealizeAttribution') as { headersFor?: (id: string | undefined) => Record<string, string> | undefined } | undefined)
      ?.headersFor?.(sessionId)

  const adapter = new PiAiAdapter({
    profiles,
    resolveApiKey,
    credentialStore: oauthStore,
  ...
  const ensureDirectory = (): void => {
    const entries = directoryEntries(profiles(), oauthStore() !== undefined)
  ...
  ensureDirectory()
  // IDEalize fork: when the OAuth store service joins (or leaves) after this
  // plugin mounted, re-derive the directory so OAuth-only routes appear or
  // withdraw with it. Contained like the onChange refreshes below.
  ctx.inject(['idealizeOAuth'], (oauthCtx) => {
    oauthCtx.effect(() => {
      const refresh = (): void => {
        try {
          ensureDirectory()
        } catch (error) {
          ctx.logger.error('llm-pi-ai: keeping the previous configurable-provider directory after a refused oauth refresh')
          ctx.logger.error(error)
        }
      }
      refresh()
      return refresh
    }, 'llm-pi-ai: oauth-aware directory refresh')
  })
  /**
