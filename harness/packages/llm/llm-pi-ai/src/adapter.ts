...
interface PiAiSnapshot {
  ...
  /** The credential store the collection was built over, part of its identity. */
...
export interface PiAiAdapterOptions {
  ...
  /**
   * IDEalize fork seam: resolve the optional persistent OAuth credential
   * store each snapshot build. With a store present, pi-ai's
   * `resolveProviderAuth` can settle an OAuth-only catalog route (such as
   * `openai-codex`) from its stored credential — including refresh — for a
   * profile that names no `apiKeyEnv`. Absent, behaviour is exactly upstream.
   */
  credentialStore?: () => CredentialStore | undefined
  /**
   * IDEalize fork seam: contribute per-request attribution headers derived
   * from the requesting session (project labels for the spend meter).
   * Returned names never override Harness attribution or profile-reserved
   * headers; absent or throwing, behaviour is exactly upstream.
   */
  annotateRequest?: (sessionId: string | undefined) => Record<string, string> | undefined
  /**
...
export class PiAiAdapter extends LlmAdapter {
  /** IDEalize fork: annotation headers for one request, never throwing. */
  private annotationFor(sessionId: string | undefined): Record<string, string> {
    try {
      return this.config.annotateRequest?.(sessionId) ?? {}
    } catch {
      return {}
    }
  }
  ...
  private current(): PiAiSnapshot {
    ...
    if (this.snapshot?.profiles === profiles && this.snapshot.store === store) return this.snapshot
    const models: MutableModels = createModels(store === undefined ? {} : { credentials: store })
    for (const profile of profiles.values()) models.setProvider(profile.piProvider)
    this.snapshot = { profiles, store, models }
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      const events = snapshot.models.streamSimple(model, context, {
        ...
        // Harness-owned and therefore win collisions. Annotation headers
        // (IDEalize fork) sit between the two.
        headers: requestHeaders({
          ...profile.headers,
          ...this.annotationFor(options.sessionId === undefined ? undefined : String(options.sessionId)),
        }),
      })
      // The caller's signal alone decides what an in-stream `error` means: the
      // watchdog also aborts `upstream`, and its expiry is reported as TIMEOUT below.
