...
export async function discoverModels(
  ...
  if (!response.ok) {
    throw new LlmError(
      `${url} answered ${response.status}${response.status === 401 || response.status === 403 ? '; check the API key (sent as a Bearer token; a provider wanting another scheme refuses it)' : ''}`,
      'DISCOVERY_FAILED',
    )
  }
  let text: string
