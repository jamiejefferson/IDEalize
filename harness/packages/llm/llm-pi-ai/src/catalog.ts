...
/**
 * pi-ai thinking formats a profile cannot name: each drives the request through
 * a template-argument dict (`chatTemplateKwargs`, or `chatTemplateArgs` for
 * `baseten`), which this configuration does not expose.
 */
 ...
 * key type is a drift gate: a pi-ai upgrade that adds a format fails
 * compilation here until the format is classified as offered here or withheld
 * above, so the offer never silently lags the upstream set.
 */
...
/**
 * Whether the installed catalog provider for one route offers an OAuth
 * method. IDEalize fork seam: with a persistent credential store injected
 * (see `PiAiAdapterOptions.credentialStore`), such a route is serviceable
 * from its stored credential, so the directory may offer it.
 * @param provider - provider route key.
 * @returns whether the catalog provider declares OAuth auth.
 */
