...
Shared filesystem path helpers for IDEalize harness user data.

## DSH home

`resolveDshHome()` resolves the single-root harness home. Precedence, highest first: an explicit configured path, `$DSH_HOME`, then `~/.dsh`. The harness keeps all user data under one root.
...
`defaultDshHome()` returns the default harness home by joining the operating-system home directory with `.dsh`, using Node's platform path rules.
