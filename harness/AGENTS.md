...
The IDEalize harness, forked from [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness), is a plugin-based agent harness on vendored Cordis: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.
...
Docs accompany every code change: update affected README and JSDoc contracts together. Docs are English only (JJ, 1 and 16 Sep 2026): the inherited Chinese corpus and its translation tooling are gone; write no `.zh.md` files. Current-state prose, one physical line per paragraph, one home per fact, and word budgets live in [docs/AGENTS.md](docs/AGENTS.md).
...
## Amending the running app (service hatch sessions)

If this chat was opened from IDEalize's own service hatch, you are editing the app hosting you. Any dev server you preview against is a separate process; the running app only picks up changes on restart, and restarting it ends every open chat, this one included; say so and get a go-ahead before suggesting it. Commit or checkpoint before any restart. Work in short verifiable steps: `pnpm run build` and the affected package's tests before declaring a change done. [FORK.md](FORK.md) owns fork discipline — IDEalize code goes in `packages/idealize/*`; touch upstream files only when a seam is missing, and log each touch in the FORK.md table.
