...
flowchart LR
  ...
  pkg_artefacts["artefacts"]
  svc_artefacts["ctx.artefacts<br/>Generated-artefact store"]
  pkg_gen_tools["gen-tools"]
  pkg_ui_soundstage["ui-soundstage"]
  pkg_generate["generate"]
  svc_generation["ctx.generation<br/>Media generation capability"]
  pkg_services["services"]
  pkg_gen_fixture["gen-fixture"]
  pkg_spaces["spaces"]
  pkg_doc_policy["doc-policy"]
  svc_docPolicy["ctx.docPolicy<br/>Project document policy"]
  pkg_vault["vault"]
  pkg_setup["setup"]
  pkg_freetokens["freetokens"]
  svc_idealizeAttribution["ctx.idealizeAttribution<br/>Free-tokens request attribution"]
  pkg_host_bridge["host-bridge"]
  svc_idealizeBridge["ctx.idealizeBridge<br/>Desktop shell event bridge"]
  pkg_comm["comm"]
  svc_idealizeComm["ctx.idealizeComm<br/>Agent-to-agent messaging"]
  pkg_activity_pills["activity-pills"]
  pkg_studio["studio"]
  svc_idealizeStudio["ctx.idealizeStudio<br/>Studio coordination timeline"]
  pkg_ui_terminal["ui-terminal"]
  svc_idealizeTerminals["ctx.idealizeTerminals<br/>Terminal agent activity"]
  pkg_askbar["askbar"]
  pkg_skills["skills"]
  svc_idealizeSkills["ctx.idealizeSkills<br/>Skills folder catalogue"]
  pkg_ui_bar["ui-bar"]
  pkg_transcribe["transcribe"]
  svc_transcription["ctx.transcription<br/>Speech transcription capability"]
  pkg_cron["cron"]
  svc_idealizeCron["ctx.idealizeCron<br/>Scheduled tasks"]
  pkg_provider_pack["provider-pack"]
  svc_idealizeOAuth["ctx.idealizeOAuth<br/>Provider OAuth credential store"]
  pkg_models["models"]
  svc_workspaceAliases["ctx.workspaceAliases<br/>Workspace folder aliases"]
  ...
  pkg_cron --> svc_idealizeCron
  pkg_directory_picker --> svc_directoryPicker
  pkg_directory_picker_browse --> svc_directoryPicker
  pkg_directory_picker_native --> svc_directoryPicker
  pkg_doc_policy --> svc_docPolicy
  pkg_e2b --> svc_e2b
  pkg_freetokens --> svc_idealizeAttribution
  pkg_fs --> svc_fs
  pkg_fs_e2b --> svc_fs
  pkg_fs_local --> svc_fs
  pkg_fs_sandbox --> svc_fs
  pkg_gen_fixture --> svc_generation
  pkg_generate --> svc_generation
  pkg_goal --> svc_goals
  pkg_host_bridge --> svc_idealizeBridge
  ...
  pkg_provider_pack --> svc_idealizeOAuth
  pkg_pwsh_local --> svc_shell
  pkg_sandbox --> svc_sandbox
  pkg_sandbox_local --> svc_sandbox
  pkg_sandbox_policy --> svc_sandboxPolicy
  pkg_services --> svc_generation
  ...
  pkg_setup --> svc_workspaceAliases
  pkg_shell --> svc_shell
  pkg_shell_env --> svc_shellEnv
  pkg_skill --> svc_skills
  pkg_skill_badge --> svc_skills
  pkg_skill_filesystem --> svc_skills
  pkg_skills --> svc_idealizeSkills
  ...
  pkg_tools --> svc_tools
  pkg_transcribe --> svc_transcription
  pkg_typert_registry --> svc_typert
  pkg_ui_terminal --> svc_idealizeTerminals
  ...
  svc_artefacts --> pkg_gen_tools
  svc_artefacts --> pkg_ui_soundstage
  ...
  svc_docPolicy --> pkg_setup
  svc_docPolicy --> pkg_vault
  svc_dynamicCordisRunner --> pkg_tool_cordis
  svc_e2b --> pkg_fs_e2b
  svc_e2b --> pkg_subprocess_e2b
  svc_fs --> pkg_tool_fs
  svc_generation --> pkg_gen_tools
  svc_generation --> pkg_spaces
  svc_idealizeAttribution --> pkg_llm_pi_ai
  svc_idealizeComm --> pkg_activity_pills
  svc_idealizeOAuth --> pkg_activity_pills
  svc_idealizeOAuth --> pkg_llm_pi_ai
  svc_idealizeOAuth --> pkg_models
  svc_idealizeSkills --> pkg_ui_bar
  svc_idealizeStudio --> pkg_comm
  svc_idealizeTerminals --> pkg_askbar
  ...
  svc_workspaceAliases --> pkg_ui_bar
...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | [`web-search-exa`](../packages/web/web-search-exa), [`web-search-perplexity`](../packages/web/web-search-perplexity), [`web-search-deepseek`](../packages/web/web-search-deepseek), `web-search-openrouter`, [`web-fetch-http`](../packages/web/web-fetch-http) | [`tool-web`](../packages/web/tool-web) | - | Search and fetch providers register into one ctx.web seam; tool-web owns the stable model-facing names. |
...
| `ctx.artefacts` | `core` | `artefacts` | - | `gen-tools`, `ui-soundstage` | - | Stores generated media bytes under the workspace and appends the ignorable `artefact/*` session events; readers page the log for tiles. |
| `ctx.generation` | `seam` | `generate` | `services`, `gen-fixture` | `gen-tools`, `spaces` | - | Backends publish typed media models; the tools route a request to the stored per-preset model or the first compatible one, and the space roster reads availability off the same catalogue. |
| `ctx.docPolicy` | `core` | `doc-policy` | - | `vault`, `setup` | - | Classifies project documents by convention and refuses writes the policy forbids; the vault and first-run setup read the same policy. |
| `ctx.idealizeAttribution` | `core` | `freetokens` | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai) | - | Supplies the attribution headers a free-tokens route request carries; the adapter reads them per provider id. |
| `ctx.idealizeBridge` | `core` | `host-bridge` | - | - | - | Buffers host events for the desktop shell and serves them over its loopback routes; no plugin consumes it directly. |
| `ctx.idealizeComm` | `core` | `comm` | - | `activity-pills` | - | Names sessions, delivers messages between them, and owns the role presets; the activity host reads the role configuration to exclude roles from the brain roster. |
| `ctx.idealizeStudio` | `core` | `studio` | - | `comm` | - | Records the durable per-project Studio events, folds them into task state, and serves both over its loopback routes; the command surface writes and reads it for the CLI task commands. |
| `ctx.idealizeTerminals` | `core` | `ui-terminal` | - | `askbar` | - | Reports which terminal chats are working, derived from their shells' output; the Askbar's roster route probes it so a terminal agent's chip shows the working state a harness agent announces for itself. |
| `ctx.idealizeSkills` | `core` | `skills` | - | `ui-bar` | - | Answers the skills folder's packages grouped by subfolder, with each unpacked manifest's name and description; the tool rail's skills route probes it to fill the composer's Skills & Commands submenu and marks each entry with whether the registry offers that name. |
| `ctx.transcription` | `seam` | `transcribe` | `transcribe` | - | - | Providers register speech models behind an explicit resolve step; the Askbar's hold gesture reaches the resolved provider over the package's own loopback routes, so no plugin injects the service. |
| `ctx.idealizeCron` | `core` | `cron` | - | - | - | Owns the task store and the tick that runs due tasks as sessions; the Schedule pane reaches it over its loopback routes only. |
| `ctx.idealizeOAuth` | `core` | `provider-pack` | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai), `models`, `activity-pills` | - | Persists OAuth tokens per provider and refreshes them; the adapter settles OAuth-only routes through it and the model directory gates on it. |
| `ctx.workspaceAliases` | `core` | `setup` | - | `ui-bar` | - | Resolves the named folder aliases a project declares; the Files pane lists them as tabs. |
