...
<a id="ctxgeneration--generationruntime"></a>

### `ctx.generation` — `GenerationRuntime`

The generation service. Registered as `ctx.generation` (one instance per context). Also mounts the `/idealize/brains/media` preset routes and the `/idealize/generate/inputs` route while a web server and settings service are composed beside it.
...
Source: [`packages/idealize/generate/src/service.ts:45`](../../packages/idealize/generate/src/service.ts)
