...
<a id="ctxartefacts--artefactstore"></a>

### `ctx.artefacts` — `ArtefactStore`

The artefact engine service (`ctx.artefacts`): durable records over the storage domain, per-project byte storage, session-event publication, the raw route, and the `artefacts_get` tool.
