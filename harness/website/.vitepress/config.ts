...
/** The modules the guide sidebar links into, one home shared with the navigation bar. */
const modules: readonly ModuleLink[] = [
  { label: 'Development', collection: 'develop' },
  { label: 'Reference', collection: 'reference' },
]
...
function guideSidebar(): DefaultTheme.SidebarItem[] {
  return [
    ...sidebar('guide'),
    ...modules.map(({ label, collection }) => ({ text: label, link: landingLink(collection) })),
  ]
}

/**
 * Navigation-bar items for the modules the guide sidebar links into.
 *
 * @returns The module items for the navigation bar.
 */
function moduleNav(): DefaultTheme.NavItem[] {
  return modules.map(({ label, collection }) => ({
    text: label,
    link: landingLink(collection),
    activeMatch: `^/${collection}/`,
  }))
}
