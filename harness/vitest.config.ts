  ...
  test: {
    ...
    coverage: {
      ...
      exclude: [
        ...
        // IDEalize's browser halves. Every file under a fork package's
        // src/client/ runs inside the Electron renderer against the shell's
        // real slot registry, its stylesheet cascade and its IPC bridge; the
        // jsdom lane reaches the pure stores and view helpers but not the
        // mount/teardown, slot-occupancy and layout branches. The rendered
        // proof harness (.idealize/proof) walks these surfaces in the packaged
        // window instead. TODO(gui): cover and remove with the client test
        // lane above.
        'packages/idealize/*/src/client/**/*.{ts,tsx}',
        // The pi-ai OAuth login driver: every path runs a live provider
        // handshake (a PKCE authorize URL served to a localhost callback, or
        // a device-code poll) that no offline harness reaches. The sign-in
        // walk against the packaged app drives it; the pack's store, status,
        // routes and fences stay gated.
        'packages/idealize/provider-pack/src/oauth-login.ts',
        // The `idealize` command-line face executes at module scope (it reads
        // process.argv and exits), so importing it would run the CLI inside
        // the unit process — the same reason src/bin.ts is excluded above.
        // tests/cli.spec.ts drives the built entry as a real subprocess.
        'packages/idealize/comm/src/cli.ts',
