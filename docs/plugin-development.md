# IDEalize V1 Plugin Development
...
A normal DSH plugin can provide Host services, commands, routes, bundles, or a Web Client. It should depend on upstream DSH contracts whenever possible so the same package can work in the CLI, an ordinary Web profile, and IDEalize V1.
...
`desktopPnpm.run(args)` is a low-level pnpm operation with the active profile as its cwd. It does not promise profile initialization, caller-relative `file:`/`link:` anchoring, or `dsh.profile.bundles` reconciliation.
...
Read the [architecture](architecture.md) next, then use the package-level [service contract](../dsh-plugin-desktop/docs/plugin-services.md) as the API reference.
...
We advocate a browser-plugin style of development: everyone extends the same platform against the same conventions, instead of each maintaining a modified runtime of their own. IDEalize V1 follows this approach: the desktop shell itself is an ordinary plugin on the same composition path as official and third-party plugins, with no special privileges.
