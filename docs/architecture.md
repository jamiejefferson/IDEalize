# IDEalize V1 Architecture

## Overview

IDEalize V1 is a thin Electron host. It starts the harness Host in Electron's main process; the Host exposes the ordinary Web UI over loopback HTTP/WebSocket. Desktop does not create a second renderer IPC plugin system and does not expose raw Electron APIs to the page.
...
flowchart LR
  ...
  Host --> Upstream[Upstream harness services]
...
- **Web Client** contains the harness web UI and third-party browser contributions. It works over the loopback carrier and does not call Electron directly.
