...
/** Electron platforms supported by the IDEalize native adapter. */
...
/**
 * Legacy stored value from the pre-Askbar mini shell (removed 2 Sep 2026).
 * Settings files written by earlier runs may still hold it; every read
 * normalises it to `compatibility` through {@link normalizeDesktopShellMode}.
 */
export type LegacyDesktopShellMode = 'mini'

/**
 * Map a stored shell mode onto a supported one.
 * @param mode - the stored value, possibly the legacy `mini`.
 * @param warn - sink for the one-line notice when a legacy value is replaced.
 * @returns the supported mode the generation runs in.
 */
export function normalizeDesktopShellMode(
  mode: DesktopShellMode | LegacyDesktopShellMode,
  warn: (message: string) => void,
): DesktopShellMode {
  if (mode !== 'mini') return mode
  warn('dsh-plugin-desktop: the stored shell mode "mini" is no longer supported; running in compatibility mode (collapse to the Askbar with Ctrl+Alt+A)')
...
export interface DesktopShellSpec extends DesktopWindowConfig {
  ...
  url: string
  /** The same Web root marked `dsh-desktop-mode=askbar`, loaded by the Askbar window. */
  askbarUrl: string
  ...
  /** Read the mode persisted in settings right now (it may differ from `mode` while a restart is pending). */
  readShellMode(): DesktopShellMode
  /** Read the live Askbar dock edge (the `askbarSide` desktop setting). */
  readAskbarSide(): AskbarSide
  /** Persist another Askbar dock edge through the registered desktop settings scope. */
  requestAskbarSide(side: AskbarSide): Promise<void>
  /** Read where a drag last left the Askbar (the `askbarPosition` desktop setting), when one is stored. */
  readAskbarPosition(): AskbarPosition | undefined
  /** Persist where a drag left the Askbar through the registered desktop settings scope. */
  requestAskbarPosition(position: AskbarPosition): Promise<void>
}
...
export interface DesktopRuntime {
  ...
  /** Re-dock the Askbar after its edge preference changed and rebuild the tray. */
  refreshAskbar(): void
  ...
  /** Read the renderer view state persisted by a previous generation. */
  readViewState(): DesktopViewState | undefined

  /**
   * Persist the renderer view state for the next generation.
   * @param state - validated view state recorded by the renderer.
   */
  writeViewState(state: DesktopViewState): void

  /**
   * Ask whether a shell-mode switch may end the live embedded terminals.
   * @param count - live terminal count named in the dialog.
   * @returns whether the user confirmed the switch.
   */
  confirmDiscardTerminals(count: number): Promise<boolean>
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Electron adapter provided by the IDEalize launcher. */
