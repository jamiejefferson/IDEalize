/**
 * Headless version checks against the IDEalize V1 GitHub release. The latest
 * release's `tag_name` (`v1.2.3`) is the published version; the installers
 * are that release's assets, reached through the fixed latest-download URLs
 * in `update-download.ts`. Nothing else stands between the app and the
 * release: no version service to run, no key to hold.
 */

/** The GitHub repository the releases are published on. */
export const DESKTOP_RELEASE_REPOSITORY = 'jamiejefferson/IDEalize'

/** Public endpoint describing the latest published (non-draft, non-prerelease) release. */
export const DESKTOP_VERSION_ENDPOINT = `https://api.github.com/repos/${DESKTOP_RELEASE_REPOSITORY}/releases/latest`

/**
 * Maximum response body bytes accepted from the release endpoint. A release
 * document carries its notes and asset list; 256 KiB holds any release this
 * app publishes and still bounds what a hostile answer can make the app read.
 */
...
/**
 * Check the latest GitHub release for a newer stable version.
...
/**
 * The release's version from its `tag_name`: `v1.2.3` or `1.2.3`, stable
 * only. A draft or prerelease never reaches `releases/latest`, and a tag that
 * is not a version is not an update.
 */
function parseVersionResponse(body: string): ParsedSemVer | null {
  ...
  return parseCanonicalStableVersion(tag.startsWith('v') ? tag.slice(1) : tag)
}
