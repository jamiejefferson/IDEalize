/** Headless, confirmation-gated downloads for IDEalize installers. */
...
/**
 * Fixed download URLs: GitHub's latest-release redirect to the assets the
 * release workflow uploads under version-free names beside the versioned
 * ones, so the app never has to read an asset list. Windows is unbuilt in
 * V1; its URL names the asset a Windows release would carry.
 */
export const DESKTOP_DOWNLOAD_URLS: Readonly<Record<DesktopDownloadPlatform, string>> = {
  darwin: 'https://github.com/jamiejefferson/IDEalize/releases/latest/download/IDEalize-V1-mac.dmg',
  win32: 'https://github.com/jamiejefferson/IDEalize/releases/latest/download/IDEalize-V1-Setup.exe',
}
...
export function desktopUpdateFilename(platform: DesktopDownloadPlatform, version: string): string {
  ...
  return `IDEalize-${version}-${platformName}.${extension}`
}
