...
import { normalizeDesktopShellMode, type DesktopShellMode } from './runtime.ts'
import {
...
/**
 * Parse desktop presentation state and reject corrupted values. The legacy
 * `mini` value from the pre-Askbar shell is accepted and runs as
 * `compatibility`, with one stderr notice per process.
...
export function parseDesktopShellMode(value: unknown): DesktopShellMode {
  ...
  if (value === 'mini') return normalizeDesktopShellMode(value, warnLegacyShellModeOnce)
  throw new Error(`${BIN_NAME}: ${DESKTOP_SETTINGS_NAMESPACE}.mode must be "compatibility" or "advanced"`)
}

let legacyShellModeWarned = false
function warnLegacyShellModeOnce(message: string): void {
  if (legacyShellModeWarned) return
  legacyShellModeWarned = true
...
/** Resolve the IDEalize template (falling back to the public Web template) once. */
function requiredWebBundles(): string[] {
  const bundles = PROFILE_TEMPLATES.idealize ?? PROFILE_TEMPLATES.web
  if (bundles === undefined) {
    throw new Error(`${BIN_NAME}: installed dsh-app-boot has no idealize or web profile template`)
  }
  return [...bundles]
}
...
export function prepareDesktopProfile(
  ...
  if (mode === 'advanced') {
    // The advanced shell replaces the web app's root slot, so the upstream
    // layout row stands down while its child occupants stay mounted.
