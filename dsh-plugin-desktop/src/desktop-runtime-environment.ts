...
export interface DesktopPnpmRuntimeOptions {
  ...
  pnpmBinPath: string
  /** Physical packaged `@idealize/comm` CLI entry, published on PATH as `idealize`. */
  idealizeCliPath: string
...
export interface DesktopPnpmRuntimeInstallation {
  /** Public directory prepended to the Host PATH; it contains only pnpm and idealize. */
  pathDir: string
  /** Public pnpm command shim. */
  pnpmShimPath: string
  /** Public idealize command shim: the packaged comm CLI under the private Node. */
  idealizeShimPath: string
...
/** Build the public POSIX idealize command: the packaged comm CLI run by the private Node shim. */
function posixIdealizeShim(options: DesktopPnpmRuntimeOptions, nodeShimPath: string): string {
  return [
    '#!/bin/sh',
    `exec ${quoteSh(nodeShimPath)} ${quoteSh(options.idealizeCliPath)} "$@"`,
    '',
  ].join('\n')
}

/** Build the public Windows idealize command: the packaged comm CLI run by the private Node shim. */
function windowsIdealizeShim(options: DesktopPnpmRuntimeOptions, nodeShimPath: string): string {
  return [
    '@echo off',
    'setlocal DisableDelayedExpansion',
    `call ${quoteBatchWord(nodeShimPath)} ${quoteBatchWord(options.idealizeCliPath)} %*`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
...
export function installDesktopPnpmRuntime(options: DesktopPnpmRuntimeOptions): DesktopPnpmRuntimeInstallation {
  ...
  for (const [label, value] of [
    ...
    ['idealize CLI entry', options.idealizeCliPath],
  ...
  replacePrivateFile(
    idealizeShimPath,
    windows ? windowsIdealizeShim(options, nodeShimPath) : posixIdealizeShim(options, nodeShimPath),
    windows ? PRIVATE_FILE_MODE : EXECUTABLE_FILE_MODE,
  )
