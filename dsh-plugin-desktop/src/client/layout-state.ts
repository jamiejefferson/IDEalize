...
export class DesktopLayoutState implements DesktopLayoutService {
  ...
  /**
   * No-op: the advanced frame renders no drawer column — the rail panes are
   * hosted there only by the compatibility shell. The method
   * exists so vendored pane plugins (the appearance follower's ⌘⌥A path)
   * can call the full layout service without throwing.
   */
  openDrawer(): void {}

  /** No-op: see {@link openDrawer}. */
  closeDrawer(): void {}

  /** No-op: the advanced frame renders no deck column (see {@link openDrawer}). */
  openDeck(): void {}

  /** No-op: see {@link openDeck}. */
  closeDeck(): void {}
