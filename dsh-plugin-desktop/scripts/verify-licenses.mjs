...
/**
 * Whether a license expression is on the allowlist. An SPDX `OR` expression
 * (`(MIT OR CC0-1.0)`, as type-fest declares) lets the redistributor choose
 * either side, so it is accepted when any alternative is allowed.
 * @param {string} license - the manifest's license expression.
 * @returns {boolean} true when the package may ship inside the installer.
 */
function isRedistributable(license) {
  const expression = license.trim().replace(/^\((.*)\)$/u, '$1')
  if (ALLOWED_LICENSES.has(expression)) return true
  if (!/ OR /u.test(expression) || / AND /u.test(expression)) return false
  return expression.split(/ OR /u).some((alternative) => ALLOWED_LICENSES.has(alternative.trim()))
}
...
for (let index = 0; index < queue.length; index += 1) {
  ...
  if (current.name !== rootManifest.name) {
    ...
    } else if (license !== undefined && !isRedistributable(license) && !NOTICE_LICENSES.has(license)) {
...
if (noticesArg !== -1) {
  ...
  const lines = [
    '# Third-Party Notices',
    '',
    'IDEalize V1 distributes the following third-party packages inside its installers.',
