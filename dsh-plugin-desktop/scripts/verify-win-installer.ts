...
export interface WindowsInstallerVerificationOptions {
  ...
  /** Electron Builder `productName`, which names the unpacked executable. */
  readonly productName: string
}

/** The manifest fields the Windows verifiers read. */
export interface WindowsProductManifest {
  /** Product version embedded in the expected artifact name. */
  readonly version: string
  /** Electron Builder `productName`, which names the unpacked executable. */
  readonly productName: string
...
/**
 * Read the version and Electron Builder product name from the desktop manifest.
 * @param desktopRoot - Desktop package root containing package.json.
 * @returns The version and product name; throws when either is missing.
 */
export function readWindowsProductManifest(desktopRoot: string): WindowsProductManifest {
  ...
  if (typeof productName !== 'string' || productName.length === 0) {
    throw new Error(`desktop package at ${desktopRoot} has no build.productName`)
  }
  return { version: manifest.version, productName }
}
...
export function verifyWindowsInstaller(
  ...
  const installerPath = join(
    distDir,
    `IDEalize-${options.version}-x64-Setup.exe`,
  )
  const applicationPath = join(distDir, 'win-unpacked', `${options.productName}.exe`)
