...
A Markdown file (`.md`, `.markdown`) opens the same way (feedback 9583834d, 24 Sep 2026: "tried to set-up md files to open in idealize but it didn't work"). The manifest lists the extensions with the `Viewer` role and, on macOS, the `Alternate` rank, so IDEalize appears under Finder's "Open with" without taking the default from the editor the person already uses; the Windows installer registers the extension as electron-builder does, which does make IDEalize the handler for `.md` where no default was chosen. The path arrives as the keys file does, and `src/main.ts` holds it until the host bridge exists, then pushes an `open-file` event carrying the resolved path; the window shows the file in the deck's viewer. The viewer serves any file under the home folder as it is; for a file elsewhere the event also names a `folder` (the repository holding the file, else its own folder) that the window registers as a project first, as "Idealize this" would. `src/opened-file.ts` recognises the extensions, decides the folder and builds the event, and holds the argument scan the keys file shares.
