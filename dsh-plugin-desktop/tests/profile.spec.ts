...
describe('desktop profile composition', () => {
  ...
  it('runs the legacy mini mode as compatibility with one stderr notice', () => {
    const stderr = vi.spyOn(process.stderr, 'write').mockImplementation(() => true)
    try {
      expect(desktopShellModeFromSettings({ 'dsh-desktop': { mode: 'mini' } })).toBe('compatibility')
      expect(desktopShellModeFromSettings({ 'dsh-desktop': { mode: 'mini' } })).toBe('compatibility')
      const notices = stderr.mock.calls.filter(([chunk]) => String(chunk).includes('"mini"'))
