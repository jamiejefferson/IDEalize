...
/** The latest-release document GitHub answers with, reduced to the field the checker reads. */
function versionResponse(tag: unknown, init: ResponseInit = {}): Response {
  return Response.json({ tag_name: tag, draft: false, prerelease: false, assets: [] }, init)
}
...
describe('public Desktop version check', () => {
  it('reads the latest GitHub release, no-cache, and reports a newer stable version from its tag', async () => {
    const controller = new AbortController()
    const calls: Array<{ url: string, init: RequestInit }> = []
    const request: UpdateRequest = async (url, init) => {
      calls.push({ url, init })
      return versionResponse('v2.10.0')
    }
    ...
    expect(calls[0]?.url).toBe('https://api.github.com/repos/jamiejefferson/IDEalize/releases/latest')
  ...
  it('reads a tag with or without the leading v', async () => {
    for (const tag of ['v2.1.0', '2.1.0']) {
      await expect(checkForStableUpdate({
        currentVersion: '2.0.0',
        request: async () => versionResponse(tag),
      })).resolves.toEqual({ status: 'update-available', currentVersion: '2.0.0', latestVersion: '2.1.0' })
    }
  })
  ...
  it.each([
    ...
    ['a tag that is not a version', { tag_name: 'landing-38' }],
    ['missing tag', {}],
    ['the retired version field', { version: '2.1.0' }],
