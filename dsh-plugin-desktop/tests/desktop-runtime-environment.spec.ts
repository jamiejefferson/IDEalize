...
function options(
  ...
  return {
    ...
    idealizeCliPath: platform === 'win32'
      ? 'C:\\Program Files\\DSH Desktop\\resources\\app.asar.unpacked\\node_modules\\@idealize\\comm\\lib\\cli.js'
      : "/Applications/DSH O'Brien.app/Contents/Resources/app.asar.unpacked/node_modules/@idealize/comm/lib/cli.js",
...
describe('desktop Host pnpm runtime', () => {
  it.each(['darwin', 'linux'] as const)('creates a public PATH holding pnpm and idealize on %s', (platform) => {
    ...
    expect(readdirSync(installation.pathDir)).toEqual(['idealize', 'pnpm'])
    ...
    if (process.platform !== 'win32') {
      ...
      expect(lstatSync(installation.idealizeShimPath).mode & 0o777).toBe(0o700)
    ...
    // The idealize command runs the packaged comm CLI under the private Node shim.
    const idealize = readFileSync(installation.idealizeShimPath, 'utf8')
    expect(idealize).toContain(`exec '${installation.nodeShimPath}' `)
    expect(idealize).toContain(`@idealize/comm/lib/cli.js' "$@"`)
    expect(idealize).not.toContain('npm_config_')
    ...
    if (process.platform !== 'win32') {
      ...
      expect(spawnSync('/bin/sh', ['-n', installation.idealizeShimPath]).status).toBe(0)
  ...
  it('creates Windows batch shims without publishing the private Node directory', () => {
    ...
    expect(readdirSync(installation.pathDir)).toEqual(['idealize.cmd', 'pnpm.cmd'])
    expect(readdirSync(installation.nodeBinDir)).toEqual(['node.cmd'])
    const idealize = readFileSync(installation.idealizeShimPath, 'utf8')
    expect(idealize).toContain(`call "${installation.nodeShimPath}" "`)
    expect(idealize).toContain('comm\\lib\\cli.js" %*')
    const clearEnvironmentUrl = pathToFileURL(installation.clearEnvironmentPath).href
  ...
  it('removes only strictly named stale atomic files before validating commands', () => {
    ...
    expect(readdirSync(pathDir)).toEqual(['idealize', 'pnpm'])
    installation.dispose()
  })
