...
function fixture(version = '2.0.0'): { readonly root: string; readonly portable: string } {
  ...
  const portable = join(dist, `IDEalize-${version}-x64-Portable.zip`)
  const archive = new AdmZip()
  archive.addFile('IDEalize V1.exe', portableExecutable())
...
describe('Windows portable artifact verification', () => {
  it('accepts the exact versioned portable ZIP archive', () => {
    ...
    expect(verifyWindowsPortable(options(value.root))).toBe(value.portable)
  })

  it('rejects a stale portable archive from a different version', () => {
    const value = fixture('1.9.0')

    expect(() => verifyWindowsPortable(options(value.root)))
      .toThrow('IDEalize-2.0.0-x64-Portable.zip')
  })
  ...
  it('rejects an application entry without a Windows PE header', () => {
    ...
    const archive = new AdmZip()
    archive.addFile('IDEalize V1.exe', invalid)
    archive.addFile('resources/app.asar', Buffer.from('asar'))
  ...
  it('names the archived executable after the product name', () => {
    const value = fixture()

    expect(() => verifyWindowsPortable({ ...options(value.root), productName: 'DSH Desktop' }))
      .toThrow('missing DSH Desktop.exe')
  })
})
