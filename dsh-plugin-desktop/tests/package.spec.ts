...
describe('published package surface', () => {
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.win?.artifactName).toBe('IDEalize-${version}-${arch}-Portable.${ext}')
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    // onnxruntime-node ships one macOS slice per install (arm64 on this Mac),
    // so the universal merge is told those files are per-arch rather than
    // refusing the identical copy it finds in both builds.
    expect(manifest.build?.mac).toEqual(expect.objectContaining({
      x64ArchFiles: expect.stringContaining('onnxruntime-node/bin/napi-v6/darwin/**'),
    }))
  ...
  it('keeps one fixed accent-blue IDEalize monogram as the tray source for generated native assets', () => {
    const source = readFileSync(new URL('build/tray-icon.svg', packageRoot), 'utf8')

    expect(source.match(/#0757B4/gu)).toHaveLength(1)
  ...
  it('keeps the IDEalize source icon unmodified', () => {
    const digest = createHash('sha256')
      .update(readFileSync(new URL('build/app-icon.png', packageRoot)))
      .digest('hex')

    expect(digest).toBe('cce8f21ad740b812941bb96184e6cbddabe56151e6f60e16c1bd3f4c694faca1')
  })

  it('generates the macOS icon from the IDEalize artwork inside the 100-pixel safe-area inset', async () => {
