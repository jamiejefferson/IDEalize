...
import { desktopProductVersion } from '../src/electron-runtime.ts'
...
const electron = vi.hoisted(() => {
  ...
  // The frameless Askbar window is tracked apart from the app windows so the
  // app-window assertions keep their indices.
  const askbarWindowOptions: unknown[] = []
  const askbarWindows: BrowserWindow[] = []
  ...
  class BrowserWindow {
    ...
    constructor(options: unknown) {
      if ((options as { frame?: boolean }).frame === false) {
        askbarWindowOptions.push(options)
        askbarWindows.push(this)
      } else {
    ...
    // Geometry state exercised by the collapse/expand transform.
    bounds = { x: 100, y: 80, width: 1280, height: 840 }
    normalBounds = { x: 100, y: 80, width: 1280, height: 840 }
    maximized = false
    fullScreen = false
    ...
    readonly maximize = vi.fn(() => { this.maximized = true })
    readonly unmaximize = vi.fn(() => { this.maximized = false })
    readonly isFullScreen = vi.fn(() => this.fullScreen)
    readonly setFullScreen = vi.fn((flag: boolean) => { this.fullScreen = flag })
    readonly setMinimumSize = vi.fn((width: number, height: number) => { this.minimumSize = { width, height } })
    readonly setAlwaysOnTop = vi.fn((flag: boolean) => { this.alwaysOnTop = flag })
    visible = false
...
const spec: DesktopShellSpec = {
  ...
  askbarUrl: 'http://127.0.0.1:43120/?dsh-desktop-mode=askbar',
  ...
  readShellMode: () => 'compatibility',
  readAskbarSide: () => 'right' as const,
  requestAskbarSide: vi.fn(async () => {}),
  readAskbarPosition: () => undefined,
  requestAskbarPosition: vi.fn(async () => {}),
}
...
describe('Electron compatibility runtime', () => {
  beforeEach(() => {
    ...
    electron.askbarWindowOptions.length = 0
    electron.askbarWindows.length = 0
  ...
  it('cycles the tray mode between compatibility and advanced only', async () => {
    const { nextDesktopShellMode } = await import('../src/electron-runtime.ts')
    expect(nextDesktopShellMode('compatibility')).toBe('advanced')
    expect(nextDesktopShellMode('advanced')).toBe('compatibility')
  })
    ...
    expect(electron.menuTemplates[0]).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        submenu: expect.arrayContaining([
          expect.objectContaining({ label: 'Advanced', type: 'radio', enabled: true }),
        ]),
      }),
    ]))
  ...
  it('selects the restricted Linux platform adapter once for native capabilities', async () => {
    ...
    expect(electron.menuTemplates[0]).toEqual(expect.arrayContaining([
      expect.objectContaining({
        label: 'Window Mode',
        submenu: expect.arrayContaining([
          expect.objectContaining({ label: 'Advanced', type: 'radio', enabled: false }),
        ]),
      }),
    ]))
  ...
  it('persists the opposite mode when its tray command is clicked', async () => {
    ...
    await runtime.mountScheduled()
    const modeMenu = (electron.menuTemplates[0] as Array<{ label?: string, submenu?: Array<{ label?: string, click?: () => void }> }>)
      .find(candidate => candidate.label === 'Window Mode')
    expect(modeMenu).toBeDefined()
    const item = modeMenu?.submenu?.find(candidate => candidate.label === 'Advanced')
  ...
  it('opens the active profile through the packaged terminal adapter', async () => {
    ...
    try {
      ...
      expect(terminal.open).toHaveBeenCalledWith(expect.objectContaining({
        ...
        profileName: 'desktop',
        productVersion: desktopProductVersion(),
  ...
  it('shows native errors for synchronous and asynchronous terminal launch failures', async () => {
    ...
    try {
      ...
      expect(electron.dialog.showErrorBox).toHaveBeenCalledWith(
        'Unable to Open IDEalize Terminal',
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    expect(runtime.updates).toMatchObject({
      isPackaged: false,
      canDownload: false,
      currentVersion: desktopProductVersion(),
  ...
  it('uses advanced macOS material options and offers compatibility mode', async () => {
    ...
    expect(electron.menuTemplates[0]).toEqual(expect.arrayContaining([
      expect.objectContaining({
        label: 'Window Mode',
        submenu: expect.arrayContaining([
          expect.objectContaining({ label: 'Standard', type: 'radio', checked: false }),
          expect.objectContaining({ label: 'Advanced', type: 'radio', checked: true }),
        ]),
      }),
    ]))
