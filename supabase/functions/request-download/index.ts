...
// Public function (verify_jwt=false); the domain check + rate limits are the gate.
...
const MAX_PER_IP = 10;      // requests per IP per window (see ip_rate_limits)
...
// Rejection sampling: discard the biased tail of the 2^32 range so every
// 6-digit code is equally likely.
...
Deno.serve(async (req) => {
  ...
  if (!EMAIL_RE.test(email)) return json({ error: `Only @${DOMAIN} addresses are eligible.` }, 400);
  ...
  // one generic 429 for either limit; also the fail-closed answer on limiter errors
  const limited = () => json({ error: "Too many requests. Wait a few minutes and try again." }, 429);

  // per-IP limit (atomic counter via check_ip_rate_limit())
  const { data: ipOk, error: ipErr } = await sb.rpc("check_ip_rate_limit", {
    p_ip: clientIp(req), p_limit: MAX_PER_IP, p_window_min: WINDOW_MIN,
  });
  if (ipErr) { console.error("ip rate limit", ipErr); return limited(); }
  if (!ipOk) return limited();

  // per-email limit: at most MAX_PER_WINDOW codes per email per WINDOW_MIN
  const since = new Date(Date.now() - WINDOW_MIN * 60_000).toISOString();
  const { count, error: cntErr } = await sb.from("download_codes")
    .select("id", { count: "exact", head: true })
    .eq("email", email).gte("created_at", since);
  if (cntErr) { console.error("email rate limit", cntErr); return limited(); }
  if ((count ?? 0) >= MAX_PER_WINDOW) return limited();
