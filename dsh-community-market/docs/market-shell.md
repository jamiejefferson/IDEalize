...
Status: delivered and built into IDEalize V1, including the Host/Client market, limited npm installation, receipt-backed uninstall, and direct-bundle enable/disable
...
[DSH 1024Store](https://github.com/imsai-sh/awesome-deepseek-harness-plugins) is one of the providers currently cooperating with the project, and the market includes a reviewed built-in adapter for its public API. A never-used registry is seeded with it selected on first read (and with dshfind saved unselected); the seed is a starting point the person can change or remove, not a preferred source or fallback, and the cooperation does not mean that its listings were reviewed or endorsed. Its endpoint and schema remain owned by that independent project.

[dshfind](https://dshfind.com) is another optional cooperating source with a reviewed built-in adapter. The seed saves it unselected; it is not preferred, recommended, or used as a fallback. Its listings, scores, grades, `official`/featured labels, risk labels, and installation probes remain provider claims rather than Anywhere Labs trust decisions.
...
- The Host/Client package is delivered as a built-in IDEalize V1 implementation.
