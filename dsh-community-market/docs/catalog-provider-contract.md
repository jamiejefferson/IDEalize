...
Status: **Implemented public v1 contract.** The versioned Schemas, generated types, strict validation, source persistence, constrained network and media boundaries, standard HTTP adapter, reviewed DSH 1024Store and browse-only dshfind adapters, complete local indexing, and loadable Host/Client entries are implemented and tested in IDEalize V1. This document and its fixtures are the public interoperability contract for `manifestVersion` and `schemaVersion` `1.x`.
