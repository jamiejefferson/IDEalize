...
describe('MarketSettingsTab', () => {
  ...
  it('translates a built-in source description on the current row and on the Add card', async () => {
    vi.mocked(readMarketState).mockResolvedValue({
      ...availableState,
      sources: [{ ...makeSecondSource(true), descriptionKey: 'partnerSourceDescription' }],
    })
    vi.mocked(readMarketCatalog).mockResolvedValue(catalog)
    render(<MarketSettingsTab {...props} />)

    fireEvent.click(screen.getByRole('button', { name: en.sources }))
    // Once for the seeded current source, once for the built-in still on offer.
    expect(await screen.findAllByText(en.partnerSourceDescription)).toHaveLength(2)
    expect(screen.queryByText('https://second.example/v1/plugins')).toBeNull()
  })
