...
/** A registry the user has emptied after the partner sources were offered: no seed on read. */
const offeredEmptyRegistry = (): SharedMarketSettings => ({ document: { sources: [], builtInSourcesSeeded: true } })
...
describe('community market Host routes', () => {
  ...
  it('seeds both partner sources on the first read of a new registry and never reseeds after removal', async () => {
    const settings: SharedMarketSettings = { document: { sources: [] } }
    const server = await startMarketServer([], settings)
    try {
      const first = await readRoute(server, marketRoutes.state)

      expect(first.status).toBe(200)
      const state = await first.json() as { sources: readonly { sourceRecordId: string }[] }
      expect(state).toMatchObject({
        sources: [
          { builtInProviderKey: DSH_1024STORE_KEY, name: 'DSH 1024Store', enabled: true, order: 0, descriptionKey: 'partnerSourceDescription' },
          { builtInProviderKey: DSHFIND_KEY, name: 'dshfind', enabled: false, order: 1, descriptionKey: 'partnerSourceDescription' },
        ],
      })
      expect(settings.document.builtInSourcesSeeded).toBe(true)
      // The seed counts as added: a second Add of the same provider is refused.
      const duplicate = await mutateSource(server, { action: 'add-builtin', key: DSH_1024STORE_KEY })
      expect(duplicate.status).toBe(400)
      await expect(duplicate.json()).resolves.toEqual({ error: 'source already added' })
      // Removing both leaves the registry empty on the next read.
      for (const source of state.sources) {
        expect((await mutateSource(server, { action: 'remove', sourceRecordId: source.sourceRecordId })).status).toBe(200)
      }
      const emptied = await readRoute(server, marketRoutes.state)
      await expect(emptied.json()).resolves.toMatchObject({ sources: [] })
    } finally {
      await server.close()
    }
  })
  ...
  it.each([
    ...
    const server = await startMarketServer([], offeredEmptyRegistry())
    try {
