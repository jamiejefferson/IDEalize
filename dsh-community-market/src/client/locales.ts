export const zh = {
  ...
  manualInstallBody: '以下命令由 IDEalize V1 根据插件信息生成，可能与仓库中的说明不同。请先检查源码，再复制到 DSH 终端执行。',
  installCommand: '安装命令',
  ...
  restartError: '无法请求重启 IDEalize V1，请稍后手动重启。',
  installableBody: '这里显示从当前来源完整目录中初步筛选出的可安装插件。选择插件后，IDEalize V1 仍会再次验证版本、来源和兼容性。',
  ...
  desktopRequiredTitle: '需要 IDEalize V1',
  desktopUnavailable: '安装、卸载、禁用和启用只在 IDEalize V1 中可用；你仍然可以浏览插件目录。',
  ...
  confirmInstallBody: '请确认 IDEalize V1 验证的 npm 包、版本和目标配置。',
  confirmUninstallTitle: '确认卸载插件',
  confirmUninstallBody: '只会卸载当前配置中由插件市场安装并验证过的这个插件。',
  confirmDisableTitle: '确认禁用外部插件',
  confirmDisableBody: 'IDEalize V1 将停止在当前配置中加载这个插件，但不会卸载对应的 npm 包。',
  confirmEnableTitle: '确认启用插件',
  confirmEnableBody: 'IDEalize V1 将恢复在当前配置中加载这个插件。对应的代码会在重启后运行。',
  confirmInstall: '确认安装',
  ...
  restartAfterOperation: '操作完成后需要重启 IDEalize V1，改动才会生效。',
  disableWarning: '禁用不会卸载 npm 包，也不会隔离插件代码。',
  disableRecoveryWarning: '如果插件导致 IDEalize V1 无法进入此页面，需要按照恢复文档手动处理。',
  ...
  restartRequiredTitle: '需要重启 IDEalize V1',
  restartRequiredBody: '请重启 IDEalize V1，让当前配置加载最新的插件状态。',
  restartLater: '稍后重启',
  restartNow: '立即重启',
  restarting: '正在重启...',
  done: '完成',
  builtIn: '内置适配器',
  partner: '合作提供方',
  partnerSourceDescription: '合作提供方目录，默认启用。目录收录不代表插件经过审核或推荐。',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  manualInstallBody: 'IDEalize V1 generated this command from the plugin information, so it may differ from the repository instructions. Review the source first, then copy it into DSH Terminal.',
  ...
  restartError: 'IDEalize V1 could not be restarted. Restart it manually when convenient.',
  installableBody: 'This view shows plugins pre-screened from the current source’s complete catalog. IDEalize V1 verifies the version, source, and compatibility again after selection.',
  ...
  desktopRequiredTitle: 'IDEalize V1 is required',
  desktopUnavailable: 'Install, uninstall, disable, and enable are available only in IDEalize V1. You can still browse the catalog.',
  ...
  confirmInstallBody: 'Review the npm package, version, and target profile verified by IDEalize V1.',
  ...
  confirmDisableBody: 'IDEalize V1 will stop loading this plugin in the active profile without uninstalling its npm package.',
  confirmEnableTitle: 'Confirm plugin enable',
  confirmEnableBody: 'IDEalize V1 will load this plugin in the active profile again. Its code will run after restart.',
  ...
  restartAfterOperation: 'Restart IDEalize V1 after this operation for the change to take effect.',
  disableWarning: 'Disabling does not uninstall the npm package or isolate the plugin’s code.',
  disableRecoveryWarning: 'If the plugin prevents IDEalize V1 from reaching this page, use the manual recovery steps.',
  ...
  restartRequiredTitle: 'Restart IDEalize V1',
  restartRequiredBody: 'Restart IDEalize V1 so the current profile loads its updated plugin state.',
  ...
  partnerSourceDescription: 'A partner catalogue, on by default. A listing there is not a review or a recommendation.',
