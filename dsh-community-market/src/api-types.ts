...
/** Client dictionary keys a built-in provider's description may name; the client resolves them through its `t`. */
export type MarketSourceDescriptionKey = 'partnerSourceDescription'
...
export interface MarketSourceView extends LocalSourceRecord {
  readonly name: string
  /** Set for a built-in source; the client shows `t(descriptionKey)` before `description`. */
  readonly descriptionKey?: MarketSourceDescriptionKey
  /** A user-added source's manifest description. */
