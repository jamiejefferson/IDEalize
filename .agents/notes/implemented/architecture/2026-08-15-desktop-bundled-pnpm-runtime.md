...
The public runtime directory contains `pnpm` and `idealize` on POSIX or `pnpm.cmd` and `idealize.cmd` on Windows; `idealize` runs the packaged `@idealize/comm` CLI under the private `node` shim, so the command every agent's prompt names exists in the agent's shell (8 Sep 2026). A sibling private helper directory contains the Electron-backed `node` command used by pnpm lifecycle scripts. The pnpm wrapper prepends that helper only inside its own child environment, sets `NODE` so pnpm derives `npm_node_execpath` from the helper instead of Electron's executable, and locally supplies the Electron runtime, target version, and headers URL used to build native dependencies for the Host runtime.
